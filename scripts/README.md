...
- `benchmark_cutdown_for_external_ai.py`: builds a low-token benchmark package (per-run summaries + sampled diagnostics + codex-vs-baseline comparison), then optionally flattens it to markdown (`python scripts/benchmark_cutdown_for_external_ai.py <benchmark_folder> --output-dir <cutdown_folder> --overwrite`).
