...
- `benchmark_cutdown_for_external_ai.py`: builds a benchmark package for external AI review (per-run summaries + sampled diagnostics + codex-vs-baseline comparison), then optionally flattens it to markdown (`python scripts/benchmark_cutdown_for_external_ai.py <benchmark_folder> --output-dir <cutdown_folder> --overwrite`). Defaults keep more context (`--sample-limit 80`, `--excerpt-limit 440`) and now sample prompt request/response logs by category (`--prompt-pairs-per-category`, default 2 pairs each). Without `--output-dir`, default naming is timestamp-driven from discovered run IDs (`<run_id>_cutdown` for one run, `<first>__to__<last>_cutdown` for multi-run). Set `--prompt-pairs-per-category 0` to keep full prompt logs.
