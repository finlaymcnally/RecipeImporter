...
def analyze_exec_activity():
    activity_path = "var/codex_exec_activity.csv"
    output_summary = "docs/reports/codex_activity_line_items.csv"
    
    # Aggregation by (date, task_id or run_id)
    # The task_id/run_id seems to correlate with benchmark runs.
    ...
    with open(activity_path, mode='r', encoding='utf-8') as f:
        ...
        for row in reader:
            try:
                ...
                # Group by hour and run_id to see distinct benchmark events
                run_id = row.get('run_id', 'unknown')
                pipeline = row.get('pipeline_id', 'unknown')
                key = (date_hour, run_id, pipeline)
    ...
    with open(output_summary, mode='w', encoding='utf-8', newline='') as f:
        fieldnames = ['hour', 'run_id', 'pipeline', 'call_count', 'tokens_total', 'tokens_input', 'tokens_output', 'tokens_cached', 'models']
