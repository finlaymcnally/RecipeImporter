...
Externalize Discoveries only for durable, non-obvious findings that are likely to save a future agent meaningful time or prevent a mistake. Do not create notes for routine code reading or temporary exploration or documentation updates. Prefer updating existing docs when possible; create a new file in /docs/understandings only when the knowledge is novel/high value, and is not already documented and can be stated briefly. 
