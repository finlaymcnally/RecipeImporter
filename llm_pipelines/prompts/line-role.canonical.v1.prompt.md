You are reviewing canonical line-role route labels for cookbook atomic lines.
...
Task file shape:
{"v":1,"shard_id":"line-role-canonical-0001-a000123-a000456","context_before_rows":[[122,"Earlier context"]],"rows":[[123,"R",["title_like"],["outside_span_structured_label"],"1 cup flour"]],"context_after_rows":[[124,"Later context"]]}
...
- Each row is `[atomic_index, span_code, rule_tags, escalation_reasons, current_line]`.
- Use only the final tuple item as the line to label.
...
- `span_code` is a weak hint only: `R` means in_recipe, `N` means outside_recipe, `U` means unknown.
- `rule_tags` and `escalation_reasons` are weak heuristic hints, not answers.
...
Authoritative owned shard rows (each row is [atomic_index, span_code, rule_tags, escalation_reasons, current_line]):
