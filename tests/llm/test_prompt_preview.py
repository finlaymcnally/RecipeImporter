...
def _build_existing_run_at(run_dir: Path) -> Path:
    ...
    _write_json(
        ...
        {
            "schema_version": "recipe_row_ownership.v1",
            "ownership_mode": "recipe_boundary_with_explicit_divestment",
            "owned_row_indices": [0, 1],
            "divested_row_indices": [],
            "available_to_nonrecipe_row_indices": [2, 3],
            "row_owner_by_index": {
...
def test_prompt_preview_rebuilds_knowledge_and_line_role_prompts(tmp_path: Path) -> None:
    ...
    assert '<BEGIN_AUTHORITATIVE_ROWS>\n{"block_index": 0, "row_id": "r01", "text": "Ambiguous title-ish line"}' in line_role_row["rendered_prompt_text"]
    assert "one answer row for every owned input row in `rows`" in line_role_row["rendered_prompt_text"]
