"""Offline prediction-run builder for benchmark suites."""

from __future__ import annotations

from pathlib import Path
from typing import Any, Callable

from cookimport.labelstudio.ingest import generate_pred_run_artifacts


def build_pred_run_for_source(
    source_path: Path,
    out_dir: Path,
    *,
    config: dict | None = None,
    progress_callback: Callable[[str], None] | None = None,
    **kwargs: Any,
) -> Path:
    """Build a prediction run for a single source file, fully offline.

    Returns the pred-run directory path.
    """
    cfg = config or {}
    result = generate_pred_run_artifacts(
        path=source_path,
        output_dir=out_dir,
        pipeline=cfg.get("pipeline", "auto"),
        segment_blocks=cfg.get("segment_blocks", 40),
        segment_overlap=cfg.get("segment_overlap", 5),
        workers=cfg.get("workers", 1),
        pdf_split_workers=cfg.get("pdf_split_workers", 1),
        epub_split_workers=cfg.get("epub_split_workers", 1),
        pdf_pages_per_job=cfg.get("pdf_pages_per_job", 50),
        epub_spine_items_per_job=cfg.get("epub_spine_items_per_job", 10),
        epub_extractor=cfg.get("epub_extractor", "unstructured"),
        instruction_step_segmentation_policy=cfg.get(
            "instruction_step_segmentation_policy",
            "auto",
        ),
        instruction_step_segmenter=cfg.get(
            "instruction_step_segmenter",
            "heuristic_v1",
        ),
        p6_time_backend=cfg.get("p6_time_backend", "regex_v1"),
        p6_time_total_strategy=cfg.get("p6_time_total_strategy", "sum_all_v1"),
        p6_temperature_backend=cfg.get("p6_temperature_backend", "regex_v1"),
        p6_temperature_unit_backend=cfg.get(
            "p6_temperature_unit_backend",
            "builtin_v1",
        ),
        p6_ovenlike_mode=cfg.get("p6_ovenlike_mode", "keywords_v1"),
        p6_yield_mode=cfg.get("p6_yield_mode", "legacy_v1"),
        p6_emit_metadata_debug=cfg.get("p6_emit_metadata_debug", False),
        llm_recipe_pipeline=cfg.get("llm_recipe_pipeline", "off"),
        codex_farm_cmd=cfg.get("codex_farm_cmd", "codex-farm"),
        codex_farm_root=cfg.get("codex_farm_root"),
        codex_farm_context_blocks=cfg.get("codex_farm_context_blocks", 30),
        codex_farm_failure_mode=cfg.get("codex_farm_failure_mode", "fail"),
        processed_output_root=cfg.get("processed_output_root", out_dir),
        write_markdown=cfg.get("write_markdown", True),
        write_label_studio_tasks=cfg.get("write_label_studio_tasks", True),
        run_manifest_kind="bench_pred_run",
        progress_callback=progress_callback,
        **kwargs,
    )
    return Path(result["run_root"])
