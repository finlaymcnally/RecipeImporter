...
- Recipe-stage topology now flows through that same model seam: `analysis.recipe_pipeline_context` and `analysis.stage_separated_comparison` are both rendered from `UploadBundleSourceModel.topology` plus compatibility aliases instead of script-local topology inference.
