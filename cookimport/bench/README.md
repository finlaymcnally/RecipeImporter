...
- Oracle upload is a post-bundle wrapper flow in `oracle_upload.py`: interactive single-offline runs auto-upload their session `upload_bundle_v1`, multi-book single-profile runs auto-upload only the top-level group bundle, and `cookimport bench oracle-upload <existing root>` is the no-rerun/manual retry path. `--mode dry-run` stays zero-cost: it uses Oracle's real dry-run for small bundles and falls back to a local preview when the payload file exceeds Oracle's inline size cap. Browser-mode uploads now auto-shard oversized bundle files into Oracle-safe temporary attachments before launching Oracle, and they invoke the real Oracle browser engine with the Linux off-screen Chromium launcher (`chromium-nosandbox-xvfb`) plus a local debug-host override instead of relying on the drifted visible-browser wrapper.
