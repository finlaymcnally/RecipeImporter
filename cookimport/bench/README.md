...
- Canonical-text eval can choose SequenceMatcher implementation via `COOKIMPORT_BENCHMARK_SEQUENCE_MATCHER` (`auto`, `stdlib`, `cydifflib`, `cdifflib`, `dmp`, `multilayer`); evaluator telemetry records the selected implementation. `multilayer` is an opt-in experimental mode for parity/speed spikes.
