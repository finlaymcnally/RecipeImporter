...
- `last_run_store.py` persists per-operation last settings under canonical `history_root_for_output(<output_dir>)/last_run_settings_{import|benchmark}.json` (repo-local default: `.history/...`), with legacy read fallback for `<output_dir>/.history/...` and prior `<output_dir parent>/.history/...` locations.
