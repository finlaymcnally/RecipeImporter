...
- active codex-farm knobs (`llm_recipe_pipeline`, `llm_knowledge_pipeline`, `llm_tags_pipeline`, `codex_farm_cmd`, `codex_farm_model`, `codex_farm_reasoning_effort`, `codex_farm_root`, `codex_farm_workspace_root`, `codex_farm_context_blocks`, `codex_farm_knowledge_context_blocks`, `tag_catalog_json`, `codex_farm_failure_mode`) must stay wired through stage and benchmark prediction-generation paths.
- Retired Bucket 1 keys (section detector, fallback segmentation, matcher, pass ids, pass-policy toggles, debug trace toggles) must load for compatibility only. New runs should resolve them from `cookimport/config/codex_decision.py` fixed-behavior helpers and record `bucket1_fixed_behavior_version` instead of re-exposing them as live settings.
...
- `table_extraction` is now a retired legacy key accepted only for compatibility loading. New runs always perform deterministic table detection/export (`tables/<workbook>/tables.jsonl` + `tables.md`), table-aware chunking, and optional pass4 `chunk.blocks[*].table_hint` enrichment.
