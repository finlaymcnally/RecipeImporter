...
- Last-run snapshots are stored in `history_root_for_output(output_dir)/last_run_settings_{import|benchmark}.json` via `cookimport/config/last_run_store.py` (repo-local default `.history/...`; older `<output_dir>/.history/...` and prior `<output_dir parent>/.history/...` paths remain read-only fallback for migration).
