...
- Stored run settings are current-contract only. Removed extractor aliases such as `epub_extractor=auto` and the older binary-doc extractor alias should fail instead of being migrated forward.
...
- `stage(...)` should pass per-file effective extractor choices explicitly to `execute_source_job(...)` instead of depending on persistent process-wide `C3IMP_EPUB_EXTRACTOR`.
- `cookimport/cli_ui/run_settings_flow.py` is the only interactive run-profile chooser; keep it limited to the two automatic top-tier profile families (`codex-exec`, `vanilla`) so import and benchmark stay in lock-step.
...
- `llm_recipe_pipeline` accepts `off|codex-recipe-shard-v1` without environment gating; validation should reject removed recipe pipeline ids instead of normalizing them.
- Direct run surfaces (`stage`, `labelstudio-import`, and `labelstudio-benchmark`) must fail closed unless Codex-backed settings are accompanied by explicit command approval at the caller boundary.
  - `stage` and `labelstudio-import` use `--allow-codex`.
