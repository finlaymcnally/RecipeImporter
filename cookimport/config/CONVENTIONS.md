...
- `llm_recipe_pipeline` is env-gated: `llm_recipe_pipeline=codex-farm-3pass-v1` is only allowed when `COOKIMPORT_ALLOW_CODEX_FARM=1` is set; CLI/pred-run normalization must enforce this with an explicit message.
- Run-settings migration contract: if stored/global settings contain non-`off` `llm_recipe_pipeline` values but `COOKIMPORT_ALLOW_CODEX_FARM` is not enabled, coerce them back to `off` with a warning so old configs cannot silently re-enable recipe codex-farm correction.
