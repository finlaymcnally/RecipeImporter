...
def _run_direct_recipe_workers_v1(*, phase_key: str, pipeline_id: str, run_root: Path, shards: Sequence[ShardManifestEntryV1], runner: CodexExecRunner, worker_count: int, env: Mapping[str, str], model: str | None, reasoning_effort: str | None, output_schema_path: Path | None, settings: Mapping[str, Any], runtime_metadata: Mapping[str, Any], pipeline_assets: Mapping[str, Any], progress_callback: Callable[[str], None] | None=None, survivability_report: Mapping[str, Any] | None=None) -> tuple[PhaseManifestV1, list[WorkerExecutionReportV1]]:
    ...
    worker_id_by_shard_id = {shard_id: assignment.worker_id for assignment in assignments for shard_id in assignment.shard_ids}
    phase_plan = build_phase_plan(stage_key=phase_key, stage_label=stage_label(phase_key), stage_order=1, surface_pipeline=str(settings.get('llm_recipe_pipeline') or ''), runtime_pipeline_id=pipeline_id, worker_count=worker_count, requested_shard_count=int((runtime_metadata or {}).get('requested_shard_count') or len(shards) or 1), budget_native_shard_count=int((runtime_metadata or {}).get('budget_native_shard_count') or len(shards) or 1), launch_shard_count=len(shards), planning_warnings=(runtime_metadata or {}).get('planning_warnings') or [], shard_specs=[{'shard_id': shard.shard_id, 'worker_id': worker_id_by_shard_id.get(shard.shard_id), 'owned_ids': list(shard.owned_ids), 'call_ids': [shard.shard_id], 'prompt_chars': len(render_recipe_direct_prompt(pipeline_assets=pipeline_assets, input_text=_serialize_compact_prompt_json(_coerce_dict(shard.input_payload)), input_path=run_root / 'inputs' / f'{shard.shard_id}.json')), 'task_prompt_chars': len(_serialize_compact_prompt_json(_coerce_dict(shard.input_payload))), 'work_unit_count': int((_coerce_dict(shard.metadata).get('recipe_count') or len(shard.owned_ids) or 0)), 'work_unit_label': 'recipes'} for shard in shards])
    phase_plan = attach_survivability_to_phase_plan(phase_plan=phase_plan, survivability_report=survivability_report)
    phase_plan_path, phase_plan_summary_path = write_phase_plan_artifacts(stage_dir=run_root, phase_plan=phase_plan)
...
def _run_single_correction_recipe_pipeline(*, conversion_result: ConversionResult, run_settings: RunSettings, run_root: Path, workbook_slug: str, runner: CodexExecRunner | None=None, full_blocks: list[dict[str, Any]] | None=None, progress_callback: Callable[[str], None] | None=None) -> CodexFarmApplyResult:
    ...
    if recipe_shards:
        ...
        phase_runtime_summary = {'worker_count': phase_manifest.worker_count, 'shard_count': phase_manifest.shard_count, 'phase_manifest': phase_manifest_payload, 'promotion_report': promotion_report, 'telemetry': telemetry, 'worker_reports': worker_reports_payload, 'phase_plan_path': str(phase_runtime_dir / 'phase_plan.json'), 'phase_plan_summary_path': str(phase_runtime_dir / 'phase_plan_summary.json')}
