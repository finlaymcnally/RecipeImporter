...
def _recipe_recommended_command() -> str:
    return "python3 -m cookimport.llm.recipe_same_session_handoff"


def _default_recipe_same_session_state_path(*, workspace_root: Path) -> Path:
    return workspace_root / "_repo_control" / _RECIPE_SAME_SESSION_STATE_FILE_NAME


def _resolve_recipe_same_session_state_path(
    *,
    workspace_root: Path,
    candidate: str | None,
) -> Path:
    ...
    return _default_recipe_same_session_state_path(workspace_root=workspace_root)


def describe_recipe_same_session_status(
    *,
    workspace_root: Path,
    state_path: Path,
) -> dict[str, Any]:
    ...
    expected_output_paths = [
        workspace_root / str(record.get("result_path") or "").strip()
        for record in _current_task_records(state)
        if str(record.get("result_path") or "").strip()
    ]
    expected_outputs_present = sum(1 for path in expected_output_paths if path.exists())
    mode = str(task_file.get("mode") or current_original_task_file.get("mode") or "initial").strip()
    recommended_command = _recipe_recommended_command()
    if bool(state.get("completed")):
        ...
        next_action = "run same_session_handoff to validate and expand outputs"
    return {
        "stage_key": str(task_file.get("stage_key") or current_original_task_file.get("stage_key") or "recipe_refine"),
        ...
        "expected_outputs_total": len(expected_output_paths),
...
def describe_recipe_same_session_doctor(
    *,
    workspace_root: Path,
    state_path: Path,
) -> dict[str, Any]:
    ...
    return {
        "stage_key": str(status.get("stage_key") or "recipe_refine"),
...
def main() -> int:
    ...
    state_path = _resolve_recipe_same_session_state_path(
        workspace_root=workspace_root,
    ...
    if not state_path.exists():
        raise SystemExit(
            ...
            f"{state_path} or set --state-path / ${RECIPE_SAME_SESSION_STATE_ENV}"
        )
    if args.status and args.doctor:
        raise SystemExit("use either --status or --doctor, not both")
    if args.status:
        result = describe_recipe_same_session_status(
            workspace_root=workspace_root,
            state_path=state_path,
        )
    elif args.doctor:
        result = describe_recipe_same_session_doctor(
            workspace_root=workspace_root,
            state_path=state_path,
        )
    else:
