...
def _run_direct_knowledge_workers_v1(
    *,
    ...
    task_plans_by_shard_id: dict[str, tuple[_KnowledgeTaskPlan, ...]] = {}
    task_entries = [
...
def _run_phase_knowledge_worker_assignment_v1(
    *,
    ...
    del output_schema_path, cohort_watchdog_state, recovery_governor, interruption_requested
    ...
    for shard in assigned_shards:
        ...
        _write_json(
            build_knowledge_seed_output(dict(shard.input_payload or {})),
...
def _run_knowledge_workspace_worker_assignment_taskized_v1(
    *,
    ...
    out_dir = worker_root / "out"
    out_dir.mkdir(parents=True, exist_ok=True)
    prep = _prepare_taskized_workspace_assignment(
        run_root=run_root,
        assignment=assignment,
        artifacts=artifacts,
        assigned_shards=assigned_shards,
    ...
    worker_failure_count = prep.worker_failure_count
    worker_proposal_count = prep.worker_proposal_count
    worker_failures: list[dict[str, Any]] = list(prep.worker_failures)
    worker_proposals: list[ShardProposalV1] = list(prep.worker_proposals)
    worker_runner_results: list[dict[str, Any]] = []
    stage_rows: list[dict[str, Any]] = []
    runnable_shards = list(prep.runnable_shards)
    runnable_tasks = list(prep.runnable_tasks)
    ...
    relaunch_status_metadata: dict[str, Any] = {
        ...
        "workspace_relaunch_cap_reached": False,
    }
    if runnable_shards and runnable_tasks:
        session = _run_taskized_workspace_session(
            assignment=assignment,
        ...
        if session is not None:
            ...
            worker_runner_results.extend(session.worker_runner_results)
            stage_rows.extend(session.stage_rows)
            relaunch_status_metadata = dict(session.relaunch_status_metadata)
            workspace_task_entries = tuple(session.workspace_task_entries)
        else:
            run_result = _build_preflight_rejected_run_result(
                prompt_text="knowledge worker session not started",
                output_schema_path=output_schema_path,
                working_dir=worker_root,
                reason_code="no_runnable_tasks",
                reason_detail="knowledge worker assignment had no runnable tasks after preflight",
            )
    else:
        run_result = _build_preflight_rejected_run_result(
            prompt_text="knowledge worker session not started",
    ...
    worker_failure_count, worker_proposal_count = _finalize_taskized_workspace_assignment(
        run_root=run_root,
        assignment=assignment,
        artifacts=artifacts,
        runnable_shards=runnable_shards,
        ...
        worker_runner_results=worker_runner_results,
        stage_rows=stage_rows,
    )
