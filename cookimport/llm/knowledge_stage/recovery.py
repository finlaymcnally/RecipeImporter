    ...
    *,
    ...
    task_specific_semantics = (
        ...
        if stage_key == "nonrecipe_classify"
            ...
            "- `groupable_row_ids` names the only authoritative kept `row_id`s you may group.",
            "- Every kept `row_id` in `groupable_row_ids` must appear in exactly one group.",
            "- Any `ctxXX` rows in `ordered_rows` are context only and must not appear in returned spans.",
