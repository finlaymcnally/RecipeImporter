...
from ._shared import (
    ...
    _KNOWLEDGE_COHORT_WATCHDOG_MIN_COMPLETED_SHARDS,
    _KNOWLEDGE_COHORT_WATCHDOG_MIN_ELAPSED_MS,
    _KNOWLEDGE_PATHOLOGICAL_CHARS_PER_RETURNED_ROW,
    _KNOWLEDGE_PATHOLOGICAL_WHITESPACE_RUN,
    _KNOWLEDGE_RETRY_MAX_CHARS_PER_SHARD,
    _KNOWLEDGE_RETRY_MAX_CHUNKS_PER_SHARD,
    _KNOWLEDGE_WATCHDOG_RETRY_SILENCE_TIMEOUT_SECONDS,
    _KNOWLEDGE_WATCHDOG_RETRY_TIMEOUT_SECONDS,
...
from .recovery_status import (
    ...
    _combine_taskfile_worker_run_results,
    ...
    _terminal_knowledge_attempt_type,
    _terminal_knowledge_task_state,
    ...
    *,
    ...
    task_specific_semantics = (
        (
            ...
            "- Answer each unit with `category` only.",
            "- Final categories must be exactly one of `keep_for_review` or `other`.",
            "- Do not think about tags in this step. Tagging belongs entirely to the grouping step.",
            "- If the block looks like reusable cooking knowledge worth carrying into group review, answer `keep_for_review`.",
            "- If the block is not a strong standalone retrieval target, answer `other`.",
            "- Teacherly, explanatory, or motivational tone alone is not enough for `keep_for_review`.",
            "- If a split row is mainly connective coaching, setup, author voice, or rhetoric, answer `other` even when the surrounding paragraph is educational.",
            ...
            "- If a row is functioning as a heading, answer `other` even when it names the nearby concept clearly.",
            ...
            "- Contrast examples: `Salt, Fat, Acid, and Heat were the four elements that guided basic decision making in every single dish, no matter what.` => `other`; `It was a revelation.` => `other`; `My pursuit of flavor has continued to lead me around the world.` => `other`; `Taste. It will need salt.` => `other`; `You'll be better equipped to trust your own palate, to make substitutions in recipes, and cook with what's on hand.` => `other`; `Taste constantly as you cook, and adjust seasoning before serving.` => `keep_for_review`; `Cooking fats can be heated to extreme temperatures, allowing the surface temperature of foods cooked in them to climb high enough to brown.` => `keep_for_review`.",
        ...
        if stage_key == "nonrecipe_classify"
            ...
            "- Use `ordered_rows` to read the local run in source order when it is present.",
            "- Plain `rXX | ...` rows in `ordered_rows` are groupable by default.",
            "- `ctxXX | ng | ...` rows in `ordered_rows` are context only; `ng` means do not group.",
            "- Every groupable `rXX` row must appear in exactly one group.",
            ...
            "- Use concise group labels and stable short group ids such as `g01`, `g02`, and so on.",
        )
    )
...
_KNOWLEDGE_HINT_EXAMPLE_FILES = (
    ...
    "valid_heading_stays_other_with_useful_body_packet.json",
