...
def _build_recipe_workspace_worker_prompt(
    *,
    ...
    lines = [
        ...
        "2. Treat `SHARD_PACKET.md` as the authoritative packed shard summary and treat `scratch/_prepared_drafts.json` as the authoritative draft inventory. The current-task sidecars are contextual aids for the active draft, not the core loop. Use `assigned_tasks.json` only as lightweight queue inventory after that.",
        ...
        "8. `python3 tools/recipe_worker.py current`, `next`, `overview`, `show <task_id>`, `prepare-all --dest-dir scratch/`, and `finalize scratch/<task_id>.json` are fallback/debug tools, not the default queue loop.",
