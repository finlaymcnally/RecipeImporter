...
def _write_recipe_worker_hint(*, path: Path, shard: ShardManifestEntryV1) -> None:
    ...
    write_worker_hint_markdown(path, title=f'Recipe correction hints for {shard.shard_id}', summary_lines=['This sidecar is worker guidance only.', 'Open the authoritative `in/<shard_id>.json` file and work from the owned source rows.', 'Choose `st=repaired` only when you can restate a real recipe. Choose `st=fragmentary` when recipe evidence exists but is too incomplete to normalize safely. Choose `st=not_a_recipe` when the owned text is not a recipe at all.', 'When `st=repaired`, `cr` must be a complete canonical recipe object. When `st` is `fragmentary` or `not_a_recipe`, set `cr` to null and explain the judgment briefly in `sr`.', f'Owned recipe candidates in this shard: {len(recipes)}.'], sections=[('How to use this packet', ['Treat source rows in `in/<shard_id>.json` as authoritative.', 'Do not force a repaired recipe when the source is better described as fragmentary or not_a_recipe.', 'Use `db` only for blocks you are explicitly handing back to outside-recipe review.', 'A fragmentary recipe may keep all of its owned blocks when the source is still a real but incomplete recipe.', 'Keep `m` and `mr` honest. If there is no meaningful ingredient-step mapping, leave `m` empty and say why in `mr`.']), ('Worker flow', ['Read the owned rows in order.', 'Edit only the required answer surface for this shard or task file.', 'Finish with the normal handoff command for this workspace.'])])
