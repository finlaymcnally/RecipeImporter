...
- Knowledge packet planning still works in chars, but its default packet budgets are now auto-derived from the `nonrecipe_finalize` survivability ceilings with explicit headroom. Explicit packet-budget overrides still win when you want a tighter or looser native shard target.
