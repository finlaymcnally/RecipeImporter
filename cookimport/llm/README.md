...
Shard-v1 recipe and knowledge still run one direct `codex exec --json --output-schema ... -` call per shard with the full shard prompt sent once on stdin. Canonical line-role now uses a file-backed direct-exec contract: each phase writes `workers/*/in/*.json`, then sends Codex a short wrapper prompt that points at that worker-local task file.
...
Recipe no longer uses path-mode handoff under CodexFarm process. `codex_farm_orchestrator.py` now renders the compact inline shard JSON directly into the checked-in recipe prompt template, sends that prompt through `codex_exec_runner.py`, and writes per-shard `prompt.txt` / `events.jsonl` / `usage.json` / `cost_breakdown.json` / `proposal.json` artifacts under `recipe_phase_runtime/workers/<worker>/shards/<shard>/`. Knowledge also uses inline compact shard JSON. Line-role is now different: its live model input is the per-phase worker-local JSON file under `line-role-pipeline/runtime/<phase>/workers/<worker>/in/*.json`, while `prompts/<phase>/*_prompt_*.txt` and shard-local `prompt.txt` hold the short file-wrapper prompt.
