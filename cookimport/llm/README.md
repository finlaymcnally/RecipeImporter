...
- Recipe, canonical line-role, and knowledge all use assignment-first workspace workers, but the happy path now differs by stage. Recipe refine still uses the richer helper-driven surface inside `task.json`. Canonical line-role plus knowledge classification/grouping are direct-batch: open `task.json` directly, read the full assignment, edit only `/units/*/answer`, run `task-handoff`, and keep any repair/grouping follow-up in the same workspace session.
...
- Knowledge grounding now rides through the normal runtime artifacts: kept blocks must still emit `retrieval_concept` plus `grounding`, under-grounded `knowledge` rows are demoted to `other` instead of forcing tag-invention repair, `knowledge_manifest.json` plus `knowledge_stage_summary.json` expose existing-tag versus proposed-tag counts and grounding-gate demotions, and the stage writes `knowledge_tag_proposals.jsonl` when the model needs a new tag under an existing category.
