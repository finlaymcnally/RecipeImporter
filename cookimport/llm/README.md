...
- For knowledge classification specifically, repo code owns the checked-in tag catalog, exposes the real existing tag list to workers, and validates catalog membership; the model owns the retrieval-grade keep/drop judgment, and ingest no longer rewrites accepted `knowledge` rows to `other` based on repo utility heuristics.
...
- Knowledge packet planning still works in chars, and those budget-native packets are the locality-preserving unit for nonrecipe finalize. `knowledge_prompt_target_count` caps the final shard count, but the planner no longer fragments an already-budget-safe packet just to hit the cap exactly. Explicit packet-budget overrides still win when you want a tighter or looser native shard target.
...
- Knowledge grounding now rides through the normal runtime artifacts: kept blocks emit only `category` plus `grounding`, workers see the real existing tag catalog instead of repo-generated candidate-tag hints, `knowledge_manifest.json` plus `knowledge_stage_summary.json` expose existing-tag versus proposed-tag counts, and `knowledge_tag_proposals.jsonl` records when the model needs a new tag under an existing category.
