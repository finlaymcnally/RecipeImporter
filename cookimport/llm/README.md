...
The live knowledge phase still reuses the compact pack id `recipe.knowledge.compact.v1` underneath the shard runtime. The important cutover is that shard ownership, worker reuse, proposal validation, and fallback behavior are now repo-owned instead of being implicit in one direct bundle pass. The compact knowledge payload is still bundle-shaped because prompt-preview and older prompt/debug readers still reuse `raw/llm/<workbook_slug>/knowledge/{in,out}` until the later observability pass rewires them.
