...
- Actual freeform (`span`) prompts should provide row text once via `{{BLOCKS_WITH_FOCUS_MARKERS_COMPACT_LINES}}` with explicit context-before/context-after markers plus `<<<START_LABELING_ROWS_HERE>>>` / `<<<STOP_LABELING_ROWS_HERE_CONTEXT_ONLY>>>` focus boundaries; avoid duplicating focus rows as a second full text payload list.
