...
- `source_model` is the source-normalizer bridge: it normalizes canonical `SourceBlock` / `SourceSupport`, handles merge-safe block/support rebasing, rewrites referenced support ids in `bN` and legacy `block:N` forms, writes `raw/source/<workbook>/source_blocks.jsonl` plus `source_support.json`, and now fails closed when stage input omits canonical `source_blocks`.
