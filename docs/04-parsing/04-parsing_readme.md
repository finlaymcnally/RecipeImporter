...
- outside-recipe line-role is routing-only for semantic prose: explanatory prose and lesson headings should land as `NONRECIPE_CANDIDATE`, while obvious junk can land as `NONRECIPE_EXCLUDE` with an `exclusion_reason`. Knowledge remains the later stage that decides final `KNOWLEDGE` versus `OTHER`.
