...
- `RECIPE_VARIANT` can now cover short local alternate-version runs: a `Variations` heading or a long named `To make ...` lead can keep the immediately following ingredient rows and explicit-cue instruction rows inside the same variant subsection, but generic `To make the ...` method steps and following note-only rows stay under their ordinary structural labels.
...
- outside-recipe `KNOWLEDGE` labeling is deterministic-first and now high-evidence/low-default in `canonical_line_roles.py`; reusable explanatory/reference prose and explicit lesson headings still promote, but memoir, blurbs, endorsements, book-framing encouragement, and broad action-verb advice now stay `OTHER` unless stronger evidence is present.
- parser-owned outside-recipe `KNOWLEDGE` authority now goes through one explicit seam: `_outside_recipe_knowledge_label_allowed(...)` gates both deterministic promotion and `_sanitize_prediction(...)`, so unsupported Codex `KNOWLEDGE` upgrades fall back to `OTHER` deterministically.
