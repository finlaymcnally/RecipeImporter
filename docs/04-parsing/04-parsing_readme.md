...
- line-role workers now also get `line-role-pipeline/runtime/line_role/workers/*/hints/*.md` plus worker-local `examples/*.md` contrast files. Those hint sidecars are the worker-facing explanation layer: label-code legend, packet profile, packet interpretation, conservative flip policy, and short attention-row summaries. The validator still treats `in/*.json` as authoritative.
