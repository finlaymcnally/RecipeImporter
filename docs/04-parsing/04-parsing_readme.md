...
- line-role inline validation now accepts one ordered `labels` array as the primary happy-path contract. It also keeps a narrow compatibility seam before repair: if a worker still answers with packet-local `row_id` rows or with `atomic_index` fields that are obviously just packet-local ordinals (`0..n-1` or `1..n`), deterministic code remaps that output onto the shard's true ordered `atomic_index` values and keeps the answer on the happy path. Ambiguous or partial legacy outputs still fail closed.
...
- line-role validation now includes a narrow semantic-pathology guard for structurally valid but obviously absurd outside-recipe outputs. When a front-matter or contents shard is hallucinated into recipe-local structure, runtime records explicit `semantic_pathology_rejected` diagnostics, allows one bounded semantic repair follow-up, and then contains unresolved rows conservatively instead of promoting the bad labels.
- semantic containment is route-safety only: accepted Codex outside-recipe exclusions stay authoritative on the normal happy path, and only semantically rejected pathology shards may preserve deterministic obvious-junk exclusions while the rest falls back to `NONRECIPE_CANDIDATE`. Deterministic code is not trying to relabel ordinary ambiguous prose.
