...
- deterministic validation now also reasserts two narrow recipe-start invariants on the live line-role path: if the first line of a recipe span is a credible title, it can override a mistaken `RECIPE_VARIANT`, and if an in-recipe line is a strict yield header, it can override a mistaken `RECIPE_NOTES`
...
- lesson-prose prompt posture now explicitly keeps short concept headings such as `Balancing Fat` and short declarative teaching lines as candidate `OTHER`, while lone unsupported question headings such as `What is Heat?` also stay `OTHER` unless they are obvious junk.
...
- The emitted workspace helper is part of that same contract: `check-phase` / `install-phase` must reject row-level ledger errors such as invalid `exclusion_reason` values, not just malformed top-level JSON.
