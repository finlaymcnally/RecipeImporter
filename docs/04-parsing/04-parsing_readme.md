...
- line-role workers now also get `line-role-pipeline/runtime/line_role/workers/*/hints/*.md`. Those hint sidecars are the worker-facing explanation layer: label-code legend, packet profile, and short attention-row summaries. The validator still treats `in/*.json` as authoritative.
...
- line-role resume now reuses already-validated `workers/*/out/*.json` packet files and only re-asks missing or invalid packets. The worker prompt/watchdog for this stage now forbids exploratory `pwd` / `ls` / `find` / `tree` orientation commands and expects direct reads from the named files.
