...
- line-role inline-JSON prompts now derive from the same shared contract file as the other prompt surfaces, but worker-visible packets stay raw-text-first: owned rows plus optional nearby context rows only, with no repo-authored shard-profile or semantic sidecar evidence injected into the prompt.
