...
- that live `line_role` runtime root now also writes `phase_plan.json` plus `phase_plan_summary.json`, so line-role planning and prompt-budget reporting use the same shared phase-plan artifact family as recipe and knowledge.
...
- `cookimport/parsing/canonical_line_roles/contracts.py` owns the prediction model plus normalization helpers, `prompt_inputs.py` owns reusable row serialization, `artifacts.py` owns runtime/prompt artifact helpers, `planning.py` owns contiguous shard planning, `policy.py` owns local heuristics, `validation.py` owns shard-ledger checks/promotion gates, `runtime_taskfile.py` owns task-file payload/output-expansion helpers, `runtime.py` owns live worker orchestration, and `__init__.py` is the package seam.
- `cookimport/parsing/canonical_line_roles/runtime_recovery.py` now owns same-session recovery plus workspace-status assessment, while `runtime.py` resolves the small package-level override hooks explicitly so monkeypatch-heavy tests still exercise the real coordinator surface without copied package globals.
- `planning.py`, `policy.py`, and `validation.py` no longer clone `cookimport.parsing.canonical_line_roles` into module globals; they now use explicit named imports while preserving the narrow runtime override hooks that tests patch through the package/root surface.
...
- line-role workers now also get `line-role-pipeline/runtime/line_role/workers/*/hints/*.md`. Those hint sidecars are worker-facing orientation only: open the authoritative shard file, label from raw text, and use nearby rows only as reference context. The validator still treats `in/*.json` as authoritative.
- line-role worker/status telemetry and `line_role_stage_summary.json` now expose shared `repair_recovery_policy` budget rows as well. The inline contract remains one structured repair follow-up plus one watchdog retry budget, while taskfile workers keep their bounded same-session repair and fresh-session/fresh-worker recovery rows.
- line-role `telemetry_summary.json` and `line_role_stage_summary.json` now also carry Codex transport/policy metadata (`codex_transport`, `codex_policy_mode`, `codex_shell_tool_enabled`) so benchmark prompt-budget rollups can attribute line-role spend to the actual execution mode instead of an unlabeled aggregate.
- line-role stage summaries now derive high-level `stage_state` from the shard ledger when inline runs do not emit per-worker shard `status.json` files, so completed inline runs do not regress to `not_started`.
- line-role inline-JSON packets now hide sparse `atomic_index` values from the visible prompt and group raw evidence by source block. The visible packet is a compact top-level `blocks` array where each block carries `block_index` plus ordered row tuples such as `["r01", "Bright Cabbage Slaw"]` for owned rows and `["ctx", "Variations"]` for reference-only rows. Repo-owned runtime still keeps shard order authoritative and maps accepted packet-local `row_id` answers back onto real `atomic_index` values.
- Parsing keep/delete rule for block-era names: keep `block_index` and related fields only when they are the real source-layout grouping or provenance for a row-native payload. Do not reintroduce semantic block aliases on row-owned parsing contracts.
- line-role inline validation is now strict row-grounded JSON. The live worker must return one ordered top-level `rows` array with no extra keys, and each answer row must carry a packet-local `row_id` plus `label`. The initial inline turn gets a packet-local exact-length output schema so missing or extra answer rows can fail at the transport boundary instead of only in later repo validation. Row-shaped replies without usable `row_id` grounding fail closed into repair or invalid output.
- line-role inline repair reuses the same prepared execution workspace and resumes the prior transcript history. That keeps stable local paths for prompt artifacts and lineage while preserving the broader shard conversation for narrowed unresolved-row followups; repair packets now keep whole source blocks that contain unresolved owned rows, rendering accepted sibling rows as `ctx` evidence instead of dropping them. Exact-count enforcement on repair remains repo-owned because `codex exec resume` does not expose `--output-schema`.
- line-role inline-JSON prompts now derive from the same shared contract file as the other prompt surfaces, but worker-visible packets stay raw-text-first: compact block-grouped source text only, with no repo-authored shard-profile or semantic sidecar evidence injected into the prompt. The inline prompt now explicitly tells the model to read `blocks` block-by-block, use `ctx` rows as reference-only context, and avoid flattening the shard into isolated sentence judgments.
- the shared line-role contract now states the intended memoir/front-matter boundary more explicitly: dedications, acknowledgments, intro/setup framing, broad encouragement, and anecdote-plus-moral prose should usually stay `NONRECIPE_EXCLUDE`, while a later standalone lesson sentence may still be `NONRECIPE_CANDIDATE`.
- line-role validation still avoids a broad semantic veto pass, but `NONRECIPE_EXCLUDE` is now a narrow repo-owned boundary. Outside recipe, accepted Codex hard-excludes are only preserved when the same obvious-junk exclusion policy would allow them; unsupported hard-excludes fall back to the deterministic baseline so factual/process lesson rows stay reviewable as `NONRECIPE_CANDIDATE`.
- partial line-role authority is now honest: accepted rows stay accepted, repair packets preserve full relevant source blocks while still validating only unresolved owned rows, and unresolved rows fail closed if repair/retry still does not finish them. The runtime no longer pads those holes out with deterministic baseline labels.
- line-role direct-exec workers are now `task.json`-first on the happy path. The visible worker session edits only `/units/*/answer` in `task.json`; the worker-visible evidence is just shard-local `row_id` plus raw row text, not repo-authored deterministic label hints, deterministic reason hints, block provenance, pre-grouping `within_recipe_span` fields, or global `atomic_index` values. Repo-owned `assigned_shards.json`, `in/<shard_id>.json`, `debug/<shard_id>.json`, `hints/<shard_id>.md`, and `out/<shard_id>.json` remain supporting context and artifacts outside that contract.
...
- accepted Codex shard rows are now first-authority on the live route path. Repo code still validates ownership, completeness, allowed labels, and row coverage shape, but it does not compare accepted worker labels to deterministic baseline labels during runtime acceptance.
- accepted Codex shard rows are now normalized only for ownership or public-contract shape. For example, recipe-local rows cannot stay outside-recipe labels, but repo heuristics no longer demote surprising `RECIPE_TITLE`, `RECIPE_VARIANT`, `HOWTO_SECTION`, or `INSTRUCTION_LINE` calls just because local support looks weak.
- the worker `in/*.json` transport is now `{"v":2,"shard_id":...,"context_before_rows":[[atomic_index,block_index,current_line], ...],"rows":[[atomic_index,block_index,current_line], ...],"context_after_rows":[[atomic_index,block_index,current_line], ...]}`. The boundary-context keys are optional and appear only when immediate neighbors exist. It is one ordered contiguous slice of the book, and the live worker-facing payload is intentionally provenance-light: only atomic index, block index, and raw line text.
...
- `RECIPE_VARIANT` can now cover short local alternate-version runs, but the prompt now treats a bare `Variation` / `Variations` cue line as a cue rather than the label itself. The actual alternate-version title, attached intro prose, ingredient rows, explicit-cue instruction rows, and short adjustment lines such as `To add a little heat ...` or `To evoke the flavors ...` may stay `RECIPE_VARIANT` when the local run is clear, while generic `To make the ...` method steps and following note-only rows stay under their ordinary structural labels.
- The active canonical line-role prompt now also includes an explicit reset example for that seam: `Bright Cabbage Slaw` followed by `Serves 4 generously` and ingredient rows must start a new recipe, not continue the earlier `Variations` run.
...
- parent shard proposals are now shard-ledger summaries, not a second hidden task layer. Accepted rows stay authoritative, unresolved rows become repair requests, and the live worker path still prefers a clean installed shard ledger or one bounded watchdog retry. Inline JSON canonical line-role now gets up to three shard-local repair followups, and each followup must target only the currently unresolved rows instead of replaying the earlier repair context. When a shard keeps some accepted rows but still leaves holes, runtime preserves those accepted Codex rows but does not backfill the unresolved atomic indices from the deterministic baseline; the shard stays incomplete/invalid unless repair or retry finishes it. Full-zero-authority shards still fail closed.
- inline semantic repair packets now show the model's own last failed labels for the targeted rows via `previous_failed_rows`, alongside the repo validation reasons that rejected them. The repair turn sees failed-output evidence, not repo-authored semantic coaching.
- the inline compact prompt seam is still separate from the file-backed transport: prompt text now uses compact provenance strings for owned rows (`r01 | 210 | Bright Cabbage Slaw`) plus neighboring reference strings (`209 | Variations`) instead of semantic hint codes. That keeps row identity and block provenance visible without deterministic semantic steering.
...
- outside-recipe line-role is routing-only for semantic prose and now uses discard-first wording: obvious fluff should fall to `NONRECIPE_EXCLUDE` before any candidate call is considered, and `NONRECIPE_CANDIDATE` is reserved for rows that themselves stand up as portable cooking knowledge. Nearby lesson rows can clarify context, but they should not rescue short coaching/exhortation fragments such as `Taste. It will need salt.` into candidate on context alone. Knowledge remains the later stage that decides final `KNOWLEDGE` versus `OTHER`.
- outside-recipe `title_like` and `ingredient_like` rows still need nearby recipe scaffold before they should be treated as recipe structure. Accepted Codex labels remain first semantic authority on the happy path, but runtime now rejects narrow semantic-pathology patterns on outside-recipe front-matter/contents shards before promotion.
- `CanonicalLineRolePrediction` now carries only the coarse live label contract. For outside-recipe rows that means the active authority boundary is just `NONRECIPE_CANDIDATE` versus `NONRECIPE_EXCLUDE`.
- the obvious-junk heuristics remain a repo-owned deterministic seam for the rule/baseline path, and the live Codex route path still does not do broad deterministic semantic rewrites. The new runtime guard is limited to explainable semantic-pathology rejection plus conservative containment on unresolved outside-recipe shards, plus narrow recipe-local contradiction rejection on the inline line-role path.
...
- outside-recipe instruction support now also recognizes two narrow recipe-adjacent shapes that showed up in the Salt Fat benchmark: title-led mini-clusters (`Roasted Beets` followed by short local procedure/note rows) and prepositional-start procedure lines (`In a small bowl ... let the shallot sit ...`). The runtime still rejects memoir/advice prose and storage-note lines as recipe method when those rows lack real local recipe structure.
- generic `Variation` / `Variations` headings are now allowed when the immediate continuation is already recognizable as variant body prose, including named `To make ...` alternate versions. The rule is still intentionally narrow; a bare variation heading with memoir, chapter taxonomy, or generic knowledge after it should not promote to `RECIPE_VARIANT`.
- the shared prompt contract now also states three Salt Fat benchmark policies explicitly: `HOWTO_SECTION` must own a real mini-preparation with nearby ingredients and/or steps instead of a mere named component, storage/leftovers/refrigerate/freeze guidance should be `RECIPE_NOTES`, and bare cue headings like `Variations` should usually label as `OTHER` while signaling that the following rows may be `RECIPE_VARIANT`.
...
- heuristic label-mix diagnostics are now telemetry plus semantic-guard evidence. The live acceptance path still does not reject ordinary divergence from deterministic label distribution, but it now rejects narrow, explainable outside-recipe pathology patterns and records those diagnostics in shard metadata.
...
- `chunks.py` still exposes narrow utility-profile helpers for the optional knowledge stage. Those helpers summarize positive/negative utility cues such as cause/effect, troubleshooting, substitutions, storage/safety, framing, memoir, navigation, or true-but-low-utility filler. They are now an optional evidence surface only; the active knowledge ingest path no longer uses them to demote accepted model `knowledge` rows back to `other`.
...
- When `line_role_pipeline=codex-line-role-route-v2`, the visible stage-owned authority for that seam is now line-role itself: the run reuses the early line-role predictions to build `recipe_boundary`, writes authoritative labeled-line mirrors under `line-role-pipeline/authoritative_labeled_lines.jsonl`, and no longer publishes separate stage-owned `label_deterministic` / `label_refine` directories for that path.
...
- the active pre-grouping line-role runtime now also strips stale recipe ownership hints from any candidate objects it receives. Before grouping, live line-role cache identity, debug payloads, prompt preview, and worker-facing surfaces should be driven by raw line text plus nearby context only, not by `recipe_id`, `within_recipe_span`, or span-derived rule tags left over from older helper paths.
- `cookimport/parsing/recipe_span_grouping.py` now treats recipe-boundary acceptance as one explicit gate: a span needs a real `RECIPE_TITLE` anchor plus ingredient evidence plus instruction evidence before it becomes an accepted recipe span. `HOWTO_SECTION`, `YIELD_LINE`, `TIME_LINE`, `RECIPE_NOTES`, and `RECIPE_VARIANT` remain supportive only, and `RECIPE_VARIANT` can only stay inside an already-open parent recipe span instead of starting its own standalone recipe.
- open recipe spans now use one unified bounded-gap rule for `NONRECIPE_CANDIDATE` and `NONRECIPE_EXCLUDE`: a short run of those rows does not close an already-open title-anchored recipe, those gap rows stay outside the recipe, and any later non-title recipe-local label can resume the same recipe. A new `RECIPE_TITLE` or an overlong gap still closes the prior recipe.
- rejected pseudo-recipe spans now normalize their authoritative block labels to `NONRECIPE_CANDIDATE`, so incoherent recipe-shaped clusters re-enter nonrecipe knowledge review instead of surviving as recipe-owned structure or being thrown away as obvious junk.
...
- Title-only shells, title-plus-note junk, title-plus-yield/time stubs, title-plus-ingredient-only spans, and title-plus-instruction-only spans are all rejected during grouping unless the nearby cluster contains both ingredient evidence and instruction evidence.
- `build_conversion_result_from_label_spans(...)` no longer reopens ordinary recipe acceptance. If an already accepted span somehow projects to no ingredients, no instructions, and no yield/time metadata, the runtime keeps the accepted span and records an explicit invariant warning instead of demoting it semantically. Accepted in-span `RECIPE_VARIANT` rows now flow into the built `RecipeCandidate.recipeInstructions` sequence so downstream draft conversion can continue extracting `recipe.variants` from that instruction stream.
...
- Supports optional shard-worker labeling over the full ordered candidate set when `line_role_pipeline=codex-line-role-route-v2`; each shard row now carries only `atomic_index`, `block_index`, and raw line text, with neighboring context rows in that same compact tuple form when available. The live inline prompt renders owned rows as compact strings like `r01 | 210 | Bright Cabbage Slaw`, renders neighboring context as `210 | Variations`, and still expects one ordered `labels` array rather than replaying ids.
...
- Shard ledgers use strict ownership validation with the full global line-role label set available on every row; invalid or missing shard outputs now fail closed for their owned rows, and `shard_status.jsonl` plus `line_role_stage_summary.json` record unresolved-row counts plus `repair_recovered` / `repair_failed` / `invalid_output` outcomes.
- Row-level shard validation still belongs to deterministic code: wrong ownership, wrong row counts, duplicate rows, and frozen-row rewrites must all be rejected even when the live worker contract is just one editable `task.json`.
- The compact line-role summary now also exposes `attention_summary` so unresolved rows, transport mode, and missing/invalid shard counts are easy to spot at run-review time.
