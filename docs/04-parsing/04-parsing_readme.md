...
- heuristic label-mix diagnostics are now promotion gates rather than telemetry-only hints: a pathological uniform or near-uniform shard stays unresolved, triggers same-session repair on the live worker path, and still fails closed unless the shard later validates through that repair loop or one bounded watchdog retry after a retryable kill.
