...
- parent shard proposals are now shard-ledger summaries, not a second hidden task layer. Accepted rows stay authoritative, unresolved rows become repair requests, and the live worker path succeeds only when same-session repair ends in a clean installed shard ledger. If unresolved rows remain, the shard fails closed instead of borrowing deterministic labels.
...
- heuristic label-mix diagnostics are now promotion gates rather than telemetry-only hints: a pathological uniform or near-uniform shard stays unresolved, triggers same-session repair if allowed, and fails closed if the worker never installs a clean repaired ledger.
...
- Accepted Codex structure upgrades now fail closed instead of being silently rewritten or completed with baseline rows. Unsupported `HOWTO_SECTION`, `RECIPE_TITLE`, `RECIPE_VARIANT`, and `INSTRUCTION_LINE` labels are rejected explicitly with `codex_policy_rejected:*` evidence, while baseline-only rescues such as `OTHER -> INSTRUCTION_LINE` or `OTHER -> RECIPE_VARIANT` no longer run after Codex acceptance on the live worker path.
