...
- line-role inline-JSON packets now hide sparse `atomic_index` values from the visible prompt: the packet shows local `row_id` values in reading order plus optional boundary context rows, while repo-owned runtime keeps the `row_id -> atomic_index` mapping and now reports missing, duplicate, and unknown `row_id` coverage errors explicitly.
...
- semantic containment is route-safety only: deterministic-supportable outside-recipe exclusions may stay `NONRECIPE_EXCLUDE`, while the rest of a semantically rejected outside-recipe shard falls back to `NONRECIPE_CANDIDATE`. Deterministic code is not trying to relabel ordinary ambiguous prose.
...
- accepted Codex shard rows are still subject to repo policy rejection before promotion. Outside-recipe `HOWTO_SECTION`, `RECIPE_TITLE`, `INSTRUCTION_LINE`, and mismatched `NONRECIPE_EXCLUDE` outputs that violate local support rules are rejected back to the deterministic baseline with explicit `codex_policy_rejected:*` reason tags instead of being accepted as authoritative.
...
- Outside-recipe `NONRECIPE_EXCLUDE` is now effectively deterministic-authority even on the Codex path: Codex may still return that label, but runtime only accepts it when the local exclusion heuristic agrees on the same reason code; unsupported or mismatched exclusions fall back to the deterministic baseline instead of collapsing the nonrecipe candidate queue.
