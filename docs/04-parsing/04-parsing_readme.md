...
- line-role inline-JSON prompts now derive from the same shared contract file as the other prompt surfaces and carry shard-profile evidence for front-matter, contents, and other outside-recipe-heavy shards. Deterministic code packages that evidence but does not silently decide the semantic label from it.
...
- line-role validation no longer semantically vetoes structurally valid front-matter- or contents-looking shards. Shard-profile evidence is still packaged into prompt/runtime metadata for reviewer context, but it does not invalidate otherwise legal model labels.
...
- accepted Codex shard rows are still subject to repo policy rejection before promotion. Outside-recipe `HOWTO_SECTION`, `RECIPE_TITLE`, and `INSTRUCTION_LINE` outputs that violate local support rules are rejected back to the deterministic baseline with explicit `codex_policy_rejected:*` reason tags instead of being accepted as authoritative, and `NONRECIPE_EXCLUDE` is now only vetoed on narrow structural boundaries such as impossible recipe-local exclusion.
...
- `cookimport/parsing/recipe_span_grouping.py` now treats recipe-boundary acceptance as one explicit gate: a span needs a title-like anchor plus ingredient evidence plus instruction evidence before it becomes an accepted recipe span. `HOWTO_SECTION`, `YIELD_LINE`, `TIME_LINE`, `RECIPE_NOTES`, and `RECIPE_VARIANT` remain supportive only.
...
- Outside-recipe `NONRECIPE_EXCLUDE` is now primarily an LLM semantic decision on the normal Codex path. Deterministic code still owns the rule/baseline path, shard-level semantic-pathology containment, and narrow structural fail-safes such as impossible recipe-local exclusion, but it does not broad-veto ordinary outside-recipe exclusions merely because the local heuristic would have kept the row reviewable.
- Semantically rejected outside-recipe shards now also fail safe at projection time: shard metadata can carry `semantic_containment_rows`, and those rows project as fallback-owned `NONRECIPE_CANDIDATE` or deterministic-supported `NONRECIPE_EXCLUDE` outputs instead of preserving hallucinated recipe-local labels.
...
- Shard ledgers use strict ownership validation with the full global line-role label set available on every row; invalid or missing shard outputs now fail closed for their owned rows, and `shard_status.jsonl` plus `line_role_stage_summary.json` record unresolved-row counts plus `repair_recovered` / `repair_failed` / `invalid_output` / `semantic_pathology_rejected` outcomes.
...
- The compact line-role summary now also exposes `attention_summary` so unresolved rows, Codex hard-policy rejections, suspicious shards, semantic containment, transport mode, and missing/invalid shard counts are easy to spot at run-review time.
