...
- `RECIPE_VARIANT` can now cover short local alternate-version runs: a `Variations` heading or a long named `To make ...` lead can keep the immediately following ingredient or instruction rows inside the same variant subsection, but generic `To make the ...` method steps and following note-only rows stay under their ordinary structural labels.
