...
- line-role direct-exec workers are now `task.json`-first on the happy path. The visible worker session edits only `/units/*/answer` in `task.json`; repo-owned `assigned_shards.json`, `in/<shard_id>.json`, `debug/<shard_id>.json`, `hints/<shard_id>.md`, and `out/<shard_id>.json` remain supporting context and artifacts outside that contract.
