...
- Shard ledgers use strict ownership validation with the full global line-role label set available on every row; invalid or missing shard outputs fall back only for their owned rows, and `shard_status.jsonl` plus `line_role_stage_summary.json` record exactly which shards fell back.
