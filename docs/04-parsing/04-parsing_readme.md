...
- line-role validation no longer semantically vetoes structurally valid but surprising Codex labels. Repo code still enforces row coverage, ownership, and allowed-label contract shape, but it no longer rewrites accepted Codex answers back to deterministic baseline labels for local-support reasons.
...
- the obvious-junk heuristics remain a repo-owned deterministic seam for the rule/baseline path, and the live Codex route path still does not do broad deterministic semantic rewrites. The new runtime guard is limited to explainable semantic-pathology rejection plus conservative containment on unresolved outside-recipe shards.
