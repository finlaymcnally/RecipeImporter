...
- line-role direct-exec workers are now `task.json`-first on the happy path. The visible worker session edits only `/units/*/answer` in `task.json`; the worker-visible evidence is raw row text plus stable provenance, not a repo-authored deterministic label hint. Repo-owned `assigned_shards.json`, `in/<shard_id>.json`, `debug/<shard_id>.json`, `hints/<shard_id>.md`, and `out/<shard_id>.json` remain supporting context and artifacts outside that contract.
...
- the line-role workspace watchdog is still warning-only for ordinary helper use, repeated reasoning, and cohort-runtime outliers, but it now has one explicit missing-output fail-fast branch: after a final agent message, runtime gives missing `workers/*/out/*.json` files a 2-second grace window, then kills that session with `workspace_final_message_missing_output`, records a deterministic `--status` / `--doctor` recovery assessment, and either spends the one shared fresh-session retry budget on the preserved workspace or fails closed with durable rescue metadata.
