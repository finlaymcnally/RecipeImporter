...
- line-role validation does not rewrite accepted Codex labels back to deterministic baseline guesses, and it does not run a post-acceptance semantic veto pass. Once a live inline `labels` array passes structural validation, shard ownership checks, and label legality checks, runtime accepts those rows as-is.
