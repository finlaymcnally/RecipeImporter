...
- each line-role run now writes one immutable `canonical_line_table.jsonl` before any worker promotion happens. Packet manifests, packet status rows, and final labels all refer back to those stable line ids (`atomic_index` string ids).
- each line-role packet now writes three durable artifacts: the authoritative worker input at `line-role-pipeline/runtime/line_role/workers/*/in/*.json`, the richer local debug copy at `.../debug/*.json`, and one packet-status ledger row in `line-role-pipeline/runtime/line_role/task_status.jsonl`.
- line-role workers now also get `line-role-pipeline/runtime/line_role/workers/*/hints/*.md` plus worker-local `examples/*.md` contrast files. Those hint sidecars are the worker-facing explanation layer: label-code legend, packet profile, packet interpretation, conservative flip policy, short attention-row summaries, and book-local `HOWTO_SECTION` posture. The validator still treats `in/*.json` as authoritative.
- line-role direct-exec workers now expose the same paved-road workspace surface as the other long-lived stages: `current_task.json` for the cheapest next-task row, `OUTPUT_CONTRACT.md` for exact output shape, `examples/valid_line_role_output.json` for one good structural example, and `tools/line_role_worker.py` for fallback/debug helper verbs. The repo now prewrites deterministic seed drafts under `scratch/<task_id>.json` and surfaces that path in `metadata.scratch_draft_path`, so the default loop is open the current hint/input/draft, edit only real disagreements, then run `finalize` or `finalize-all` rather than spending commands on draft creation.
- accepted Codex packet rows now follow one narrow post-validation contract: structurally valid labels either survive unchanged as `decided_by="codex"` or reject explicitly to the deterministic baseline for that row with `codex_policy_rejected:*` reason tags. Metadata cleanup may still normalize `review_exclusion_reason`, but repo code must not silently rewrite accepted Codex semantics into a third label.
- the older March 19 inline-authority workaround for one-shot direct calls is retained only as history. Current mainline authority is the worker-local packet files plus repo-written hint/debug sidecars, not illustrative wrapper examples.
- the worker `in/*.json` transport is now `{"v":1,"shard_id":...,"packet_summary":...,"default_posture":...,"howto_section_availability":...,"howto_section_policy":...,"flip_policy":[...],"example_files":[...],"context_before_rows":[[atomic_index,current_line], ...],"rows":[[atomic_index,label_code,current_line], ...],"context_after_rows":[[atomic_index,current_line], ...]}`. It is still one ordered contiguous slice of the book, but it no longer carries per-row operator metadata such as block ids, recipe hints, rule tags, or escalation reasons.
- `context_before_rows` and `context_after_rows` are task-local reference rows around one worker packet. Workers may read them for nearby context, but only `rows` is authoritative and labelable.
- packet-mode classification now fails closed when every row has `within_recipe_span=None`: front matter and contents-style title lists are framed as `front_matter_navigation` instead of `recipe_body` unless the packet shows strong local ingredient/step evidence.
...
- parent shard proposals are now packet-first summaries, not the authority seam. A bad packet falls back only for its owned rows, while valid packet outputs still promote for the rest of the shard.
...
- outside-recipe line-role is now routing-only for semantic prose: clear lesson headings and reusable explanatory prose still stay review-eligible `OTHER`, and only the later knowledge stage can turn those rows into final semantic `KNOWLEDGE`. Line-role still keeps recipe structure plus decisive review exclusion.
- `CanonicalLineRolePrediction` can now carry optional `review_exclusion_reason` on outside-recipe `OTHER` rows. The allowed repo-owned reason codes are only for overwhelming obvious junk surfaces such as navigation/front matter, publishing/legal metadata, endorsements, and isolated page furniture.
- the obvious-junk veto is now a little broader than the first routing-only pass: endorsement quote clusters and explicit publisher/signup ebook boilerplate can also be excluded upstream, while plausible lesson headings and explanatory cooking prose must still fail open as review-eligible `OTHER`.
...
- valid packet outputs are now authoritative even when heuristic label mix diagnostics complain; the heuristic only records `semantic_diagnostics` and `suspicious_packet` counters instead of invalidating the packet on disagreement alone.
- line-role resume now reuses already-validated `workers/*/out/*.json` packet files and only re-asks missing or invalid packets. The worker prompt for this stage now emphasizes direct reads from the named files, conservative flips, and repo-written contrast examples rather than open-ended semantic relabeling.
- workspace-worker watchdog completion for line-role is now output-aware: once every assigned `workers/*/out/*.json` packet exists and stabilizes across snapshots, the session finalizes as `completed` with `workspace_outputs_stabilized` instead of being counted as a late command-loop kill.
...
- line-role runtime telemetry now also carries `final_proposal_status` on workspace rows when a later retry or repair changes the accepted outcome. Summary counters such as `invalid_output_shard_count` and `missing_output_shard_count` should therefore reflect the final shard result instead of stale first-attempt proposal status.
...
- Inside recipe spans, `KNOWLEDGE` is restricted; accepted Codex `KNOWLEDGE` rows now either survive with local support or reject explicitly to deterministic baseline, and the deterministic baseline keeps any repo-owned coercions in its own lane.
...
- Outside recipe spans, prose now defaults to `OTHER`; line-role still treats outside-recipe `KNOWLEDGE` as non-final routing input, so accepted Codex outside-recipe `KNOWLEDGE` rows reject explicitly to deterministic baseline instead of being silently coerced afterward.
...
- Packet outputs use strict ownership validation with the full global line-role label set available on every row; invalid or missing packets fall back only for their owned rows, and `task_status.jsonl` plus `line_role_stage_summary.json` record exactly which packets fell back.
...
- Accepted Codex structure upgrades now fail closed by explicit baseline fallback, not by hidden rewrite. Unsupported `HOWTO_SECTION`, `RECIPE_TITLE`, `RECIPE_VARIANT`, and `INSTRUCTION_LINE` labels land on deterministic baseline with `codex_policy_rejected:*` evidence, while baseline-only rescues such as `OTHER -> INSTRUCTION_LINE` or `OTHER -> RECIPE_VARIANT` no longer run after Codex acceptance.
- The practical intent of that reject-to-baseline seam is narrow: keep explicit `FOR THE ...` component headings and real outside-span ingredient/instruction clusters when Codex already got them right, but stop generic cookbook lesson headings and memoir prose from being promoted into recipe structure by a hidden repo rewrite.
...
- Outside-recipe `KNOWLEDGE` labeling is still benchmark-critical, but canonical benchmark scoring now projects recipe-local Stage 2 labels together with explicit final non-recipe authority only. Unreviewed review-eligible seed guesses are kept out of scored `KNOWLEDGE` promotion until the knowledge stage actually reviews them.
...
- `INSTRUCTION_LINE` now means recipe-local procedural steps for the current recipe only. Cookbook advice, explanatory prose, and action-verb guidance outside a concrete recipe procedure should stay review-eligible `OTHER` unless they are obvious junk that should be excluded.
...
- The main 2026-03-22 Salt Fat miss families were:
  - lesson prose collapsing from `KNOWLEDGE -> OTHER`
  - front matter or contents rows drifting from `OTHER -> RECIPE_TITLE`
  - obvious imperative steps drifting from `INSTRUCTION_LINE -> OTHER`
  - short variant runs collapsing to `OTHER` or ordinary instruction lines
  The current contract fixes those only on exact-evidence seams and still relies on `thefoodlabcutdown` and `seaandsmokecutdown` as contrast-book canaries.
