...
- parent shard proposals are now shard-ledger summaries, not a second hidden task layer. Accepted rows stay authoritative, unresolved rows become repair requests, and the live worker path still prefers a clean installed shard ledger or one bounded watchdog retry. When a shard keeps some accepted rows but still leaves holes, runtime preserves those accepted Codex rows but does not backfill the unresolved atomic indices from the deterministic baseline; the shard stays incomplete/invalid unless repair or retry finishes it. Full-zero-authority shards still fail closed.
