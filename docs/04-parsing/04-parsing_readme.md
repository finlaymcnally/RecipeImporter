...
- line-role inline-JSON packets now hide sparse `atomic_index` values from the visible prompt and use raw row objects instead of compact pipe-delimited strings. Owned rows render as packet-local `{row_id, block_index, text}` objects and optional neighboring context rows render as `{block_index, text}` reference objects, while repo-owned runtime keeps the shard order authoritative and maps accepted `row_id` answers back onto real `atomic_index` values.
...
- line-role inline repair reuses the same prepared execution workspace and resumes the prior transcript history. That keeps stable local paths for prompt artifacts and lineage while preserving the broader shard conversation for narrowed unresolved-row followups; exact-count enforcement on repair remains repo-owned because `codex exec resume` does not expose `--output-schema`.
- line-role inline-JSON prompts now derive from the same shared contract file as the other prompt surfaces, but worker-visible packets stay raw-text-first: owned rows plus optional nearby context rows only, with no repo-authored shard-profile or semantic sidecar evidence injected into the prompt. The inline prompt also explicitly tells the model to treat `rows` as one contiguous shard slice and not judge rows in isolation while labeling.
...
- partial line-role authority is now honest: accepted rows stay accepted, repair packets narrow to unresolved owned rows, and unresolved rows fail closed if repair/retry still does not finish them. The runtime no longer pads those holes out with deterministic baseline labels.
