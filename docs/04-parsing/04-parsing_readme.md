...
- line-role inline-JSON packets now hide sparse `atomic_index` values from the visible prompt and no longer ask the worker to replay packet-local ids. The packet shows owned rows as ordered text-only rows plus optional boundary context rows, while repo-owned runtime keeps the shard order authoritative and zips returned label lists back onto real `atomic_index` values.
- line-role inline validation now accepts one ordered `labels` array as the primary happy-path contract, but packet-local ordinal `rows` replies are also first-class when they are unambiguous. If a worker answers with packet-local `row_id` rows or with `atomic_index` values that clearly form local ordinals (`0..n-1` or `1..n`), deterministic code remaps that output onto the shard's true ordered `atomic_index` values, preserves any validated prefix rows, trims one obvious extra trailing context-row spill, and reprompts only the unresolved owned rows. Ambiguous wider malformed outputs still fail closed.
...
- `chunks.py` now also exposes narrow utility-profile helpers for the optional knowledge stage. Those helpers summarize positive/negative utility cues such as cause/effect, troubleshooting, substitutions, storage/safety, framing, memoir, navigation, or true-but-low-utility filler. The active knowledge ingest path now reuses that helper for one narrow post-review demotion seam: memoir/framing/navigation/decorative-heading/true-but-low-utility rows can be dropped back to `other` even after a plausible model grounding, while broader semantic keep/drop authority still stays with the model.
