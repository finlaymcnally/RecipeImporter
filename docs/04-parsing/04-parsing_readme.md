...
- line-role workers now also get `line-role-pipeline/runtime/line_role/workers/*/hints/*.md`. Those hint sidecars are worker-facing orientation only: shard profile, short static reminders, label-code legend, and attention-row summaries. The validator still treats `in/*.json` as authoritative.
- line-role direct-exec workers now expose one shard-owned paved road: `current_phase.json`, `CURRENT_PHASE.md`, `CURRENT_PHASE_FEEDBACK.md`, `assigned_shards.json`, `OUTPUT_CONTRACT.md`, `tools/line_role_worker.py`, immutable `in/<shard_id>.json`, editable `work/<shard_id>.json`, and optional `repair/<shard_id>.json`. `check-phase` now freezes accepted rows immediately, writes repair requests only for unresolved owned rows, and rejects later edits that try to reopen those frozen rows; `install-phase` succeeds only after the whole shard validates cleanly, and there is no separate repo-owned line-role repair prompt after the worker session.
...
- the older March 19 inline-authority workaround for one-shot direct calls is retained only as history. Current mainline authority is the worker-local shard ledger plus repo-written hint/debug sidecars, not illustrative wrapper examples or contrast packs.
