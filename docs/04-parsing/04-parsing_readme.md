...
- `cookimport/parsing/canonical_line_roles/contracts.py` now owns the prediction model plus normalization helpers, `cookimport/parsing/canonical_line_roles/prompt_inputs.py` owns the reusable row-serialization helpers, and `cookimport/parsing/canonical_line_roles/__init__.py` is the still-large planning/runtime facade while the rest of the package split is in flight.
