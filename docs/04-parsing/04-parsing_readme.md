...
- outside-recipe line-role now preserves parser-supported `KNOWLEDGE` for clear lesson headings and reusable explanatory prose, while still leaving unsupported outside-recipe prose as review-eligible `OTHER`. The optional knowledge stage handles the harder semantic border cases, and downstream projection now ignores unresolved Stage 7 seed guesses instead of treating them as reviewed `OTHER`.
...
- parser-owned outside-recipe review exclusion now goes through two explicit seams in `canonical_line_roles.py`: `_outside_recipe_knowledge_label_allowed(...)` is the narrow authority for keeping outside-recipe `KNOWLEDGE` or falling unsupported structure/instruction flips back to knowledge-safe labels, while `_outside_recipe_review_exclusion_reason(...)` is the coarse-veto seam that marks only obviously useless outside-recipe `OTHER` rows for knowledge-review pruning.
- lesson-prose prompt posture now explicitly keeps short concept headings such as `Balancing Fat` and short declarative teaching lines in the same knowledge cluster as `KNOWLEDGE`, while lone unsupported question headings such as `What is Heat?` still stay `OTHER`.
- outside-recipe `INSTRUCTION_LINE` support now treats nearby recipe-flow steps as local structure, not just nearby ingredients/headings. This keeps trailing tail steps such as the Salt Fat crouton cool/store instructions from falling back to `OTHER` when they sit just outside the grouped recipe span.
