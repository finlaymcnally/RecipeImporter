...
- outside-recipe line-role is now fail-open `OTHER` plus optional review-exclusion metadata, not final semantic `KNOWLEDGE` authority. Explanatory lesson prose, essay fragments, and headings still pass through line-role, but they stay review-eligible `OTHER` so the optional knowledge stage can make the final `knowledge` versus `other` call.
...
- parser-owned outside-recipe review exclusion now goes through two explicit seams in `canonical_line_roles.py`: `_outside_recipe_knowledge_label_allowed(...)` still recognizes lesson-like prose so unsupported structure/instruction flips can fall back safely, while `_outside_recipe_review_exclusion_reason(...)` is the narrow coarse-veto seam that marks only obviously useless outside-recipe `OTHER` rows for knowledge-review pruning.
