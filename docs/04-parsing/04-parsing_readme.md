...
- line-role inline-JSON packets now hide sparse `atomic_index` values from the visible prompt and no longer ask the worker to replay packet-local ids. The packet shows owned rows as one ordered array of raw-text strings plus optional boundary context arrays, while repo-owned runtime keeps the shard order authoritative and zips returned label lists back onto real `atomic_index` values.
- line-role inline validation is now strict labels-only. The live worker must return one ordered `labels` array with no extra keys, deterministic code validates label count and label legality, and then maps `labels[i]` back onto the shard's ordered `atomic_index` values. The initial inline turn gets a packet-local exact-length output schema so too-short label arrays can fail at the transport boundary instead of only in later repo validation. Row-shaped fallback replies are no longer salvaged on the live inline path; they fail closed into repair or invalid output.
...
- line-role validation still does not rewrite accepted Codex labels back to deterministic baseline guesses, but it now does narrow semantic rejection for strong recipe-local contradictions. When a structurally valid `labels` array puts an obvious recipe title, strict yield line, obvious ingredient, strong instruction line, note line, or variant line into a locally contradictory label, runtime removes only those suspicious rows from the accepted ledger, records `semantic_diagnostics`, and routes those rows through bounded repair instead of silently promoting the whole first-pass array.
