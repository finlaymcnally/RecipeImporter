...
- `context_before_rows` and `context_after_rows` are reference-only overlap from adjacent shards. Workers may read them for context, but only `rows` is authoritative and labelable.
