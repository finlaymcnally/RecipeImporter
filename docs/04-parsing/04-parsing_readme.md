...
- line-role workers now also get `line-role-pipeline/runtime/line_role/workers/*/hints/*.md` plus worker-local `examples/*.md` contrast files. Those hint sidecars are the worker-facing explanation layer: label-code legend, shard profile, shard interpretation, conservative flip policy, short attention-row summaries, and book-local `HOWTO_SECTION` posture. The validator still treats `in/*.json` as authoritative.
- line-role direct-exec workers now expose one shard-owned paved road: `current_phase.json`, `CURRENT_PHASE.md`, `CURRENT_PHASE_FEEDBACK.md`, `assigned_shards.json`, `OUTPUT_CONTRACT.md`, `examples/valid_line_role_output.json`, `tools/line_role_worker.py`, immutable `in/<shard_id>.json`, editable `work/<shard_id>.json`, and optional `repair/<shard_id>.json`. `check-phase` now freezes accepted rows immediately, writes repair requests only for unresolved owned rows, and rejects later edits that try to reopen those frozen rows; `install-phase` succeeds only after the whole shard validates cleanly, and there is no separate repo-owned line-role repair prompt after the worker session.
- accepted Codex shard rows now follow one narrow post-validation contract: structurally valid labels either survive unchanged as `decided_by="codex"` or reject explicitly to the deterministic baseline for that row with `codex_policy_rejected:*` reason tags. Metadata cleanup may still normalize `review_exclusion_reason`, but repo code must not silently rewrite accepted Codex semantics into a third label.
- the older March 19 inline-authority workaround for one-shot direct calls is retained only as history. Current mainline authority is the worker-local shard ledger plus repo-written hint/debug sidecars, not illustrative wrapper examples.
- the worker `in/*.json` transport is now `{"v":1,"shard_id":...,"shard_summary":...,"default_posture":...,"howto_section_availability":...,"howto_section_policy":...,"flip_policy":[...],"example_files":[...],"context_before_rows":[[atomic_index,current_line], ...],"rows":[[atomic_index,label_code,current_line], ...],"context_after_rows":[[atomic_index,current_line], ...]}`. It is still one ordered contiguous slice of the book, but it no longer carries per-row operator metadata such as block ids, recipe hints, rule tags, or escalation reasons.
- `context_before_rows` and `context_after_rows` are reference-only neighboring rows around the owned shard rows. Workers may read them for nearby context, but only `rows` is authoritative and labelable.
...
- parent shard proposals are now shard-ledger summaries, not a second hidden task layer. A bad shard ledger no longer forces all-or-nothing fallback: accepted rows stay authoritative, unresolved rows become repair requests, and any unresolved remainder after repair exhausts falls back only for those owned rows.
...
- outside-recipe `title_like` and `ingredient_like` rows now fail closed unless nearby lines show real recipe scaffold such as yield, quantity ingredients, how-to structure, or direct instructions. Chapter-taxonomy trios like `The Four Elements of Good Cooking / SALT / What is Salt?` should stay `OTHER`, not seed pseudo-recipes.
...
- heuristic label-mix diagnostics are now promotion gates rather than telemetry-only hints: a pathological uniform or near-uniform shard stays unresolved, triggers repair if allowed, and falls back only for the unresolved rows that remain after repair is exhausted.
- line-role resume now reuses already-validated `workers/*/out/*.json` shard ledgers and only re-asks missing or invalid shards. The worker prompt for this stage now emphasizes direct reads from the named files, conservative flips, and repo-written contrast examples rather than open-ended semantic relabeling.
- workspace-worker watchdog completion for line-role is now output-aware: once every assigned `workers/*/out/*.json` shard ledger exists and stabilizes across snapshots, the session finalizes as `completed` with `workspace_outputs_stabilized` instead of being counted as a late command-loop kill.
...
- line-role runtime telemetry now also carries `final_proposal_status` on workspace rows when a later watchdog retry or final row-local fallback changes the accepted outcome. Summary counters such as `invalid_output_shard_count` and `missing_output_shard_count` should therefore reflect the final shard result instead of stale first-attempt proposal status.
...
- Shard ledgers use strict ownership validation with the full global line-role label set available on every row; invalid or missing shard outputs fall back only for their owned rows, and `shard_status.jsonl` plus `line_role_stage_summary.json` record exactly which shards fell back. The compact line-role summary now also exposes `attention_summary` so fallback rows, Codex hard-policy rejections, suspicious shards, and missing/invalid shard counts are easy to spot at run-review time.
