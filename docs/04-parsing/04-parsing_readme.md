...
- line-role workers now also get `line-role-pipeline/runtime/line_role/workers/*/hints/*.md` plus worker-local `examples/*.md` contrast files. Those hint sidecars are worker-facing orientation only: shard profile, short static reminders, label-code legend, and attention-row summaries. The validator still treats `in/*.json` as authoritative.
...
- the worker `in/*.json` transport is now `{"v":1,"shard_id":...,"context_before_rows":[[atomic_index,current_line], ...],"rows":[[atomic_index,label_code,current_line], ...],"context_after_rows":[[atomic_index,current_line], ...]}`. The boundary-context keys are optional and appear only when immediate neighbors exist. It is still one ordered contiguous slice of the book, but it no longer carries per-row operator metadata such as block ids, recipe hints, rule tags, escalation reasons, or shard-level posture fields.
