...
- outside-recipe `KNOWLEDGE` labeling is deterministic-first and now high-evidence/low-default in `canonical_line_roles.py`; reusable explanatory/reference prose and explicit lesson headings still promote, and brief first-person lesson prose can also promote when it clearly states a reusable cooking rule, but memoir, blurbs, endorsements, book-framing encouragement, and broad action-verb advice still stay `OTHER` unless stronger evidence is present.
- parser-owned outside-recipe `KNOWLEDGE` authority now goes through one explicit seam: `_outside_recipe_knowledge_label_allowed(...)` gates both deterministic promotion and `_sanitize_prediction(...)`, so unsupported Codex `KNOWLEDGE` upgrades fall back to `OTHER` deterministically, while Codex `OTHER` can be rescued back to `KNOWLEDGE` when that same seam has strong evidence.
