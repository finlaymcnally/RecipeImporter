...
- `cookimport/parsing/canonical_line_roles/contracts.py` now owns the prediction model plus normalization helpers, `cookimport/parsing/canonical_line_roles/prompt_inputs.py` owns the reusable row-serialization helpers, `cookimport/parsing/canonical_line_roles/artifacts.py` owns runtime/prompt artifact helpers, and `cookimport/parsing/canonical_line_roles/__init__.py` is the still-large planning/runtime facade while the rest of the package split is in flight.
...
- Post-Codex sanitizers now fail closed on unsupported structure upgrades. `HOWTO_SECTION`, outside-span `INSTRUCTION_LINE`, and other recipe-ish labels survive only when the local rows show actual recipe-component evidence; otherwise the sanitizer demotes them back to `KNOWLEDGE`, `OTHER`, or `RECIPE_NOTES`.
