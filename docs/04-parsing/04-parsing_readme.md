...
- parent shard proposals no longer silently pass when one task packet returns unowned or otherwise invalid rows. The runtime may still synthesize baseline rows for local debugging, but any task-level validation error now invalidates the parent shard so parse-error artifacts and deterministic fallback stay honest.
