...
- Knowledge groups are reviewer-facing context from the always-on second pass; Codex `block_decisions` are what refine final `KNOWLEDGE` versus `OTHER`, and the promoted group artifact records both grouping plus proposal-resolution outcomes for kept rows.
