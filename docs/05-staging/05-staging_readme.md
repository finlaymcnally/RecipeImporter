...
- Recipe and non-recipe live Codex roots now also keep `phase_plan.json` plus `phase_plan_summary.json` beside `phase_manifest.json`. Those planning artifacts are the durable source for requested shards vs survivability recommendation vs budget-native packetization vs actual launch count.
