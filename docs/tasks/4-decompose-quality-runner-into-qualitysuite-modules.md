...
- [x] (2026-03-23 21:06 EDT) Restored the remaining lightweight compatibility imports (`_expand_experiments`, `_load_experiment_file`) from the new planning module so the thin facade stays source-compatible for existing read-side callers without reintroducing the old runtime body.
