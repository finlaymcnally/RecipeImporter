...
- [ ] Continue the decomposition in small atomic commits, starting with the remaining bench and Label Studio wave and then the deep LLM runtime coordinators.
...
- Observation: AST-assisted splits of prompt tooling were fast, but they surfaced missing cross-module imports and dropped decorators that runtime tests caught immediately.
...
Current outcome: the repo now has a concrete, partially executed roadmap instead of an ad hoc “split giant files someday” intention. Milestone 1 is landed, the shared analytics manifest runtime/token layer is landed, and Milestone 2 is now complete: compare/control field normalization, filter logic, and statistics/insights live in explicit owner modules while `compare_control_engine.py` stays as the stable public facade.

Remaining gap: the bench and Label Studio wave plus the deep LLM/runtime decomposition wave are still pending. This plan remains the source of truth for those later cuts.
Current size markers after the latest wave: `prompt_artifacts.py` is now `702` lines, `prompt_budget.py` is `131`, `prelabel.py` is `427`, and `cli_commands/labelstudio.py` is `3559`.
