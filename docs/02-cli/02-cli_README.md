...
- for interactive benchmark modes (`single_book`, `selected_matched_books`, `all_matched_books`), that planner asks about:
  ...
  - knowledge grouping no longer has a separate toggle; when the knowledge surface is on, the runtime always does the second pass that groups retained knowledge and resolves `proposal_candidate` rows
