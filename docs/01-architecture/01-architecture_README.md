...
- `cookimport/staging/nonrecipe_stage.py` and `cookimport/staging/semantic_row_predictions.py` are now thin public seams. The owned logic lives under `nonrecipe_authority_contract.py`, `nonrecipe_seed.py`, `nonrecipe_routing.py`, `nonrecipe_authority.py`, `nonrecipe_finalize_status.py`, `recipe_row_evidence.py`, `knowledge_row_evidence.py`, and `row_label_resolution.py`.
...
- the semantic stage registry used by `stage_observability.json` now sorts stages to match runtime dependencies instead of historical insertion order, and it uses simple 10-step slots so the numbers are readable: Codex-backed `line_role` = 10, deterministic label stages = 20/30, `recipe_boundary` = 40, the recipe pipeline = 50/60/70, `nonrecipe_route` = 80, `nonrecipe_finalize` = 90, and `write_outputs` = 100.
