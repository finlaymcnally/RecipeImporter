---
summary: "ExecPlan for deterministic non-recipe table extraction, table-aware chunking, and optional pass4 table hints."
read_when:
  - "When implementing or auditing knowledge-table extraction behavior"
---
...
- [x] (2026-02-25 16:20Z) Implemented deterministic table extraction in `cookimport/parsing/tables.py` (pipe/tab/multispace row detection, run grouping, header inference, markdown/row_text rendering, block annotation).
- [x] (2026-02-25 16:21Z) Added table artifact writing (`tables.jsonl`, `tables.md`) in `cookimport/staging/writer.py` and wired it into non-split stage, split merge stage, and processed-output snapshots.
- [x] (2026-02-25 16:21Z) Made chunking table-aware in `cookimport/parsing/chunks.py` (no max-char split inside same `table_id`; chunks with `provenance.table_ids` are forced to `knowledge` lane).
- [x] (2026-02-25 16:22Z) Extended pass4 knowledge bundle blocks with optional `table_hint` and updated `llm_pipelines/prompts/recipe.knowledge.v1.prompt.md` with the evidence-safe rule.
- [x] (2026-02-25 16:22Z) Added tests + docs updates (`tests/parsing/test_tables.py`, chunking/LLM/CLI/run-settings tests, docs in `04-parsing`, `05-staging`, `10-llm`, `02-cli`, and conventions).
- [ ] (2026-02-25 16:24Z) Validate on at least one PDF and one EPUB that actually yield non-empty detected tables (completed: fixture runs on `sample.pdf`, `sample.epub`, `WhatGoodCooksKnowCUTDOWN.epub`, and `thefoodlabCUTDOWN.epub` wrote table artifacts but detected zero tables; remaining: capture a positive in-repo or user-supplied source with row-like table lines).

## Surprises & Discoveries

- Observation: In current fixture books, non-recipe blocks often did not preserve clear row separators (`|`, tabs, or 2+ spacing), so deterministic row heuristics produced empty table sets.
  Evidence: `/tmp/recipeimport-table-check/2026-02-25_16.22.05/tables/sample/tables.jsonl` has 0 lines; `/tmp/recipeimport-table-check/2026-02-25_16.23.11/tables/thefoodlabcutdown/tables.jsonl` has 0 lines.
- Observation: Pass4 chunk block payloads are built from `full_blocks` by absolute index, so table hints must be overlaid by index from annotated `non_recipe_blocks`.
  Evidence: `cookimport/llm/codex_farm_knowledge_jobs.py` now builds `table_hints_by_index` and injects `chunk.blocks[*].table_hint` during payload assembly.
...
- Decision: Annotate existing non-recipe block dicts (`features.table_id`, `features.table_row_index`, `table_hint`) instead of introducing a new block type.
  Rationale: Existing chunking and pass4 paths already consume `non_recipe_blocks`; annotation avoids invasive model/schema churn.
  Date/Author: 2026-02-25 / GPT-5.2 Pro

- Decision: Always emit `tables/<workbook_slug>/tables.jsonl` and `tables.md` when `table_extraction=on`, even if empty.
  Rationale: Deterministic output layout is easier for downstream automation than conditional file existence.
  Date/Author: 2026-02-25 / GPT-5.2 Pro

## Outcomes & Retrospective

- Delivered: deterministic table extraction module, stage/merge wiring, table artifacts, table-aware chunking, pass4 `table_hint` contract, CLI/run-setting integration, and regression tests.
- Confirmed: enabling `--table-extraction on` writes table artifact files under `tables/<workbook_slug>/` without breaking staging flows.
- Gap: fixture validation did not yet yield a positive detected table in EPUB/PDF runs; this needs one follow-up source that preserves row separators in non-recipe blocks.
...
- Targeted test runs (all passed):

    source .venv/bin/activate && pytest -q tests/parsing/test_tables.py tests/parsing/test_chunks.py tests/llm/test_knowledge_job_bundles.py tests/cli/test_cli_llm_flags.py tests/llm/test_run_settings.py tests/staging/test_run_manifest_parity.py

    source .venv/bin/activate && pytest -q tests/cli/test_c3imp_interactive_menu.py tests/llm/test_llm_pipeline_pack_assets.py tests/llm/test_llm_pipeline_pack.py

- Stage fixture runs with table extraction enabled (artifacts written, zero detected tables in current fixtures):

    source .venv/bin/activate && cookimport stage tests/fixtures/sample.epub --out /tmp/recipeimport-table-check --workers 1 --pdf-split-workers 1 --epub-split-workers 1 --table-extraction on

    source .venv/bin/activate && cookimport stage tests/fixtures/sample.pdf --out /tmp/recipeimport-table-check --workers 1 --pdf-split-workers 1 --epub-split-workers 1 --table-extraction on

    source .venv/bin/activate && cookimport stage data/input/WhatGoodCooksKnowCUTDOWN.epub --out /tmp/recipeimport-table-check --workers 1 --pdf-split-workers 1 --epub-split-workers 1 --table-extraction on

    source .venv/bin/activate && cookimport stage data/input/thefoodlabCUTDOWN.epub --out /tmp/recipeimport-table-check --workers 1 --pdf-split-workers 1 --epub-split-workers 1 --table-extraction on

- Example generated table-artifact paths from those runs:

    /tmp/recipeimport-table-check/2026-02-25_16.22.05/tables/sample/tables.md
    /tmp/recipeimport-table-check/2026-02-25_16.22.05/tables/sample/tables.jsonl
    /tmp/recipeimport-table-check/2026-02-25_16.23.11/tables/thefoodlabcutdown/tables.md
    /tmp/recipeimport-table-check/2026-02-25_16.23.11/tables/thefoodlabcutdown/tables.jsonl
...
- In `cookimport/parsing/tables.py`:
    ...
    def detect_tables_from_non_recipe_blocks(non_recipe_blocks: Sequence[Mapping[str, Any]], *, source_hash: str = "unknown", min_rows: int = 3, caption_lookback: int = 3) -> list[ExtractedTable]

    def annotate_non_recipe_blocks_with_tables(non_recipe_blocks: list[dict[str, Any]], tables: Sequence[ExtractedTable]) -> None
    def extract_and_annotate_tables(non_recipe_blocks: list[dict[str, Any]], *, source_hash: str = "unknown") -> list[ExtractedTable]
...
- Block/table markers:
  ...
  - table_hint: dict | None (`table_id`, `caption`, `markdown`, `row_index_in_table`)
...
- 2026-02-25: Updated after implementation to reflect shipped interfaces, tests, docs updates, and fixture-validation results (including the current zero-detection limitation on sampled EPUB/PDF fixtures).
