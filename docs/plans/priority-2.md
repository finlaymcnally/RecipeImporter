...
- [x] (2026-02-27_22.31.00) Milestone 1 completed: added shared detector core in `cookimport/parsing/section_detector.py` (`SectionKind`, `SectionSpan`, `DetectedSections`, line/block detection, structured section extraction helper) with focused tests in `tests/parsing/test_section_detector.py`.
- [x] (2026-02-27_22.32.00) Milestone 2 completed: `cookimport/parsing/sections.py` now delegates internals to shared detector while preserving public contracts (`SectionedLines`, `SectionHeaderHit`, exported functions and key normalization semantics).
- [x] (2026-02-27_22.33.00) Milestone 3 completed: Text/Excel blob section extraction now uses shared `extract_structured_sections_from_lines(...)` with existing no-header fallback retained; ingestion tests updated for `For the X` preservation.
- [x] (2026-02-27_22.34.00) Milestone 4 completed: EPUB/PDF gained `shared_v1` extraction branches with `legacy` fallback and `run_settings`-controlled backend selection; shared path preserves component headers in sectioned ingredient/instruction lines.
- [x] (2026-02-27_22.35.00) Milestone 5 completed: added `section_detector_backend` run setting (`legacy|shared_v1`) and propagated through stage + benchmark prediction generation (`cli.py`, `run_settings_adapters.py`, `labelstudio/ingest.py`) with runConfig/report coverage.
- [x] (2026-02-27_22.37.23) Fixed Priority-2-adjacent test regressions in `tests/labelstudio/test_labelstudio_ingest_parallel.py` by updating `FakeImporter.convert(...)` doubles to accept extra kwargs (including new `run_settings`).
- [x] (2026-02-27_22.38.49) Milestone 0 evidence captured: staged baseline (`legacy`) and shared (`shared_v1`) fixture runs to `/tmp/priority2-baseline/2026-02-27_22.38.46` and `/tmp/priority2-shared/2026-02-27_22.38.49`; section artifact diff for `r0.sections.json` was empty.
- [x] (2026-02-27_22.40.27) Milestone 6 completed: docs updated (`docs/04-parsing/04-parsing_readme.md`, `docs/02-cli/02-cli_README.md`, `docs/07-bench/07-bench_README.md`, `cookimport/parsing/README.md`) and full targeted validation suite passed.
...
- Observation: Shared EPUB/PDF extraction initially collapsed `For the X` headers into the following instruction when wrapped-line merge stayed enabled.
  Evidence: `tests/ingestion/test_epub_importer.py::test_extract_fields_shared_backend_preserves_for_the_component_headers` and `tests/ingestion/test_pdf_importer.py::test_extract_fields_shared_backend_preserves_for_the_component_headers` failed until shared paths skipped `_merge_wrapped_lines`.

- Observation: Label Studio ingest tests used `FakeImporter.convert` doubles that rejected the new `run_settings` keyword.
  Evidence: `tests/labelstudio/test_labelstudio_ingest_parallel.py::test_run_labelstudio_import_emits_post_merge_progress` raised `TypeError: ... got an unexpected keyword argument 'run_settings'` until test doubles were updated to accept `**_kwargs`.

- Observation: Stage run-level `run_manifest.json` does not carry `runConfig`; run-config backend evidence is in per-source import report JSON.
  Evidence: `.../run_manifest.json` had no `runConfig` field while `sectioned_components_recipe.excel_import_report.json` included `runConfig.section_detector_backend` (`legacy` vs `shared_v1`).
...
- Decision: In shared EPUB/PDF extraction, preserve component header lines and avoid wrapped-line merge that can absorb headers into instruction text.
  Rationale: `For the X` lines are semantic section markers and must remain standalone lines for section mapping parity.
  Date/Author: 2026-02-27_22.36.00 / Codex GPT-5

- Decision: Keep Label Studio tests resilient to importer signature growth by allowing `FakeImporter.convert(...)` kwargs in ingest-parallel tests.
  Rationale: Prediction-generation wiring now passes `run_settings`; future additive kwargs should not break unrelated test intent.
  Date/Author: 2026-02-27_22.37.23 / Codex GPT-5


## Outcomes & Retrospective


Completed for Priority 2 scope.

Delivered outcomes:

- Shared deterministic section detection now exists in one module (`cookimport/parsing/section_detector.py`) and is reused by parser helpers and importer extraction paths.
- `legacy` and `shared_v1` are both available through run settings/CLI; `legacy` remains the default.
- Text/Excel/EPUB/PDF extraction paths now support consistent `For the X` behavior under `shared_v1`.
- Stage and benchmark prediction-generation run configs include `section_detector_backend` for reproducibility.
- All-method reporting surfaces `section_detector_backend` in dimensions only when non-default, without auto-growing permutations.

Validation recap:

- Focused and full Priority 2 targeted pytest suites passed, including parsing, ingestion, staging, CLI, run-settings, and Label Studio wiring tests.
- Fixture stage runs for baseline/shared backends produced identical `r0.sections.json` for `tests/fixtures/sectioned_components_recipe.txt`.
- Per-source import reports show backend-specific `runConfig.section_detector_backend` values (`legacy` vs `shared_v1`).
...
Importer extraction is now shared where Priority 2 required:

- `cookimport/plugins/text.py` and `cookimport/plugins/excel.py` both use `extract_structured_sections_from_lines(...)`.
- `cookimport/plugins/epub.py` and `cookimport/plugins/pdf.py` both support `legacy` and `shared_v1` extraction branches controlled by run settings.
- EPUB/PDF keep legacy candidate-boundary safeguards while shared extraction is enabled only when configured.
...
`RunSettings` now includes `section_detector_backend` (`legacy|shared_v1`, default `legacy`), and stage/prediction run reports include it in `runConfig`.

All-method benchmark variants are still extractor-focused by default; when backend is not `legacy`, dimensions include `section_detector_backend` without expanding the permutation matrix.
...
4. Record run-config surfaces for baseline (`legacy`) and shared (`shared_v1`) runs.
...
Executed evidence snapshot (this implementation pass):

- `pytest -q tests/labelstudio/test_labelstudio_ingest_parallel.py tests/labelstudio/test_labelstudio_benchmark_helpers.py` initially failed due stale `FakeImporter.convert` test signatures; after updating test doubles to accept `**_kwargs`, rerun passed.
- `pytest -q tests/parsing/test_section_detector.py tests/parsing/test_recipe_sections.py tests/staging/test_section_outputs.py tests/ingestion/test_text_importer.py tests/ingestion/test_excel_importer.py tests/ingestion/test_epub_importer.py tests/ingestion/test_pdf_importer.py tests/llm/test_run_settings.py tests/cli/test_toggle_editor.py tests/cli/test_cli_output_structure.py tests/labelstudio/test_labelstudio_ingest_parallel.py tests/labelstudio/test_labelstudio_benchmark_helpers.py` passed.
- `cookimport stage ... --section-detector-backend legacy` and `cookimport stage ... --section-detector-backend shared_v1` produced run folders `/tmp/priority2-baseline/2026-02-27_22.38.46` and `/tmp/priority2-shared/2026-02-27_22.38.49`; `diff -u .../r0.sections.json .../r0.sections.json` produced no output.
...
- 2026-02-27_22.40.27 (Codex GPT-5): Marked Milestones 0-6 complete; logged implementation/test discoveries (shared header merge guard, Label Studio test-double signature drift, runConfig evidence location); added concrete validation evidence and updated retrospective to final delivered state.
