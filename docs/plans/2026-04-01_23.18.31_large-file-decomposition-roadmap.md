...
- [ ] Implement the decomposition in small atomic commits, starting with the lowest-risk extractions and ending with the deepest LLM runtime coordinators.
- [ ] Update the relevant subsystem docs after each milestone if file ownership moved enough to make the current docs misleading.
...
Initial outcome: the repo now has a concrete, ranked plan for decomposing the oversized file set instead of an ad hoc “split giant files someday” intention. The main unresolved work is execution.
...
Current rough size markers that motivated this plan:
