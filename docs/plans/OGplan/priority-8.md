...
- [ ] Implement segmentation metric primitives and tests (`cookimport/bench/segmentation_metrics.py`, `tests/bench/test_segmentation_metrics.py`).
- [ ] Extend `evaluate_stage_blocks(...)` to compute additive segmentation metrics/taxonomy and emit optional boundary mismatch artifacts.
- [ ] Extend CLI eval entrypoints (starting with `cookimport bench eval-stage`) to expose boundary tolerance/projection/metric-selection knobs.
- [ ] Add optional `segeval` backend as an extra (`pyproject.toml`) and wire graceful missing-dependency behavior.
- [ ] Update bench docs and conventions to describe the new `segmentation` report contract and interpretation guidance.
- [ ] Run targeted and end-to-end validation, then record concrete evidence snippets in this plan.
...
- Plan is now aligned with current code reality and active benchmark command surfaces.
- Implementation has not started yet; this revision is planning-only.

Expected implementation outcome:

- A regression-resistant segmentation diagnostics layer on top of existing stage-block evaluation, including boundary PRF, taxonomy summaries, and optional `segeval` metrics.
...
- `evaluate_stage_blocks(...)` currently computes only block-label classification metrics and mismatch artifacts.
