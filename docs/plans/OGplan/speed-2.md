...
- [ ] Add explicit “eval phase” counting to scheduler snapshots and dashboard line.
- [ ] Implement CPU-aware default/effective eval-tail headroom (auto) while preserving explicit user overrides.
- [ ] Update smart admission logic to use eval-tail headroom to admit additional pipelines during eval tails.
- [ ] Ensure process pool sizing supports the new maximum concurrent pipelines without deadlocking.
- [ ] Add unit tests for eval-tail headroom computation and admission decisions.
- [ ] Run focused pytest suites + one end-to-end all-method smoke run; verify identical scoring outputs and improved tail utilization.
- [ ] Update internal docs (`cookimport/bench/CONVENTIONS.md` and/or `docs/07-bench/07-bench_README.md`) to reflect new defaults/telemetry.
- [ ] Write `Outcomes & Retrospective` entry comparing before/after.

## Surprises & Discoveries

(Keep this section empty until you run the baseline and begin implementation.)
...
(Write an entry at the end of each milestone and at completion. Compare observed behavior to the Purpose.)
