...
- [x] (2026-02-27 10:30Z) Implemented disk-backed alignment cache integrated into canonical evaluator (`cookimport/bench/canonical_alignment_cache.py`, `cookimport/bench/eval_canonical_text.py`).
- [x] (2026-02-27 10:31Z) Implemented lock + stale-lock + atomic-write + corruption recovery safety paths for concurrent cache access.
- [x] (2026-02-27 10:31Z) Wired all-method execution to share a per-source cache directory (`.cache/canonical_alignment`) across configs.
- [x] (2026-02-27 11:25Z) Added dedicated cache regression tests in `tests/bench/test_canonical_alignment_cache.py`, including same-key multi-process race proof and end-to-end hit/invalidation checks.
- [x] (2026-02-27 11:28Z) Captured real all-method miss→hit telemetry and wall-time evidence from a two-config sweep on `DinnerFor2CUTDOWN.epub`.
...
- Observation: In this sandbox, process-pool execution can fail with permissions and fall back to serial all-method execution, but cache reuse behavior still proves out because cache sharing is per-source, not tied to parallel scheduling.  
  Evidence: Real run output reported `Parallel executor unavailable ([Errno 13] Permission denied); falling back to serial mode.` followed by config-1 miss and config-2 hit with identical cache key.
...
- Decision: Add a dedicated cache test module (`tests/bench/test_canonical_alignment_cache.py`) rather than only extending `test_eval_stage_blocks.py`, and include a same-key multi-process race test at the cache-layer API.  
  Rationale: This restores plan/file-path traceability and directly proves duplicate-compute suppression under concurrent writers.  
  Date/Author: 2026-02-27 / GPT-5 Codex
...
- Runtime cache feature is complete and wired through all-method per-source sharing.
- Regression coverage is complete across hit-path parity, boundary-key invalidation, stale-lock/corrupt-entry recovery, and multi-process same-key contention.

Real-run evidence outcome (2026-02-27):

- Command path: internal `_run_all_method_benchmark(...)` with two identical configs over `data/input/DinnerFor2CUTDOWN.epub` + `data/golden/pulled-from-labelstudio/dinnerfor2cutdown/exports/freeform_span_labels.jsonl`.
- Config 1 telemetry:
  - `alignment_cache_hit=false`
  - `alignment_sequence_matcher_seconds=12.626098081003875`
  - `duration_seconds=23.607534372014925`
- Config 2 telemetry (same cache key `v1/legacy/n1/canon=f6414447e837/pred=afcb8dc3247f/b=0771453945fc`):
  - `alignment_cache_hit=true`
  - `alignment_sequence_matcher_seconds=0.0`
  - `duration_seconds=10.013305764994584`
- Correctness parity held across runs:
  - `overall_line_accuracy=0.20454545454545456` on both configs
  - `macro_f1_excluding_other=0.2823843188260994` on both configs

Retrospective:

- The largest gap was not core runtime logic; it was closure evidence and plan maintenance. Keeping the plan/living-doc sections synchronized with landed code prevented another audit loop.
- Multi-process proof is best validated at the cache layer, where lock/write behavior can be asserted deterministically without full benchmark-runtime nondeterminism.
...
Plan revision note (2026-02-27): Updated progress to completed state, added real-run telemetry evidence and retrospective, and recorded the dedicated test-file + concurrency-proof decision so the plan is self-contained for restart/closure review.
