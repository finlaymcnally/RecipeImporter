...
- [x] (2026-04-04 11:57 America/Toronto) Moved this roadmap into `docs/plans/` and established [docs/plans/2026-04-04_11.52.39_finish-owner-module-decomposition.md](/home/mcnal/projects/recipeimport/docs/plans/2026-04-04_11.52.39_finish-owner-module-decomposition.md) as the focused follow-on plan for the remaining transitional cleanup.
...
Latest bench-side outcome: the external-AI wrapper now also keeps its root README/flattened-summary story in an explicit owner and its heavier starter-pack writer in a second owner. `root_rendering.py` owns full cutdown README generation, flattened-copy assembly, aggregated root-summary rendering, `starter_pack_writer.py` owns the assembly root for derived starter-pack artifact writing, and `scripts/benchmark_cutdown_for_external_ai.py` has dropped further to `11841` lines.

Remaining gap: the bench and Label Studio wave plus the deep LLM/runtime decomposition wave are still pending. This plan remains the broad source of truth for those later cuts, while [docs/plans/2026-04-04_11.52.39_finish-owner-module-decomposition.md](/home/mcnal/projects/recipeimport/docs/plans/2026-04-04_11.52.39_finish-owner-module-decomposition.md) is the focused active plan for the remaining compatibility-scaffolding cleanup and final owner-root shrink work.
Current size markers after the latest wave: `prompt_artifacts.py` is now `702` lines, `prompt_budget.py` is `131`, `prelabel.py` is `427`, `cli_commands/labelstudio.py` is `3559`, `recipe_stage_shared.py` is `1647`, `knowledge_stage/workspace_run.py` is `1918`, `knowledge_stage/recovery.py` is `2092`, `codex_exec_runner.py` is `5089`, and `bench_all_method.py` is `5442`.
