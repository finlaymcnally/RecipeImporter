...
- [ ] Implement consolidation pass in `cookimport/parsing/chunks.py` and thread it into `process_blocks_to_chunks`.
- [ ] Add focused unit tests covering safe merges and “do not merge” boundaries.
- [ ] Update `docs/04-parsing/04-parsing_readme.md` to reflect the new pipeline step and its rules.
- [ ] Run focused and full test suites and capture evidence snippets in `Artifacts and Notes`.

## Surprises & Discoveries

- Observation: (none yet; to be filled during implementation)
  Evidence: (add a short pytest snippet or a before/after `chunks.md` excerpt)
...
(To be filled after at least one milestone lands.)
...
3) the merged chunk stays within a safe size bound.
...
Tests should be placed alongside existing chunking tests, typically `tests/test_chunks.py` (as referenced by docs).
...
3) Add tests:
    ...
    rg -n "test_chunks|process_blocks_to_chunks" tests/test_chunks.py
...
(As you implement, paste small evidence snippets here.)

- Example pytest evidence:
...
- Example before/after excerpt to capture once you find a real book that demonstrates the issue:

    BEFORE (chunks.md):
      Chunk 12 (knowledge) Heading: Knife Skills
      [prose...]

      Chunk 13 (knowledge) Heading: Knife Skills
      [list...]
...
- `def should_merge_adjacent_chunks(a: Chunk, b: Chunk, *, max_merged_chars: int) -> bool:`
...
- `def chunk_abs_range(chunk: Chunk) -> tuple[int, int]:`
