---
summary: "ExecPlan for optional codex-farm pass5 tag suggestions with stage integration."
read_when:
  - "When implementing or maintaining pass5 recipe tag suggestions"
  - "When reconciling deterministic tagging with optional codex-farm tagging"
---
...
- [x] (2026-02-25 16:24Z) Read current tagging + codex-farm boundaries, including `I4.1-Auto-tag.md` clarifications and stage/CLI run-settings contracts.
- [x] (2026-02-25 16:29Z) Finalized tag output contract: keep `*.tags.json` naming, add `source`, `llm_pipeline_id`, `new_tag_proposals`, run-level/source counts.
- [x] (2026-02-25 16:35Z) Added pass5 pipeline assets: `recipe.tags.v1` pipeline JSON, prompt template, strict output schema.
- [x] (2026-02-25 16:48Z) Implemented batchable codex-farm provider (`cookimport/tagging/codex_farm_tags_provider.py`) with strict catalog+shortlist validation and pass5 raw manifest output.
- [x] (2026-02-25 16:54Z) Wired `llm_second_pass.py` to real provider with fallback/fail semantics and shortlist fallback behavior.
- [x] (2026-02-25 17:07Z) Added shared tagging orchestrator (`cookimport/tagging/orchestrator.py`) for CLI draft-folder runs and stage pass integration.
- [x] (2026-02-25 17:16Z) Integrated optional stage pass via `--llm-tags-pipeline` + run settings (`llm_tags_pipeline`, `codex_farm_pipeline_pass5_tags`, `tag_catalog_json`) and manifest artifact indexing.
- [x] (2026-02-25 17:20Z) Extended deterministic tests/fake-runner coverage for pass5 assets/provider/fallback/stage artifact wiring.
- [x] (2026-02-25 17:24Z) Ran targeted pytest slices for tagging/LLM/CLI/staging parity and verified green.

## Surprises & Discoveries

- Observation: `llm_second_pass.py` was still scaffold-only; existing `--llm` behavior in tagging CLI silently mocked/no-op and needed full provider wiring.
  Evidence: Original module had `_check_provider_configured()` hardcoded `False` and `_mock_llm_call()` returning `[]`.

- Observation: Stage integration was cleanest as a run-level post-write pass over `final drafts/`, not per-worker, because split/single flows already converge there.
  Evidence: Both single-file and split-merge flows finish by writing `final drafts/<workbook_slug>/` and then stage writes one run manifest.

- Observation: `I4.1-Auto-tag.md` still had high-value policy/risk guidance (catalog-keyed rules, insert-only DB safety, precision-first) that remained valid for pass5 behavior and docs.
  Evidence: Decision/policy sections in `I4.1-Auto-tag.md` matched current code constraints and were merged into updated docs + this plan.

- Observation: Default pytest output suppression in this repo can hide pass counts in normal `-q` runs.
  Evidence: `COOKIMPORT_PYTEST_VERBOSE_OUTPUT=1 pytest -o addopts='' ... -q` reported `45 passed, 2 warnings in 4.15s`.
...
- Decision: Implement tags as a separate codex-farm “pass” (`recipe.tags.v1`) rather than expanding `recipe.final.v1` output schema.
  ...
  Date/Author: 2026-02-25 / Codex (GPT-5)
...
- Decision: Preserve `*.tags.json` + `tagging_report.json` artifact naming and extend shape additively (`source`, `llm_pipeline_id`, `new_tag_proposals`) instead of introducing a parallel filename family.
  Rationale: Keeps existing consumer paths stable while making LLM provenance explicit.
  Date/Author: 2026-02-25 / Codex (GPT-5)

- Decision: Use run-level stage pass (`run_stage_tagging_pass`) gated by `llm_tags_pipeline`, writing to `tags/` + `raw/llm/.../pass5_tags`.
  Rationale: Avoids touching split-worker protocols and reuses existing deterministic stage outputs as pass5 inputs.
  Date/Author: 2026-02-25 / Codex (GPT-5)

## Outcomes & Retrospective

- Delivered pass5 tagging end-to-end with strict validation, fake-runner-backed tests, and stage/CLI wiring while keeping default behavior deterministic and off-by-default.
- Merged prior-plan context from `I4.1-Auto-tag.md`: catalog-key stability, precision-first policy, insert-only DB safety, and explicit optional LLM lane boundaries.
- Targeted validation slices are green (`45 passed`): tagging provider/orchestrator tests, LLM pack asset tests, run settings tests, CLI flag passthrough tests, and stage/pred manifest parity tests.
...
- 2026-02-25: Updated this ExecPlan from design-only to implementation-tracking status, added concrete decisions/evidence, and merged durable clarifications from `docs/plans/I4.1-Auto-tag.md` so this file can stand alone for future maintenance/restart.
