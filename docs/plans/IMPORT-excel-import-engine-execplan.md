...
- [ ] Optimization: Implement streaming (openpyxl `read_only=True`) for very large workbooks to reduce memory usage.
- [ ] Enhancement: Implement LLM repair for "Layout C" (messy tall tables) where heuristics fail to group rows.
- [ ] Enhancement: Implement LLM repair for cells where ingredients and instructions are smashed together (e.g., using `structure_repair` placeholder).
