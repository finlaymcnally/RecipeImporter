...
- `run_settings_contracts.py`: contract names, projection logic, summarized projection logic, and functions that define which names belong to which contract.
...
At the end of this milestone, `cookimport/config/run_settings_contracts.py` will exist and own the contract constants, projection helper, summary helper, and contract-name grouping helpers. `run_settings.py` may re-export them temporarily, but the new module becomes the canonical home.
...
At the end of this milestone, major runtime callers such as CLI stage paths, Label Studio benchmark/import paths, speed runner, quality runner, and interactive flows should use the authoritative contract helpers. Temporary compatibility shims may remain, but new logic should not be built on ad hoc projection choices.
...
Start by creating `cookimport/config/run_settings_contracts.py` and moving the contract constants and helpers there. The minimum set is `RUN_SETTING_CONTRACT_*`, `_normalize_run_setting_contract(...)`, `_run_setting_names_for_contract(...)`, `project_run_config_payload(...)`, `summarize_run_config_payload(...)`, and the specific grouping functions such as `product_run_setting_names()`. Keep re-export shims in `run_settings.py` during migration so existing imports do not all have to change in one patch.

Once the contracts module exists, update `run_settings_adapters.py` to import from it directly. Then audit the main call sites. The highest-value ones are the CLI stage and benchmark paths, Label Studio paths, speed runner, quality runner, and interactive settings flow. Move them so they consistently use the contract and adapter helpers rather than assembling runtime payloads in their own local way.
...
Add narrow tests as soon as contract logic moves. Test the projection boundaries directly and test the adapter outputs directly. Then update docs so the new module layout becomes discoverable.
...
The first acceptance criterion is authority. A contributor should be able to find contract names, projection helpers, and runtime call assembly in the new dedicated modules without treating `RunSettings` as the only way into the config story.

The second acceptance criterion is caller behavior. Major runtime callers should build their kwargs through the authoritative contract and adapter helpers, not through open-coded field selection.

The third acceptance criterion is narrow proof. Tests must show that product, operator, benchmark-lab, and internal surfaces remain stable and that stage and benchmark adapters include the expected fields and omit accidental extras.
...
In `cookimport/config/run_settings_contracts.py`, define and keep authoritative:
    ...
    def project_run_config_payload(...) -> dict[str, Any]: ...
    def summarize_run_config_payload(...) -> str: ...
...
`run_settings.py` should remain the home of the full `RunSettings` model and validation logic, plus temporary compatibility re-exports while migration proceeds.

## Revision note

Created on 2026-03-22 as one of three standalone child ExecPlans replacing the earlier umbrella AI-readiness refactor plan. This file owns only run-settings contract layering and runtime call adapter authority.
