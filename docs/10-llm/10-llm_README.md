...
- the shared worker instructions now explicitly steer agents away from broad queue dumps and ad hoc shell schedulers. The preferred pattern is: open the repo-named file directly, run one small local helper if needed, and write the final answer only to the approved `out/` path.
...
- Recipe worker cost regressions are usually readback regressions now, not watchdog regressions. If recipe token spend spikes again, inspect whether workers have slipped back into raw `hint_path` / `input_path` rereads or wider-than-needed workspace exploration before changing retry policy or prompt wording.
