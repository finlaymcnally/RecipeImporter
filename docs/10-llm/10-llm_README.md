...
- the knowledge stage now has one live authority path when enabled: first-pass classification returns `knowledge`, `proposal_candidate`, or `other`; then the second pass always runs grouping plus proposal approval/rejection together, so `proposed_tags` only appear after approved proposal review, never in first-pass classification
...
- `_apply_line_role_semantic_guard(...)` is currently a metadata/no-op seam; it preserves proposal validity/status unchanged and does not reject, demote, replace, or remap model rows.
...
- line-role shard validation is still structurally strict, but the old semantic guard is no longer an active rejection layer. If an edited shard fails today, it is because coverage/shape/repair validation did not produce a valid owned-row ledger, not because a separate semantic guard overruled a structurally valid answer.
