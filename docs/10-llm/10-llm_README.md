...
The worker-visible self-help surface is now explicit and wrapper-first. `task.json` carries repo-authored `helper_commands`, `next_action`, `workflow`, and `answer_schema` metadata. General single-file stages use wrapper commands such as `task-summary`, `task-show-unit`, `task-show-unanswered`, `task-template`, and `task-apply`, while knowledge classification uses the queue-style `task-show-current`, `task-show-neighbors`, `task-answer-current`, and `task-next` surface instead of batch answer files. The stage-neutral `task-status`, `task-doctor`, and `task-handoff` wrappers dispatch to the correct same-session helper from the workspace root, auto-discover hidden state under `./_repo_control/`, and preserve the same guidance plus stage-specific top-level metadata if they rewrite `task.json` into repair mode.
...
- recipe, knowledge, and line-role now allow one bounded fresh-session recovery attempt on the happy path when the first session exits cleanly, leaves recoverable repo-owned worker state behind, and did not hit a hard watchdog boundary. For canonical line-role that recoverable state can now include repo-owned batch `answers.json` work in addition to edited `task.json` plus same-session state. That restart is stage-owned recovery, not a watchdog policy change.
...
- knowledge grouping task files are now bounded deterministic batches rather than one flat all-kept-rows file: deterministic code splits large grouping work by row/evidence budget, writes top-level `grouping_batch` progress metadata into `task.json`, and keeps the worker on the same helper-first `knowledge_group` handoff until the final batch completes
...
- classification `task.json` now also carries a repo-written `review_contract`, and `task-summary` surfaces that contract compactly before the worker opens raw rows. The intended mindset is close semantic review of each owned block, not inventing a packet-wide heuristic or shell-script classifier; classification rejects `task-template`, `task-apply`, `answers.json`, and scripted `task.json` synthesis so the happy path stays queue-style
