...
Line-role inline validation is now strict labels-only: live inline turns must return one ordered top-level `labels` array with no extra keys. Deterministic code zips `labels[i]` back onto the shard's true `atomic_index` order, validates count and label legality, narrows repair packets to only unresolved owned rows, allows up to three bounded shard-local repair followups using the newest unresolved ledger state each time, and fails closed only after those repair attempts still leave unresolved rows. It still fails closed if the model returns any row-shaped fallback instead of the labels array.
