...
For inline-JSON line-role, the visible prompt is transport-thin and evidence-thin: the prompt now derives from the shared `line-role.shared-contract.v1.md` contract, shows owned rows as compact provenance strings such as `r01 | 210 | Bright Cabbage Slaw` rather than visible `atomic_index`, and asks the turn output for one ordered `labels` array. The watchdog retry prompt now reuses that same compact row rendering instead of falling back to row-object examples. Deterministic code remains the owner of shard order, label-to-row projection, safety-boundary checks, repair routing, and final incompleteness handling; it does not add shard-profile semantics or silently replace unresolved rows with deterministic baseline guesses.
...
Line-role inline validation is now strict labels-only: live inline turns must return one ordered top-level `labels` array with no extra keys. The initial packet gets a packet-local output schema with an exact `labels` length so short arrays can be rejected before promotion rather than only in later repo validation. Deterministic code zips `labels[i]` back onto the shard's true `atomic_index` order, validates count and label legality, narrows repair packets to only unresolved owned rows, allows up to three bounded shard-local repair followups using the newest unresolved ledger state each time, and fails closed only after those repair attempts still leave unresolved rows. It still fails closed if the model returns any row-shaped fallback instead of the labels array.
Line-role semantic validation is not a second-pass semantic veto layer on that inline path. After structural validation succeeds, deterministic code does not reopen rows just because repo heuristics disagree with the labels. Repair is reserved for structural or contract failures such as malformed output, missing rows, duplicate rows, wrong label count, or illegal labels.
