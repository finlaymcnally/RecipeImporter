...
- knowledge worker assignments now launch one long-lived workspace-worker Codex session per worker assignment with shared `assigned_shards.json`, a skinny worker-local `assigned_tasks.json`, repo-written `current_batch.json`, `CURRENT_BATCH.md`, `CURRENT_BATCH_FEEDBACK.md`, compatibility `current_task.json` / `CURRENT_TASK*` fallback files for the first active task, authoritative `in/*.json`, harvested `out/*.json`, `scratch/`, `OUTPUT_CONTRACT.md`, `examples/`, and `tools/knowledge_worker.py`; the worker starts from the batch bundle, gets a representative good-vs-bad snippet example instead of a first-task front-matter example, uses the repo-written helper verbs `current-batch|complete-batch|check-batch|install-batch|next-batch|show-batch|explain-failure` (with the older single-task verbs and lower-level `overview|show|scaffold|check|install` still mirrored for recovery/debugging), and the repo advances the queue by contiguous valid prefix while also mirroring the advanced batch bundle back into the live execution workspace so the same worker session can continue with the next batch immediately. `CURRENT_BATCH.md` is now intentionally the cheap first read, `current_batch.json` keeps only batch-local task facts, `assigned_tasks.json` is fallback/debug inventory only, and strong-cue scaffolds are visibly marked `strong_cue_review_required` instead of masquerading as accepted reviewed-empty answers. The prompt, `CURRENT_BATCH.md`, `CURRENT_BATCH_FEEDBACK.md`, and `install-batch` stdout now all frame that handoff as one continuing assignment: do not ask permission mid-queue, and do not treat a successful install as a stopping point while later tasks remain.
- the remaining runtime guardrail is now explicit too: the knowledge main-worker watchdog kills batch-bypass behavior such as direct `assigned_tasks.json` dumps, helper-source spelunking against `tools/knowledge_worker.py`, `install-current`-style single-task detours while a batch is active, and ad hoc queue-control rewrites. The batch contract is not just prompt text anymore; it is enforced at runtime.
