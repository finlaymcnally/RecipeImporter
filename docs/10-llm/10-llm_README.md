...
- the knowledge stage now has one live authority path when enabled: first-pass classification returns `knowledge`, `proposal_candidate`, or `other`; then the second pass always runs grouping plus proposal approval/rejection together. `proposed_tags` still never appear in first-pass classification, but grouping may now approve one new tag for either a kept `knowledge` row or a `proposal_candidate` row when the existing catalog is a bad fit
...
- in grouping, kept `knowledge` rows normally use `proposal_decision=not_applicable`, but they may also approve exactly one new proposed tag when the row is keepable and the existing catalog remains a bad semantic fit.
