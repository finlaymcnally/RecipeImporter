...
- `prompts/activity_trace_summary.jsonl` and `prompts/activity_trace_summary.md` summarize activity-trace coverage and the visible command/message/reasoning surface from the merged prompt log
- legacy `prompts/thinking_trace_summary.jsonl` and `prompts/thinking_trace_summary.md` remain as compatibility mirrors during rollout, but they now point at the same reviewer-facing activity-trace export rather than a hidden-reasoning concept
