...
Line-role semantic validation is no longer a no-op on that inline path. After structural validation succeeds, deterministic code now checks only narrow recipe-local contradiction families such as fresh title resets, strict yield anchors, obvious ingredient anchors, and similarly strong local instruction/note/variant mismatches. When one of those triggers, repo code removes only the suspicious rows from the accepted ledger, records `semantic_diagnostics`, adds `semantic_invariant_violation:*` errors, and sends just those rows back through bounded repair with the previous failed row labels plus the rejection reasons. Repair sees failed-output evidence, not repo-authored label suggestions.
...
For inline-JSON knowledge, deterministic code now allows a small bounded series of shard-local repair followups for both classification and grouping before a shard is marked invalid. Those followups reuse the prepared execution workspace for stable local paths, but they no longer resume the previous Codex transcript, which keeps repair/grouping calls bounded instead of replaying the whole shard conversation. Classification and grouping packets now also get packet-local output schemas with exact `rows` lengths, so short replies can fail at the transport boundary before repo validation. Grouping-response parsing still accepts `group_index` as a worker alias for `group_key`, and the structured response contract now works on packet-local `row_id` values instead of visible global block indices: ordered rows stay in reading order, the direct prompt and structured packets render owned evidence as compact provenance strings such as `r01 | 21 | Acid brightens rich dishes.`, grouping adds compact factual `row_facts` instead of verbose row objects, nearby context stays compact and only appears when present, classification carries the checked-in ontology rather than a repo-generated candidate-tag shortlist, and the parser reports missing, duplicate, and unknown row ids explicitly. Untouched classification rows still fail as `knowledge_block_missing_decision` rather than generic `invalid_category`.
Knowledge grouping no longer treats the configured unit and evidence caps as metadata-only. The planner now partitions retained knowledge rows into real grouping batches, keeps validated answers across attempts, narrows grouping repair packets to only unresolved rows, and the same-session taskfile path can advance across multiple grouping batches instead of assuming one shard-sized grouping turn forever.
...
Knowledge keep/drop policy is now aligned across both non-recipe transports: inline JSON and taskfile workers should both treat retrieval-grade reusable cooking guidance as `knowledge`, while memoir/story framing, praise/foreword/thesis prose, and unsupported heading-only blocks stay `other`.
...
- the knowledge stage now has one live authority path when enabled: first-pass classification returns `knowledge`, `proposal_candidate`, or `other`; then the second pass always runs grouping plus proposal approval/rejection together. `proposed_tags` still never appear in first-pass classification, but grouping may now approve one or more new tags for either a kept `knowledge` row or a `proposal_candidate` row when the existing catalog is a bad fit
...
- `_apply_line_role_semantic_guard(...)` now rejects only narrow recipe-local contradiction rows after structural validation. It does not rewrite labels; instead it prunes suspicious rows from `accepted_rows`, records `semantic_diagnostics` plus repair hints, emits `semantic_invariant_violation:*` validation errors, and forces those rows through bounded repair.
...
- line-role shard validation is still structurally strict, and the semantic guard is now a narrow active rejection layer after structural validation. If an edited shard fails with `semantic_invariant_violation:*`, repo code found a strong recipe-local contradiction and re-opened only those suspicious rows for repair; it still does not rewrite the labels deterministically.
