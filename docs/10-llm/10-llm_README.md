...
For inline-JSON knowledge, deterministic code now allows a small bounded series of same-session repair followups for both classification and grouping before a shard is marked invalid. Grouping-response parsing also accepts `group_index` as a worker alias for `group_key`; deterministic validation still owns the final non-empty grouping contract.
