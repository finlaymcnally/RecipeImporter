...
- knowledge worker hints now mirror line-role more closely: `hints/<shard_id>.md` summarizes shard profile, default posture, decision policy, example-file pointers, and attention rows so workers can calibrate before opening the authoritative `in/<shard_id>.json`
