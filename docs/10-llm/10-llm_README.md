...
- knowledge now uses the packet contract end to end. `codex_farm_knowledge_jobs.py` writes one ordered packet payload per task under `knowledge/in/*.json`, each task owns packet-local `block_decisions` plus packet-local `idea_groups`, and the validator requires exact block coverage plus exact knowledge-block-to-group coverage.
...
- `codex_farm_knowledge_jobs.py` now writes one ordered packet payload per task under `knowledge/in/*.json`. Hard packet caps define a packet floor, so a requested `knowledge_prompt_target_count` can lower worker pressure but cannot collapse multiple packets into one lossy task.
...
- knowledge runtime now also writes `task_manifest.jsonl` plus worker-local `assigned_tasks.json`; those task rows match `knowledge/in/*.json` one-for-one because each row owns one planned packet and one result path
