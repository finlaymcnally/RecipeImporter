...
- workspace-worker sessions now also get a repo-written `worker_manifest.json` in the worker root and mirrored sterile workspace. The live semantic contract is `task.json`: open it, edit only answer fields in place, and stop. `worker_manifest.json`, `assigned_shards.json`, `hints/*.md`, `in/*.json`, and similar files are now auxiliary repo-owned context or observability surfaces, not the happy-path output language.
...
7. The knowledge Codex surface finalizes the non-recipe side. The program partitions the category-neutral `nonrecipe-route` candidate queue into worker assignments, writes one repo-owned `task.json` per worker session, and keeps one long-lived worker session on that immutable assignment. The worker makes the `knowledge|other` and grouping calls by editing answer fields inside that file. Repo code validates exact owned block coverage plus kept-block group coverage, promotes only accepted outputs, and uses one explicit repair pass only when deterministic validation rejects the edited answers.
...
- recipe worker roots now also carry `worker_manifest.json`, and the shared worker prompt tells the worker to open `task.json` first and treat the other mirrored files as repo-owned context rather than the main output language
...
- completed knowledge workspace workers should now always carry a non-null `reason_code` in `live_status.json`; successful task-file workers finish as ordinary completed sessions once every declared shard output validates, untouched task files fail closed as `process_exited_without_final_packet_state`, invalid outputs can roll into `repair_packet_exhausted` after one repair pass, and true watchdog kills keep their original watchdog reason codes
...
- line-role worker assignments now launch one long-lived workspace-worker Codex session per worker assignment with repo-written `task.json`, shared `prompt.txt`, worker-local `hints/*.md`, authoritative `in/*.json`, richer `debug/*.json`, and repo-written `out/*.json`; the happy path no longer uses `CURRENT_PHASE*`, `work/*.json`, or same-session install/check loops
...
- line-role worker roots now also carry `worker_manifest.json` and `OUTPUT_CONTRACT.md`. The shared worker prompt frames deterministic labels as weak hints rather than starting truth or tie-breakers, tells the model to read `task.json` first, and treats the mirrored shard-local files plus `OUTPUT_CONTRACT.md` as fallback/debug context rather than startup requirements.
