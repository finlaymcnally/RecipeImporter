...
- that workspace-worker policy is now intentionally boundary-based rather than shell-shape allowlisted: main sessions are killed for real workspace escapes, forbidden executables, or pathological no-output loops, not for ordinary local shell syntax by itself.
