...
- prompt-planning field names are still `recipe_prompt_target_count`, `knowledge_prompt_target_count`, and `line_role_prompt_target_count`; recipe and line-role still use them as direct shard-count overrides, but knowledge now keeps the compatibility field name only and records a planning warning instead of rebundling because the live knowledge planner emits one shard per deterministic chunk
...
- `knowledge_prompt_target_count` and `knowledge_shard_target_chunks` are now compatibility-only knobs on the live knowledge planner. If either is set, `build_knowledge_jobs(...)` still writes one shard per deterministic chunk and records a planning warning that the knob was ignored.
