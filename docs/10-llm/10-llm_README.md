...
- Pass 1 work files are seeded as row-ledger files, not final-output payloads, and `tools/knowledge_worker.py` now auto-migrates the short-lived bad `packet_id/block_decisions/idea_groups` seed shape if an older workspace root still has it
...
- knowledge worker hints now mirror line-role more closely: `hints/<shard_id>.md` summarizes shard profile, default posture, decision policy, example-file pointers, and attention rows so workers can calibrate before reopening the authoritative `in/<shard_id>.json`
...
- line-role worker roots now also carry `worker_manifest.json` plus repo-written `examples/*.md` contrast files. The shared worker prompt now frames deterministic labels as weak hints rather than starting truth or tie-breakers, tells the model to read the shard-local hint/input files first, and uses the examples only for calibration.
