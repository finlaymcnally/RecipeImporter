...
For inline-JSON line-role, the visible prompt is still transport-thin and semantic-hint-thin, but it is now more row-grounded than the earlier labels-only version: the prompt derives from the shared `line-role.shared-contract.v1.md` contract, shows owned rows as ordered row objects with packet-local `row_id`, visible `block_index`, and raw `text`, keeps nearby context as reference-only row objects, and asks the turn output for one ordered `rows` array whose items each claim a `row_id` plus `label`. The watchdog retry prompt reuses that same row-object rendering. Deterministic code remains the owner of shard order, `row_id`-to-atomic-index projection, safety-boundary checks, repair routing, and final incompleteness handling; it does not add shard-profile semantics or silently replace unresolved rows with deterministic baseline guesses.
...
Line-role inline validation is now strict row-grounded JSON: live inline turns must return one ordered top-level `rows` array with no extra keys, and each answer row must claim a packet-local `row_id` plus `label`. The initial packet gets a packet-local output schema with an exact `rows` length so missing or extra answer rows can be rejected before promotion rather than only in later repo validation. Deterministic code maps the claimed `row_id` values back onto the shard's true `atomic_index` order, validates row coverage and label legality, narrows repair packets to only unresolved owned rows, allows up to three bounded shard-local repair followups using the newest unresolved ledger state each time, and fails closed only after those repair attempts still leave unresolved rows. It still fails closed if the model returns row answers without usable `row_id` grounding.
...
Line-role repair followups reuse the prepared execution workspace and resume the prior Codex transcript so the model keeps the broader shard conversation while answering the narrowed unresolved-row packet. Because `codex exec resume` does not expose `--output-schema`, repair still relies on repo validation for exact-count enforcement after the resumed turn returns.
