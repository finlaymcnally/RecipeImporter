...
Knowledge keep/drop policy is now aligned across both non-recipe transports: inline JSON and taskfile workers should both use a binary first pass of `keep_for_review` versus `other`, with all tag thinking deferred to the second-pass grouping step. Memoir/story framing, praise/foreword/thesis prose, and unsupported heading-only blocks should stay `other`. Short headers can still survive that pass when they are the actual concept or strategy name that makes the following explanatory row materially clearer, rather than just decorative topic scaffolding.
