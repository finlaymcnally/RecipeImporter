...
- current shared policy matrix:
  ...
  - line-role `inline-json-v1`: `structured_repair_followup=1`
  - knowledge `taskfile-v1`: worker-assignment `fresh_session_retry=1`, `fresh_worker_replacement=1`; semantic-step `same_session_repair_rewrite=1` for both `nonrecipe_classify` and `knowledge_group`
  - knowledge `inline-json-v1`: semantic-step `structured_repair_followup=3` for both `nonrecipe_classify` and `knowledge_group`
