...
- repo-owned knowledge validation/reporting metadata is row-native now (`owned_row_indices`, `unresolved_row_indices`, `missing_owned_row_indices`, `knowledge_rows_missing_group`, and related summary keys), while compact worker payload aliases such as `block_decisions` and `idea_groups[*].block_indices` still remain as the persisted packet/provenance surface for now
...
- those `workers/*/in/*.json` files are now compact row ledgers: `{"v":2,"shard_id":...,"context_before_rows":[[atomic_index,block_index,current_line], ...],"rows":[[atomic_index,block_index,current_line], ...],"context_after_rows":[[atomic_index,block_index,current_line], ...]}`. The boundary-context keys are optional and appear only when immediate neighbors exist. The model-facing file stays compact row-wise and carries only direct row evidence plus immediate neighboring context.
...
- Prompt preview labeled-line reconstruction is row-native only. When preview code rebuilds line-role candidates from `labeled_lines.jsonl`, those rows must carry `source_block_index`; prompt preview no longer falls back to semantic `block_index` on that row-facing seam.
...
- Current line-role transport is intentionally split three ways:
  ...
  The billed payload is now compact provenance-first: owned rows are `[atomic_index, block_index, current_line]` plus optional `context_before_rows` / `context_after_rows` in that same tuple form, while the debug copy stays repo-owned.
