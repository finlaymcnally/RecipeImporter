...
- for knowledge planning specifically, the raw budget-native packet split is still recorded for review, but interactive/runtime warnings now defer to survivability once the requested shard count already meets `minimum_safe_shard_count`; oversized packet-budget estimates become context instead of a second warning
