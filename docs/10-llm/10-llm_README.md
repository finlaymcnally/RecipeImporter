...
LLM usage in this repo is optional. The direct-exec transport is now mixed by both attempt type and configured Codex style. `recipe_codex_exec_style=inline-json-v1`, `line_role_codex_exec_style=inline-json-v1`, and `knowledge_codex_exec_style=inline-json-v1` are the default thin paths. Recipe `taskfile-v1` remains available as an explicit fallback for comparison or debugging, but the normal recipe happy path is now one structured shard response with shell disabled. Deterministic repo code still owns validation, repair routing, grounding normalization/quality reporting, and final artifact expansion; line-role semantic validation now challenges suspicious rows through repair instead of overwriting them.

For inline-JSON line-role, the visible prompt is still transport-thin and semantic-hint-thin, but it now preserves source continuity more directly than the earlier flat row-object packet. The prompt derives from the shared `line-role.shared-contract.v1.md` contract, shows evidence as an ordered top-level `blocks` array, and each block carries compact row tuples such as `["r01", "Bright Cabbage Slaw"]` for owned rows and `["ctx", "Variations"]` for reference-only rows from the same local block. The turn output still uses one ordered top-level `rows` array whose items each claim a `row_id` plus `label`, and the watchdog retry prompt still reuses that same row-grounded answer contract. Deterministic code remains the owner of shard order, `row_id`-to-atomic-index projection, safety-boundary checks, repair routing, and final incompleteness handling; it does not add shard-profile semantics or silently replace unresolved rows with deterministic baseline guesses.
...
Line-role inline validation is now strict row-grounded JSON: live inline turns must return one ordered top-level `rows` array with no extra keys, and each answer row must claim a packet-local `row_id` plus `label`. The initial packet gets a packet-local output schema with an exact `rows` length so missing or extra answer rows can be rejected before promotion rather than only in later repo validation. Deterministic code maps the claimed `row_id` values back onto the shard's true `atomic_index` order, validates row coverage and label legality against the owned-row set, allows up to three bounded shard-local repair followups using the newest unresolved ledger state each time, and fails closed only after those repair attempts still leave unresolved rows. It still fails closed if the model returns row answers without usable `row_id` grounding.
Line-role semantic validation is not a second-pass semantic veto layer on that inline path. After structural validation succeeds, deterministic code does not reopen rows just because repo heuristics disagree with the labels. Repair is reserved for structural or contract failures such as malformed output, missing rows, duplicate rows, unknown `row_id`s, or illegal labels.
Line-role repair followups reuse the prepared execution workspace and resume the prior Codex transcript so the model keeps the broader shard conversation while answering the narrowed unresolved-row packet. Those repair packets now keep whole source blocks containing the unresolved rows, with accepted sibling rows rendered back to the model as `ctx` evidence while repo validation still scores only the unresolved owned rows. Because `codex exec resume` does not expose `--output-schema`, repair still relies on repo validation for exact-count enforcement after the resumed turn returns.
Benchmark prompt artifacts now expand line-role structured sessions turn-by-turn, including repair followups, so `full_prompt_log.jsonl` and `prompt_line_role.txt` reflect actual line-role call volume instead of only the saved initial prompt files.

For inline-JSON knowledge, deterministic code now allows a small bounded series of shard-local repair followups for both classification and grouping before a shard is marked invalid. Those followups always reuse the prepared execution workspace for stable local paths, and the transcript policy is now explicit through `knowledge_inline_repair_transcript_mode`: `resume` is the default and continues the prior Codex session, while `fresh` keeps repair as a bounded new call. Knowledge inline-json now relies on repo validation and repair rather than transport-enforced output schemas. The structured response contract works on packet-local `row_id` values instead of visible global block indices: ordered rows stay in reading order, the direct prompt and structured packets render owned evidence as compact provenance strings such as `r01 | 21 | Acid brightens rich dishes.`, grouping adds compact factual `row_facts` instead of verbose row objects, repair packets also preserve row-local `previous_answer` plus `validation_feedback` and packet-level `repair_validation_summary`, nearby context stays compact and only appears when present, pass 1 now asks for an ordered `rows` array of `{row_id, category}` decisions using `keep_for_review` versus `other`, and pass 2 now returns a top-level `groups` array whose items claim contiguous `start_row_id`/`end_row_id` spans, `topic_label`, and one shared `grounding` story. Missing classification decisions now narrow repair through unresolved `row_id`s in that `rows` array rather than through any legacy parallel output shape. Grouping packets and same-session task files may expose one mixed reading-order surface, `ordered_rows`, where plain `rXX | ...` rows are groupable by default and `ctxXX | ng | ...` rows are context-only; `ng` means do not group. The model is asked to choose the split with the tag story in mind, and repo validation rejects missing, duplicate, unknown, overlapping, and non-contiguous group coverage before projecting accepted group grounding back onto row-level outputs. Grouping-response parsing still accepts `group_index` as a worker alias for `group_id`. Untouched classification rows still fail as `knowledge_row_missing_decision` rather than generic `invalid_category`.
Knowledge grouping no longer treats the configured unit and evidence caps as metadata-only. The planner now partitions retained knowledge rows into real grouping batches, keeps validated answers across attempts, narrows grouping repair packets to only unresolved rows, and the same-session taskfile path can advance across multiple grouping batches instead of assuming one shard-sized grouping turn forever. Inline pass 2 now proposes contiguous split-and-tag groups directly, and the same-session grouping taskfile now mirrors that worker-visible contract by exposing one batch-level `groups` answer over ordered `row_id`s before handoff expands accepted groups back into the internal row-keyed answer map. Repo-owned finalization still preserves contiguous runs even when the same topic/grounding story reappears later in the shard. Batch-local ids like `g01` are now treated as opaque structural labels during repair merge: when a repair subset reuses an id that is already reserved by a different accepted story elsewhere in the same batch, repo code renumbers the merged answers onto collision-safe contiguous ids before full-batch validation. That keeps valid repairs from bouncing on fake `knowledge_group_mixed_tag_story` conflicts without changing the underlying topic label or grounding story. The grouping parser/validator also now salvages a narrow family of obvious proposed-tag shape mistakes before failing closed: it will pull `why_no_existing_tag` and `retrieval_query` up from the wrong nested location when the intent is unambiguous, derive a missing `display_name` from the local group topic or proposed key when that is the only obvious source, and lift accidentally nested `group_id` or `topic_label` fields back to the group object. Grouping repair packets now carry both the latest validator summary and a preserved first-failure `repair_root_cause_summary`, plus `previous_groups` summaries for the failing rows; those `previous_groups` now come from the latest parsed grouping attempt even when that attempt still failed semantic validation, so later retries stay focused on patching the broken groups instead of drifting into full regrouping. If all batch-local grouping turns validate but the merged inline shard still fails final packet validation, the runtime now spends one extra whole-shard grouping repair followup with full shard row context before failing closed.
Knowledge proposal wrappers are expected to round-trip as single-packet outputs during finalization. Current reread logic will recover the packet id from the wrapper `shard_id` for legacy same-session payloads that accidentally omitted `packet_id`/`bid`, but new validated wrappers should still carry their own packet id directly.

Knowledge keep/drop policy is now aligned across both non-recipe transports: inline JSON and taskfile workers should both use a binary first pass of `keep_for_review` versus `other`, with all tag thinking deferred to the second-pass grouping step. Memoir/story framing, praise/foreword/thesis prose, and heading rows should stay `other` in that first pass, even when a heading names the nearby concept clearly. Pass 2 can still see short heading-like `other` rows as context-only `ordered_rows` entries when they are local to the kept knowledge run, so preserving structure no longer requires promoting headings into `keep_for_review`.
The classification prompts now also carry explicit contrast examples for the failure shapes seen in benchmark run `2026-04-14_17.18.19`: broad book-philosophy lines, reaction/framing lines, and memoir/travel lines stay `other`, while a direct standalone lesson such as `Taste constantly as you cook, and adjust seasoning before serving.` can still survive as `keep_for_review`.

The worker-visible self-help surface is now intentionally split, but all current single-file task files are slimmer than the older helper-protocol versions. Direct-batch task files (`recipe_refine`, `line_role`, `nonrecipe_classify`, `knowledge_group`) are multiline, semantically ordered, advertise the editable surface through `answer_schema.editable_pointer_pattern`, and omit worker-facing `helper_commands`, `workflow`, `next_action`, and `editable_json_pointers`. Recipe taskfile fallback now stays on the same direct `task.json` edit path as the other single-file stages instead of teaching a helper-heavy batch-write workflow.
...
- interactive `codex-exec` selection now uses one per-step planner in `cookimport/cli_ui/run_settings_flow.py`: each visible step row owns its own `Off` / `JSON` / `Taskfile` mode plus shard count, recipe defaults to `Taskfile`, and line-role plus non-recipe finalize default to `JSON`.
...
- `cookimport/llm/codex_exec_workspace.py` is the explicit owner for direct-exec workspace preparation and mirror-manifest shaping. It resolves the small runner-owned hook/constant surface deliberately instead of inheriting the runner globals wholesale.
- `cookimport/llm/codex_exec_telemetry.py` is the explicit owner for Codex JSON/text event parsing, token-usage availability/status rollups, live-activity summaries, and watchdog reason formatting.
- `cookimport/llm/codex_exec_command_builder.py` is the explicit owner for `codex exec` argv building and Linux taskfile filesystem-fence command assembly; `codex_exec_runner.py` keeps thin wrappers so the public patch/test surface stays import-stable.
- `cookimport/llm/taskfile_prompt_contract.py` now owns the shared prompt-section renderer for the remaining taskfile workers, so recipe, knowledge, and canonical line-role all emit the same repo-owned prompt skeleton while keeping stage-specific answer rules.
...
- those retry and repair budgets now live in one shared owner, `cookimport/llm/repair_recovery_policy.py`. Recipe, line-role, and knowledge still keep their own stage-specific runtime loops, but fresh-session retry, fresh-worker replacement, and structured inline-repair limits now come from one declarative policy table instead of stage-local magic numbers.
- `cookimport/llm/codex_exec_policy.py` is the repo-owned direct-exec policy selector. Inline transports run under `shell_disabled` (`-c features.shell_tool=false`), while taskfile transports run under `taskfile_allowlist` and a restricted helper-only `PATH`.
- repo-owned telemetry and summary artifacts now expose that shared contract directly through `repair_recovery_policy`: each budget row carries `followup_kind`, `followup_surface`, `budget_scope`, `allowed_attempts`, `spent_attempts`, and `remaining_attempts`, so operators can see both the budget and what the run actually spent without reverse-engineering stage-local fields. Aggregate stage summaries now also multiply shard-scoped budgets where needed instead of pretending line-role inline or knowledge shard repair budgets are one worker-wide counter.
- current shared policy matrix:
  - recipe `inline-json-v1`: no follow-up budget; malformed or incomplete shard output fails closed
  - recipe `taskfile-v1`: `same_session_repair_rewrite=1`, `fresh_session_retry=1`, `fresh_worker_replacement=1`
  - line-role `taskfile-v1`: `same_session_repair_rewrite=1`, `fresh_session_retry=1`, `fresh_worker_replacement=1`
  - line-role `inline-json-v1`: shard-result `structured_repair_followup=1`, `watchdog_retry=1`
  - knowledge `taskfile-v1`: worker-assignment `fresh_session_retry=1`, `fresh_worker_replacement=1`; shard-result `structured_repair_followup=1` for invalid final shard outputs after same-session handoff; semantic-step `same_session_repair_rewrite=1` for both `nonrecipe_classify` and `knowledge_group`
  - knowledge `inline-json-v1`: semantic-step `structured_repair_followup=3` for both `nonrecipe_classify` and `knowledge_group`, applied per shard-step instance rather than as one stage-global cap
...
- knowledge shard-plan assembly now has one shared owner in `cookimport/llm/knowledge_stage/stage_plan.py`; prompt preview and live runtime both consume that builder instead of reconstructing the knowledge phase-plan story separately
...
- inline-JSON knowledge runs now write the same terminal `task_status.jsonl` outcomes that the summary layer expects, and their stage telemetry rows carry explicit per-step classification/grouping counters. Healthy completed workers/tasks are no longer counted as `pre_kill` failures just because their final ledgers exist.
- prompt preview and live direct-exec stages now also write one shared phase-planning artifact per stage: `phase_plan.json` plus `phase_plan_summary.json`. Recipe, knowledge, and canonical line-role all now persist those files on the live runtime path. Those files separate the operator-requested shard count, survivability recommendation, budget-native shard count, and actual launch shard count instead of collapsing them into one number.
- knowledge packet budget planning now treats pass-2 grouping output as one capped grouping batch, not one response row for every owned row in the shard. `knowledge_group_task_max_units` therefore constrains the worst-case grouping-output estimate that feeds `budget_native_shard_count`.
...
- the knowledge stage now has one live authority path when enabled: first-pass classification returns only `keep_for_review` or `other`, and the second pass always runs grouping plus shared group grounding together. If no existing tag fits a group, pass 2 may emit `proposed_tags`, but then the group must also supply `why_no_existing_tag` and `retrieval_query`. Final row grounding is a projection of that shared group grounding, not a row-local proposal decision.
...
- current runner behavior assumes current local `codex-farm` support; benchmark flag and stderr/progress handling now follow only the current surface
...
- `cookimport/parsing/canonical_line_roles/runtime.py`
  - the deterministic baseline still exists as the non-LLM line-role path and as the fail-closed fallback when live line-role authority is unavailable.
  - partially resolved live shards no longer backfill unresolved rows from the deterministic baseline. Accepted Codex rows stay accepted; unresolved rows must resolve through repair/retry or remain incomplete/invalid.
...
- `_normalize_prediction_metadata(...)` still normalizes deterministic baseline outputs into the public line-role contract, but accepted Codex answers are no longer post-processed into different labels.

Codex-specific override seams after a valid Codex answer exists:

- `_reject_codex_prediction_to_baseline_if_policy_violated(...)` remains as a named seam, but the current implementation is a no-op placeholder that returns the accepted prediction unchanged.
  The live Codex path no longer replaces accepted model rows with deterministic baseline labels, and it no longer remaps accepted Codex labels into fallback labels after validation.

Validation/promotion seams that can discard or partially replace Codex output:

- `_translate_line_role_output_to_atomic_indices(...)` remains the shared remap helper for repo-owned or legacy row-shaped payloads, but the live inline structured-session path now validates packet-local `row_id` coverage directly before projection back onto real `atomic_index` order.
...
- after structural validation succeeds, the live line-role path does not run a deterministic semantic-guard rejection pass. Accepted rows stay accepted unless they fail a structural ownership or label-contract rule.
...
- `validate_knowledge_classification_task_file(...)` normalizes the worker grounding payload and fails closed on first-pass contract drift.
- direct first-pass `knowledge` now requires at least one surviving existing `tag_key`; otherwise the row is rejected for repair instead of surviving as weakly grounded `knowledge`.
- first-pass proposal invention is forbidden. Rows that seem retrieval-worthy without an existing-tag fit must use `proposal_candidate` with empty grounding and wait for second-pass approval.
...
- in grouping, kept `knowledge` rows normally use `proposal_decision=not_applicable`, but they may also approve one or more new proposed tags when the row is keepable and the existing catalog remains a bad semantic fit.
...
- After a live direct-exec stage finishes, that same `shard_survivability_report.json` is rewritten with observed per-shard runtime evidence from direct telemetry: initial input/output tokens, structured follow-up spend, billed totals, final proposal/supervision state, and a `predicted_vs_observed` block so later budget tuning can compare prediction against reality.
...
- recipe worker folders now also write `status.json`; taskfile runs still emit the worker-session mirrors, while inline runs write shard-local `events.jsonl`, `last_message.json`, `usage.json`, and `workspace_manifest.json` under `workers/*/shards/<shard_id>/` so prompt-budget and replay tooling can still attribute per-shard cost without reviving a taskfile transcript
...
- knowledge structured-response conversion now clears stale placeholder answers for omitted rows before validation/repair, so missing-row repairs surface as missing-row contract failures instead of fake blank `group_id` / `topic_label` carry-forward; grouping repair packets also suppress `previous_groups` entries when there was no meaningful prior grouping answer
- knowledge grouping task files now carry the checked-in ontology too, so grouping sees the real allowed category keys while deciding whether a new tag is warranted
- knowledge grouping task files now batch retained rows by the configured unit/evidence caps, and same-session handoff can advance across multiple grouping batches instead of assuming one shard-sized grouping turn forever
...
- knowledge worker hints now stay intentionally compact: `hints/<shard_id>.md` is a small workflow sidebar, not a second prompt, and it keeps only the shard id, owned row count/range, nearby recipe guardrail indices, worker-flow reminders, and example-file pointers before the worker reopens the authoritative `task.json`
- `codex_farm_knowledge_ingest.py` still validates exact owned row coverage plus exact kept-row-to-idea-group coverage, but the worker-facing contract reaches that final packet shape through two validated task-file steps rather than one combined semantic answer
- repo-owned knowledge validation/reporting metadata is row-native now (`owned_row_indices`, `unresolved_row_indices`, `missing_owned_row_indices`, `knowledge_rows_missing_group`, and related summary keys), while compact worker payload aliases such as `row_decisions` and `row_groups[*].row_indices` still remain as the persisted packet/provenance surface for now
- the knowledge semantic boundary is now ontology-grounded and outcome-first: `knowledge` means a portable concept worth storing and later retrieving on its own, not just something cooking-adjacent that happens to be true. The classification file stays block-local and grouping-free, and a kept row now emits only `category` plus `grounding` to an existing tag or a proposed tag under an existing category. The inline packet now exposes the real existing tag list directly instead of any repo-generated candidate-tag shortlist, so proposed tags are the fallback path rather than the first easy move.
- the knowledge classification prompt now says teacherly or motivational prose is not enough by itself: split coaching fragments and broad encouragement should stay `other` unless the owned row or block itself stands alone as a concrete reusable rule, mechanism, troubleshooting idea, substitution, storage/safety rule, or compact reference fact.
- the knowledge prompt contract now also tells the worker not to invent broad chapter-theme or pedagogy-summary proposal tags. New proposals should name one concrete row-local retrieval concept, not umbrella lesson labels like generic "heat science" or "fat management" abstractions.
- the knowledge worker vibe is now explicitly anti-heuristic: the prompt, review contract, and hint sidecar all say that block text is primary evidence, neighboring rows plus heading/profile cues are weak hints only, heading rows should stay `other` in pass 1, and if the worker feels tempted to invent one rule for many rows it should stop and reread the owned block text instead
- live knowledge validation is structural and grounding-focused: deterministic code checks ownership, coverage, block order, allowed enum values, kept-block grouping completeness, checked-in catalog membership for `tag_keys` and `category_keys`, and normalized proposed tags, but it does not rewrite model `knowledge` rows to `other` after the fact.
- accepted knowledge outputs are compact shard outputs: top-level `packet_id`/shard id, ordered `row_decisions`, and model-authored `row_groups`
- the knowledge prompt/worker contract now treats heading rows as `other` in pass 1 even when they directly introduce useful explanatory blocks in the same owned shard; short heading-like `other` rows can still survive into pass 2 as batch-local context-only `ordered_rows`, while decorative or menu-like headings remain ordinary `other`
...
- `prediction-run/line-role-pipeline/runtime/line_role/phase_manifest.json`
- `prediction-run/line-role-pipeline/runtime/line_role/phase_plan.json`
- `prediction-run/line-role-pipeline/runtime/line_role/phase_plan_summary.json`
- `prediction-run/line-role-pipeline/runtime/line_role/shard_manifest.jsonl`
- `prediction-run/line-role-pipeline/runtime/line_role/shard_status.jsonl`
- `prediction-run/line-role-pipeline/runtime/line_role/worker_assignments.json`
- `prediction-run/line-role-pipeline/runtime/line_role/proposals/*.json`
- `prediction-run/line-role-pipeline/runtime/line_role/line_role_stage_summary.json`
...
- the live line-role runtime root now also writes the shared planning artifacts `phase_plan.json` and `phase_plan_summary.json`, so line-role post-run reporting uses the same requested-vs-survivability-vs-budget-native-vs-launch count story as recipe and knowledge
...
- the line-role prompt/hint contract now treats outside-recipe routing as a stricter `NONRECIPE_CANDIDATE` versus `NONRECIPE_EXCLUDE` gate: portable explanation/reference prose and supported lesson headings can stay `NONRECIPE_CANDIDATE`, while memoir, blurbs, endorsements, book-framing encouragement, rhetorical setup, and short split coaching fragments such as `Taste. It will need salt.` should stay `NONRECIPE_EXCLUDE`
...
- there is no longer any live support for a separate LLM recipe gate inside line-role; the only active line-role model surface is the single `line_role` labeling phase
...
- `workers/*/hints/*.md` are worker-facing decision aids. They are not validator authority. Line-role hints stay intentionally narrow: open the authoritative `in/*.json` payload, label the owned rows directly from text, and use nearby rows only as boundary context.
...
- those `workers/*/in/*.json` files are now compact row ledgers: `{"v":2,"shard_id":...,"context_before_rows":[[atomic_index,row_index,current_line], ...],"rows":[[atomic_index,row_index,current_line], ...],"context_after_rows":[[atomic_index,row_index,current_line], ...]}`. The boundary-context keys are optional and appear only when immediate neighbors exist. The model-facing file stays compact row-wise and carries only direct row evidence plus immediate neighboring context.
...
- when processed-output projection adds final-authority metadata (`mode=final_authority_projection`), it now stamps those fields onto the existing runtime `telemetry_summary.json` instead of replacing the runtime `summary` / `phases` token ledger
...
- reconstructed prompt-template fallbacks are reviewer conveniences only; if a shard has concrete runtime identity but no telemetry or worker/session call evidence, it must not emit a `full_prompt_log.jsonl` row
...
- Prompt preview labeled-line reconstruction is row-native only. When preview code rebuilds line-role candidates from `labeled_lines.jsonl`, those rows must carry `source_block_index`; prompt preview no longer falls back to semantic `row_index` on that row-facing seam.
...
- older saved runs that predate the prompt-target fields should default preview planning to the current shard-v1 target count (`5`) for each enabled phase instead of falling back to older shard-size defaults
...
- `build_knowledge_jobs(...)` now sizes ordered candidate packets from the candidate block queue and keeps one task per packet. When `knowledge_prompt_target_count` is set, it is a hard cap on final shard count, not a requirement to fragment already-budget-safe packets; packet budgets stay the locality-preserving source of truth, and knowledge worker count still controls concurrency separately from shard planning. Unset knowledge packet budgets are now auto-derived from the `nonrecipe_finalize` survivability ceilings with explicit headroom instead of using the older tiny fixed `18k` / `6k` defaults.
...
- line-role shard validation is still structurally strict, but structural success is the end of deterministic validation on the live path. Repo code does not emit `semantic_invariant_violation:*` errors or reopen rows for repair just because local heuristics prefer different labels.
- line-role row transport is now split cleanly by seam: the inline compact prompt uses provenance strings such as `r01 | 210 | Bright Cabbage Slaw` for owned rows plus `210 | Variations` for neighboring context, the file-backed billed shard JSON is the tuple-based `v=2` transport, and a parallel debug copy keeps the richer local object rows
...
- for inline-json knowledge structured-session runs, `prompt_nonrecipe_finalize.txt` now mirrors the real per-turn structured-session prompt / packet / response text rather than only the stage IO dump, while `full_prompt_log.jsonl` remains the canonical row-level prompt log
...
- After shard-v1 cutover, active run-setting surfaces are already strict about pipeline ids. If older behavior still shows up, it is more likely to be a reader/fixture/tooling seam than the live recipe, knowledge, or line-role execution path.
...
- Current line-role runtime truth is one file-backed `line_role` phase under `line-role-pipeline/runtime/line_role/`. The abandoned inline path and the brief `recipe_region_gate` / `recipe_structure_label` split are retired; new docs and prompt exports should describe the single file-backed phase plus its immutable line table, shard-owned row ledger, and worker `in/*.json` payloads.
- That runtime root now also keeps `phase_plan.json`, `phase_plan_summary.json`, and `line_role_stage_summary.json`, so planning and post-run budget readers do not need a separate line-role-only count story.
...
- Current line-role transport is intentionally split three ways:
  ...
  The billed payload is now compact provenance-first: owned rows are `[atomic_index, row_index, current_line]` plus optional `context_before_rows` / `context_after_rows` in that same tuple form, while the debug copy stays repo-owned.
...
- the shared line-role prompt now also treats bare `Variation` / `Variations` lines as cue rows that usually stay `OTHER` rather than `RECIPE_VARIANT`, allows the following heading-like variant title plus attached intro prose to carry `RECIPE_VARIANT` when the local alternate-version run is clear, requires `HOWTO_SECTION` to own a real mini-preparation with nearby ingredients and/or steps, and classifies storage/refrigerate/freeze/leftovers guidance as `RECIPE_NOTES`.
...
- Knowledge reliability now depends on two separate rules that are easy to conflate:
  - requested counts remain an upper bound for recipe, knowledge, and line-role launch planning
  - knowledge still records warnings when a lower requested cap forces oversized or non-local bundles
