...
LLM usage in this repo is optional. The direct-exec transport is now mixed by both attempt type and configured Codex style. Recipe refine stays on the assignment-first `taskfile` contract. `line_role_codex_exec_style=inline-json-v1` and `knowledge_codex_exec_style=inline-json-v1` are the default thin paths for block labelling and nonrecipe finalize, while `taskfile-v1` remains available on each surface for comparison or debugging. Deterministic repo code still owns validation, repair rewrites, grounding gates, and final artifact expansion.

For inline-JSON line-role, the visible prompt is still transport-thin, but it is no longer semantically thin: the prompt now derives from the shared `line-role.shared-contract.v1.md` contract, carries shard-profile evidence for front-matter/contents risk, and can trigger one bounded semantic repair follow-up when structurally valid output is still obviously pathological. Deterministic code remains the owner of row-id coverage, safety-boundary checks, semantic-pathology rejection, and conservative outside-recipe containment; it does not silently relabel ordinary ambiguous prose.
...
The worker-visible self-help surface is now intentionally split, but all current single-file task files are slimmer than the older helper-protocol versions. Direct-batch task files (`line_role`, `nonrecipe_classify`, `knowledge_group`) are multiline, semantically ordered, advertise the editable surface through `answer_schema.editable_pointer_pattern`, and omit worker-facing `helper_commands`, `workflow`, `next_action`, and `editable_json_pointers`. Recipe refine now also omits those serialized helper metadata fields from `task.json`; its prompt still recommends the wrapper-first helper flow (`task-summary`, `task-show-unit`, optional `task-template` plus `task-apply`, then `task-handoff`), but the helper surface now comes from repo-owned commands plus stage key rather than from worker-visible task-file metadata.
