...
LLM usage in this repo is optional. The direct-exec transport is now mixed by attempt type rather than by stage: recipe, line-role, and knowledge main work uses assignment-first workspace-worker sessions, while narrow retry / repair follow-up attempts can still use structured one-shot calls. The happy path is now the same across those stages: open repo-written `task.json`, edit only the declared `/units/*/answer` fields in place, save the same file, run the repo-owned same-session helper once, and stop when that helper reports completion. Deterministic repo code still owns validation, repair rewrites, and final artifact expansion. If the first validation fails, the same helper can rewrite `task.json` into repair mode for one bounded in-session fix pass before the stage fails closed.

The worker-visible self-help surface is now explicit. `task.json` carries repo-authored `helper_commands`, `next_action`, and `answer_schema` metadata, `python3 -m cookimport.llm.editable_task_file --summary` can summarize the current file, and `python3 -m cookimport.llm.editable_task_file --set-answer ...` or `--apply-answers-file ...` can apply explicit answers without ad hoc JSON surgery. Recipe, knowledge, and canonical line-role same-session helpers also expose read-only `--status` and `--doctor` modes from the workspace root, auto-discover their hidden same-session state under `./_repo_control/`, and preserve the same guidance metadata if they rewrite `task.json` into repair mode.
...
- recipe, knowledge, and line-role now allow one bounded fresh-session recovery attempt on the happy path when the first session exits cleanly, leaves recoverable `task.json` plus same-session state behind, and did not hit a hard watchdog boundary. That restart is stage-owned recovery, not a watchdog policy change.
...
- canonical line-role now also has one narrow watchdog fail-fast path after a final assistant message: if the worker emits a final message but the required `workers/*/out/*.json` shard outputs are still missing after a 2-second grace window, runtime records a deterministic `--status` / `--doctor` recovery assessment, may spend the same shared fresh-session retry budget on one preserved-workspace rescue session, and otherwise fails closed with explicit `workspace_final_message_missing_output` metadata instead of letting the same session spiral until exhaustion.
- the shared worker instructions now explicitly steer agents away from broad queue dumps and ad hoc shell schedulers. The preferred pattern is: open `task.json` directly, avoid shell on the happy path except for the one repo-owned same-session helper command, and let repo code expand validated answers into final `out/` artifacts.
- the shared worker contract is now deliberately calm as well as strict: it tells the worker that `task.json` is the whole job, that there is no hidden repo-side context to discover on the happy path, and that brief local rereads or false starts should be corrected rather than treated as fatal.
...
- line-role happy-path `task.json` now keeps the worker-visible evidence raw-text-first as well: immutable row evidence still includes stable provenance and context, but it no longer exposes a copyable `deterministic_hint_label` in the worker-visible file.
...
- recipe, knowledge, and canonical line-role progress emitters now summarize those repo-owned `live_status.json` files into operator-facing progress rows: `active_tasks` can carry compact attention suffixes, `detail_lines` can report warning/stalled-worker counts plus a short `attention:` summary, and `last_activity_at` now reflects the freshest worker event seen by the stage runtime
- knowledge and canonical line-role runtimes now also run a small progress heartbeat while workspace workers are active so quiet-worker attention changes appear in the interactive panel before the shard completes; recipe reuses its existing polling loop for the same effect
