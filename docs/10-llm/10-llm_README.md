...
- the knowledge stage now lets classification and grouping split cleanly through `knowledge_grouping_enabled`; when it is `false`, knowledge still decides `knowledge` versus `other` but skips the second grouping pass and writes empty `idea_groups`
