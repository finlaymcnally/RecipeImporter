...
- knowledge no longer treats raw deterministic chunks as the public shard contract. Deterministic chunking still builds the ordered review units, but the planner now groups those chunks into approximately `knowledge_prompt_target_count` shards, and each shard becomes one task row under `assigned_tasks.json`.
- knowledge promotion is now task-aware at the shard handoff: valid shard-owned proposals promote directly while missing or semantic-invalid tasks remain unreviewed and the rollups count owned chunk truth explicitly.
- knowledge phase manifests now also persist the blast-radius policy explicitly in `runtime_metadata`: `bundle_policy` records the shard-owned chunk-bundle task policy, task totals and per-worker task counts show the live queue size, and the old packet-named counters remain as compatibility aliases.
...
- knowledge runtime now also writes `task_manifest.jsonl` plus worker-local `assigned_tasks.json`; those task rows now match `knowledge/in/*.json` one-for-one because each row owns one planned shard and one result path, and that shard may contain one or more chunk ids
...
- the authoritative knowledge contract is now: `knowledge/in/*.json` immutable shard payloads over ordered chunk slices, worker-local semantic task-result candidates under `workers/*/out/*.json`, `knowledge/proposals/*.json` validated repo-serialized shard proposals, then deterministic promotion into `08_nonrecipe_seed_routing.json`, `09_nonrecipe_authority.json`, `09_nonrecipe_review_status.json`, and reviewer-facing knowledge artifacts
