...
- line-role no longer uses the path-mode CodexFarm `process` transport either. Line-role worker assignments now start in one long-lived workspace-worker Codex session per worker assignment, split each shard into smaller tasks under `assigned_tasks.json`, let each task own one line packet, write one immutable `canonical_line_table.jsonl`, and record packet truth in `task_status.jsonl`; only missing or invalid packets fall back, and reruns resume durable packet outputs instead of re-asking the whole shard.
...
3. The line-role Codex surface reviews the whole book line set in worker-local shard files. Operator-wise this is still just "label the lines," but the live main path now gives each worker session its assigned local tasks inside those shards, with each task owning one line packet, and writes one `out/<task_id>.json` per task. Repo code validates and joins those packet results back into the shard proposal, using deterministic baseline fallback for any missing packet rows instead of rerunning the whole shard.
...
- line-role `assigned_tasks.json` and `current_task.json` are now metadata-first control files. They name `metadata.input_path`, `metadata.hint_path`, `metadata.scratch_draft_path`, and `metadata.result_path`, but they no longer duplicate the full packet payload that already lives in `in/<task_id>.json`.
