...
LLM usage in this repo is optional. The direct-exec transport is now mixed by attempt type rather than by stage: recipe, line-role, and knowledge worker assignments can use one long-lived local workspace-worker session, while stage-specific retry / repair follow-up attempts can still use structured one-shot calls. Recipe still validates task-owned outputs. Recipe and knowledge now use repo-owned leased packets on the happy path; canonical line-role keeps a phase-ledger workspace.
...
- `packet`: a compact model-facing payload shape. Recipe uses one leased packet per recipe task. Knowledge uses leased packets for the current worker control surface even though final ownership and promotion stay shard-scoped.
...
- workspace-worker sessions now also get a repo-written `worker_manifest.json` in the worker root and mirrored sterile workspace. The worker contract is explicit: start from `worker_manifest.json`, then open the repo-written active control surface when it exists (`CURRENT_PHASE*` for line-role, `current_packet.json` plus friends for recipe and knowledge), then use the named `hints/*.md` and `in/*.json` files directly, and stay inside the bounded workspace rather than reaching back into the repo.
- watchdog policy is now split by transport: structured retry / repair calls still treat shell commands as immediate off-contract behavior, and leased-packet workspace happy paths are now file-first rather than shell-first. The preferred normal loop is: open the repo-named current packet files directly, write the approved result path, then reopen the packet files after the repo advances the lease.
- the shared worker instructions now explicitly steer agents away from broad queue dumps and ad hoc shell schedulers. The preferred pattern is: open the repo-named file directly, avoid shell on the happy path, use one tiny local helper only when truly necessary, and write the final answer only to the approved `out/` path.
...
- the relaxed workspace-worker policy now also tolerates the multi-line helper scripts the knowledge workers actually emit in production, including `>/dev/null` sinks and bounded `jq`/`cat` loops over `assigned_shards.json`, `current_packet.json`, `current_result_path.txt`, `hints/*.md`, `in/*.json`, and `scratch/*.json`
...
- workspace-worker main attempts still terminate primarily on clear boundary/liveness violations, but leased-packet stages now treat shell usage as fallback rather than routine startup behavior. Narrow local helpers may still be tolerated when they stay inside the worker root and do not rewrite repo-owned control files or invent queue schedulers.
...
- recipe worker staging now short-circuits terminal scaffold tasks before the workspace queue: if the repo-authored task scaffold is already `fragmentary` or `not_a_recipe`, runtime validates and installs that task locally and keeps it out of the live leased-packet worker prompt entirely.
...
- `recipe_stage_summary.json` now uses `followups.label="packet_followup"` and reports recipe followup attention in packet terms: `handled_locally_skip_llm_count`, `repair_attempted_count`, `repair_completed_count`, `repair_running_count`, and `proposal_count`.
...
- for leased-packet failures, `task_status.jsonl` is now explicit rather than generic: packet rows can terminate as `worker_exited_with_packet_still_leased`, `packet_result_validation_blocked`, `repair_packet_exhausted`, `queue_completed_without_promoted_output`, or `process_exited_without_final_packet_state`, while watchdog kills still keep their original watchdog `terminal_reason_code`
...
- the line-role and knowledge success watchdogs no longer stop a worker the instant outputs stabilize; they now wait for one repo-visible completion signal, or a short quiet period, before asking the runner to terminate the session
...
- recipe no longer uses the path-mode CodexFarm `process` transport either. Recipe worker assignments now start in one long-lived workspace-worker Codex session per worker assignment, but the live control surface is packet-first rather than draft-first: `current_packet.json`, `current_hint.md`, `current_result_path.txt`, and `packet_lease_status.json` are the only active-task files on the happy path. Each leased packet owns exactly one recipe task. The worker writes one `out/<task_id>.json`, repo code validates it, advances the lease when valid, and writes at most one repair packet for that same task when the first output is invalid. The happy path no longer uses `assigned_tasks.json`, `SHARD_PACKET.md`, `CURRENT_TASK.md`, `CURRENT_TASK_FEEDBACK.md`, `scratch/`, `tools/recipe_worker.py`, `OUTPUT_CONTRACT.md`, or same-session draft-install loops. Recipe telemetry and summaries now separate worker-session counts, leased-packet counts, repair-packet counts, locally skipped packet counts, and final per-recipe outcomes instead of rolling those into same-session fix stories.
...
- knowledge now plans one ordered shard input file per shard under `knowledge/in/*.json`, and each shard stays owned by one long-lived worker session through repo-owned Pass 1 and Pass 2 leased packets. The validator still requires exact owned block coverage and exact kept-block-to-group coverage, but the happy path no longer uses worker-edited phase ledgers.
...
- `cookimport/llm/knowledge_runtime_state.py` now defines the shared knowledge runtime vocabulary (`KnowledgePacketRecord`, worker-outcome buckets, follow-up kinds, and `KnowledgeStageRollup`) and `cookimport/llm/knowledge_runtime_replay.py` now rebuilds packet state from authoritative `task_status.jsonl` rows while still replaying worker/follow-up/artifact rollups from the saved knowledge root.
...
3. The line-role Codex surface reviews the whole book line set in worker-local shard files. Operator-wise this is still just "label the lines," but the live main path now gives each worker session one shard-owned row ledger at a time, writes one `out/<shard_id>.json` per shard, validates that ledger locally, prefers same-session repair when validation fails, allows one bounded watchdog retry after a retryable worker kill, and otherwise fails closed. Deterministic fallback is not final authority on the live worker path.
...
- recipe worker assignments now launch one long-lived workspace-worker Codex session per worker assignment with shared `assigned_shards.json`, authoritative `in/*.json`, repo-owned leased packet files (`current_packet.json`, `current_hint.md`, `current_result_path.txt`, `packet_lease_status.json`), and harvested `out/*.json`; recipe tasks own one `recipe_id`, workers write `out/<task_id>.json`, deterministic code validates those task outputs, and shard proposals are rebuilt from the accepted task files
- recipe runtime now writes `task_manifest.jsonl`, but the live worker surface is the leased packet rather than a worker-owned task inventory. `assigned_tasks.json` is gone from the live recipe path
...
- the live knowledge model call now goes through `codex_exec_runner.py`, and the worker surface is leased-packet-first rather than phase-ledger-first
- knowledge worker assignments now launch one long-lived workspace-worker Codex session per worker assignment with shared `assigned_shards.json` for context, repo-written `current_packet.json`, `current_hint.md`, `current_result_path.txt`, `packet_lease_status.json`, authoritative `in/*.json`, harvested `out/*.json`, worker-local `scratch/*.json`, `OUTPUT_CONTRACT.md`, and `examples/`; deterministic preflight still rejects malformed shards before the worker session starts
- `packet_lease_status.json` is now the repo-side lease truth surface for no-result classification. Besides the active packet pointer, it keeps `schema_version`, `last_runtime_action`, current packet/result/promotion fields, and per-shard `shard_statuses` summaries so the repo can tell whether a worker exited with a packet still leased, hit structural validation, exhausted a repair packet, or merely lacks enough evidence for a stronger reason.
- `knowledge_phase_workspace_tools.py` is now the packet-helper/validator owner for that worker surface. The normal paved road is: open `current_packet.json`, `current_hint.md`, and `current_result_path.txt`; write exactly one JSON object to the named result path; then reopen the current-packet files after the repo advances the lease. `OUTPUT_CONTRACT.md` and `examples/` are fallback contract/debug surfaces rather than the default first read.
- accepted knowledge rows now freeze in repo-owned packet state rather than in worker-edited phase ledgers; repair packets name only unresolved rows and accepted structural context
...
- Pass 1 packets carry only the owned rows plus mechanical context; the worker returns compact per-row `category` results instead of editing repo-owned work ledgers
...
- repo-owned lease advancement is the handoff: it validates the current packet result, freezes accepted rows, advances the same session from Pass 1 to Pass 2 when ready, canonicalizes final `group_id` values during final assembly, and then installs the final shard output when Pass 2 is accepted
...
- knowledge worker roots now also carry `worker_manifest.json`, and the shared worker prompt tells the worker to prefer the named local files first and avoid shell on the happy path; any tiny local helper remains fallback only inside the worker root
...
- live knowledge validation is structure-only: deterministic code checks ownership, coverage, block order, allowed enum values, and kept-block grouping completeness, but it does not reject a structurally valid shard because repo heuristics disagree with the model's semantic judgment.
...
- completed knowledge workspace workers should now always carry a non-null `reason_code` in `live_status.json`; successful packet-lease workers finish as `workspace_validated_task_queue_completed` once the repo-owned lease controller can see queue completion in `packet_lease_status.json` plus the installed shard outputs, stable-output runs without provable queue completion still fail closed as `workspace_validated_task_queue_incomplete`, premature clean exits after visible queue advancement first surface as `workspace_validated_task_queue_premature_clean_exit` during the auto-resume backstop, repeated conversational stops can end as `workspace_validated_task_queue_premature_clean_exit_cap_reached`, and true watchdog kills keep their original watchdog reason codes
...
- line-role worker assignments now launch one long-lived workspace-worker Codex session per worker assignment with `assigned_shards.json`, `current_phase.json`, `CURRENT_PHASE.md`, `CURRENT_PHASE_FEEDBACK.md`, shared `prompt.txt`, worker-local `hints/*.md`, authoritative `in/*.json`, richer `debug/*.json`, editable `work/*.json`, optional `repair/*.json`, and harvested `out/*.json`; same-session repair is the normal recovery path, and only retryable watchdog kills still use one bounded structured follow-up attempt on the affected shard
...
- line-role no longer launches a repo-owned post-session repair prompt. If the worker leaves `out/<shard_id>.json` missing or invalid, the runtime preserves the shard-local work/repair evidence, may allow one bounded watchdog retry after a retryable `watchdog_killed` worker outcome, and otherwise fails closed without row-level deterministic substitution. Runtime telemetry still records shard-scoped fields such as `runtime_shard_id` and `worker_session_shard_count` instead of teaching a fake line-role task layer.
- direct line-role shard folders now also write `workspace_manifest.json`, and shard roots may also contain `repair_status.json` plus `watchdog_retry_*` artifacts as runtime evidence even though ordinary repair remains workspace-local
...
- Recipe worker cost regressions are usually readback regressions now, not watchdog regressions. If recipe token spend spikes again, inspect whether workers have slipped back into raw `hint_path` / `input_path` rereads, `assigned_shards.json` inventory dumps, or wider-than-needed workspace exploration before changing retry policy or prompt wording.
- Knowledge queue regressions now split cleanly into two seams:
  - leased-packet authority and sync
  - conversational premature-clean-exit recovery
  If a worker says "I can continue if you want" mid-queue, do not treat that as success and do not reintroduce a worker-owned task queue surface as the primary control file.
...
- line-role main workspace workers now use the same rare-kill watchdog contract as the other workspace stages, but the active control surface is still line-role-specific: start from `worker_manifest.json`, `assigned_shards.json`, `CURRENT_PHASE*`, and the named shard files. Bounded local orientation and helper shell use inside the worker root are telemetry, not kill reasons, unless they turn into a real boundary violation or a pathological no-progress loop.
