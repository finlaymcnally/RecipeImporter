...
- `_apply_line_role_semantic_guard(...)` is currently a metadata/no-op seam; it preserves proposal validity/status unchanged and does not reject, demote, or replace model rows.
...
- classification `task.json` now includes immutable task-scope `ontology` loaded from the checked-in `cookimport/llm/knowledge_tag_catalog.json` snapshot plus only unit-local text/context/structure evidence; workers are expected to ground to existing tags first, and deterministic code validates grounding catalog membership/shape plus a narrow low-utility demotion pass for memoir, framing, navigation, decorative-heading, and true-but-low-utility rows before promotion
...
- live knowledge validation is still narrow, but it now enforces the grounding contract and one explicit usefulness boundary: deterministic code checks ownership, coverage, block order, allowed enum values, kept-block grouping completeness, checked-in catalog membership for `tag_keys` and `category_keys`, normalized proposed tags, and a small post-review demotion pass that drops memoir/framing/navigation/decorative-heading/low-utility rows back to `other` even if the model attached plausible grounding.
