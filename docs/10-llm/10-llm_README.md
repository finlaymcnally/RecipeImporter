...
- knowledge worker hints now stay intentionally compact: `hints/<shard_id>.md` is a small workflow sidebar, not a second prompt, and it keeps only the shard id, owned block count/range, nearby recipe guardrail indices, worker-flow reminders, and example-file pointers before the worker reopens the authoritative `task.json`
