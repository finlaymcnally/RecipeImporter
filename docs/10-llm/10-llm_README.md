...
LLM usage in this repo is optional. The direct-exec transport is now mixed by attempt type rather than by stage: recipe, line-role, and knowledge worker assignments all start in one long-lived local workspace-worker session that writes `out/*.json` results incrementally, while stage-specific retry / repair follow-up attempts can still use structured one-shot calls. Recipe and knowledge still validate task-owned packets inside parent shards. Canonical line-role is different now: the live answer unit is the shard-owned row ledger itself, with no line-role `assigned_tasks.json`, `CURRENT_TASK*`, or `*.task-*` layer in the happy path.

## Terminology

- `shard`: the planning/scheduling bundle. The interactive `*_prompt_target_count` settings ask for shard counts.
- `task`: the smallest validated work item for recipe and knowledge. Canonical line-role no longer uses a live task layer; its validated work item is the shard ledger.
- `packet`: the model-facing content slice owned by one task. A packet is the content, not the scheduler row.
...
- line-role no longer uses the path-mode CodexFarm `process` transport either. Line-role worker assignments now start in one long-lived workspace-worker Codex session per worker assignment, keep one shard-owned row ledger per shard under `work/<shard_id>.json`, write one immutable `canonical_line_table.jsonl`, and record shard truth in `shard_status.jsonl`; accepted rows now freeze immediately, repair only receives unresolved row ids plus frozen-row context, and deterministic fallback applies only to unresolved owned rows that survive the repair ladder.
- knowledge now uses the packet contract end to end, but shards and tasks are no longer the same thing. `codex_farm_knowledge_jobs.py` still writes one ordered packet payload per file under `knowledge/in/*.json`, shard manifests can bundle several ordered packets when `knowledge_prompt_target_count` asks for fewer prompts, task manifests split those shard bundles back into one packet-owned task per result file, and the validator requires exact block coverage plus exact knowledge-block-to-group coverage per packet.
- knowledge promotion is packet-aware at the shard handoff: valid packet proposals promote directly while missing or semantic-invalid packets remain unreviewed and the rollups count packet truth explicitly.
...
7. The knowledge Codex surface reviews the non-recipe side. The program first packages the category-neutral `nonrecipe-route` review queue into ordered non-recipe packets, then groups those packets into planned shards according to the prompt target count. Codex still reviews one packet-owned task at a time using raw block text plus mechanically true structure rather than letting deterministic code declare final grouping meaning. One worker session processes its assigned packet tasks under `in/` and writes one semantic task-result JSON file per task under `out/`, then repo code aggregates those packet results back into shard proposals, validates exact owned block coverage plus packet-local idea-group coverage, and keeps/refines useful cooking knowledge while rejecting blurbs, filler, and other author yapping.
...
- Knowledge now has a real shard-bundling seam: packet files remain immutable and packet-sized, but a lower `knowledge_prompt_target_count` can bundle several packets into one planned shard while preserving one packet-owned task/result per packet inside that shard.
...
Recipe runtime note:
- the live recipe implementation now defaults to one candidate-owned shard per recipe, executes those shards through the repo-owned direct recipe worker runtime in `recipe_stage/runtime.py`, validates exact owned `recipe_id` coverage in `recipe_stage/validation.py`, and promotes only validated per-recipe outputs through `recipe_stage/promotion.py`
...
- deterministic repo code now decides only packet sizing and ordering. It does not decide final local grouping, and it does not bypass review-eligible packets based on utility cues.
- `codex_farm_knowledge_jobs.py` now writes one ordered packet payload per file under `knowledge/in/*.json`, then groups those packets into real shard bundles. Hard packet caps still define the packet floor, but a requested `knowledge_prompt_target_count` now changes the number of shard entries rather than just warning; packet tasks and output files remain one-per-packet inside each shard.
...
- knowledge worker assignments now launch one long-lived workspace-worker Codex session per worker assignment with shared `assigned_shards.json`, a skinny worker-local `assigned_tasks.json`, repo-written `current_batch.json`, `CURRENT_BATCH.md`, `CURRENT_BATCH_FEEDBACK.md`, compatibility `current_task.json` / `CURRENT_TASK*` fallback files for the first active task, authoritative `in/*.json`, harvested `out/*.json`, `scratch/`, `OUTPUT_CONTRACT.md`, `examples/`, and `tools/knowledge_worker.py`. The repo-written worker surfaces are packet-native even when the parent shard owns several packets: every task talks about one packet id, block decisions, and idea groups rather than chunk-result wrappers or utility-cue skepticism scaffolds.
...
- those knowledge `out/*.json` files are semantic packet-result candidates: top-level `packet_id`, ordered `block_decisions`, and packet-local `idea_groups`
- knowledge runtime now also writes `task_manifest.jsonl` plus worker-local `assigned_tasks.json`; those task rows stay one-per-packet even when `shard_manifest.jsonl` bundles multiple packet ids into the same parent shard
...
- `codex_farm_knowledge_ingest.py` validates exact owned block coverage plus exact knowledge-block-to-idea-group coverage and rejects any snippet evidence that points outside the packet's eligible block surface
- the knowledge validator now also rejects low-trust semantic packet shapes such as non-grounded snippet bodies, full-packet surface echoes, and substantial verbatim copies of one evidence quote, while deterministic salvage stays narrow to trailing `EOF` and obvious shell-wrapper suffix noise after a valid JSON object
...
- the knowledge semantic boundary is now explicitly utility-first: `knowledge` means durable cooking leverage, not just something cooking-adjacent that happens to be true. The billed packet payload stays raw block text plus mechanically true structure only; repo-written helper copy now frames the decision in packet/block/group terms instead of utility-cue taxonomies.
- accepted knowledge outputs are packet bundles only: `v=3`, `bid`, ordered block decisions under `d`, and model-authored idea groups under `g`. Runtime rollups no longer pretend packet review carries per-result `reason_code` telemetry.
- the knowledge prompt/worker contract now also treats short conceptual headings as keepable when they directly introduce useful explanatory blocks in the same owned packet; decorative or menu-like headings still stay `other`
- the authoritative knowledge contract is now: `knowledge/in/*.json` immutable packet payloads, worker-local semantic task-result candidates under `workers/*/out/*.json`, `knowledge/proposals/*.json` validated repo-serialized shard proposals, then deterministic promotion into `08_nonrecipe_seed_routing.json`, `09_nonrecipe_authority.json`, `09_nonrecipe_review_status.json`, and reviewer-facing knowledge artifacts
...
- line-role worker assignments now launch one long-lived workspace-worker Codex session per worker assignment with `assigned_shards.json`, `current_phase.json`, `CURRENT_PHASE.md`, `CURRENT_PHASE_FEEDBACK.md`, shared `prompt.txt`, worker-local `hints/*.md`, authoritative `in/*.json`, richer `debug/*.json`, editable `work/*.json`, optional `repair/*.json`, and harvested `out/*.json`; only watchdog-retry and repair still use structured follow-up attempts on the affected shard
...
- line-role worker roots now also carry `worker_manifest.json` plus repo-written `examples/*.md` contrast files. The shared worker prompt now frames deterministic labels as a strong prior, tells the model to read the shard-local hint/input files first, and uses the examples only for conservative calibration.
- invalid or semantically rejected line-role outputs now get one repo-owned repair attempt; parent-shard folders are created before repair/watchdog follow-up artifacts are written, shard folders can include `repair_prompt.txt`, `repair_events.jsonl`, `repair_last_message.json`, `repair_usage.json`, and `repair_status.json`, and the repair prompt now requests only an unresolved-row patch while frozen accepted rows stay off-limits. Runtime telemetry now records shard-scoped fields such as `runtime_shard_id` and `worker_session_shard_count` instead of teaching a fake line-role task layer.
...
- `workers/*/hints/*.md` are worker-facing decision aids. They are not validator authority and are intentionally allowed to be richer, more explanatory summaries than the billed `in/*.json` payload. Line-role hints now include shard interpretation, conservative flip policy, and example-file pointers in addition to the older shard-profile/attention-row summaries.
...
- those `workers/*/in/*.json` rows are now compact tuples: `{"v":1,"shard_id":...,"shard_summary":...,"default_posture":...,"flip_policy":[...],"example_files":[...],"rows":[[atomic_index,label_code,current_line], ...]}`. The model-facing file stays compact row-wise, but it now carries small shard-level context fields so a cold-start worker can stay conservative without reading repo code.
...
- the active line-role wrapper now teaches `HOWTO_SECTION` as a book-optional, recipe-internal subsection label only (`FOR THE SAUCE`, `TO FINISH`, `FOR SERVING`). Shard files now carry `howto_section_availability` plus `howto_section_policy`, and the wrapper explicitly fences chapter/topic/cookbook-lesson headings back to `KNOWLEDGE` or `OTHER`
