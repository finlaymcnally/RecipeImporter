...
- workspace-worker sessions now also get a repo-written `worker_manifest.json` in the worker root and mirrored sterile workspace. The worker contract is now explicit: start from `worker_manifest.json`, then open `current_task.json` when it exists, then use the named `hints/*.md` and `in/*.json` files directly, and stay inside the bounded workspace rather than reaching back into the repo.
- watchdog policy is now split by transport: structured retry / repair calls still treat shell commands as immediate off-contract behavior, while workspace-worker main attempts now use a relaxed local-shell policy. The main worker session can use normal local shell shapes, including searches, filters, pipelines, redirections, heredocs, root-relative scratch files, short-lived local temp roots such as `/tmp`, and glob reads, as long as it stays inside the isolated workspace.
- that workspace-worker policy is now intentionally boundary-based rather than shell-shape allowlisted: main sessions are killed for real workspace escapes, forbidden executables, or pathological no-output loops, not for ordinary local shell syntax by itself. Non-temp absolute paths are still forbidden unless the command is surfacing the session's own assigned execution root (or a child of it) on the main workspace-worker path; local temp roots such as `/tmp` and `/var/tmp` are tolerated for short-lived helper files.
...
- workspace-worker main attempts now terminate only when command use leaves the bounded local workspace surface, invokes clearly off-contract repo/network/package-manager tools, or grows into a clearly pathological loop without producing outputs; bounded local `python`/`python3`/`node` transforms are allowed when every visible path stays inside the workspace; the shared loop budget is intentionally much looser than the earlier startup-only guardrail and now gives some extra tolerance right after real `out/*.json` progress.
...
- recipe no longer uses the path-mode CodexFarm `process` transport either. Recipe worker assignments now start in one long-lived workspace-worker Codex session per worker assignment, split multi-recipe shards into recipe-local task files under `assigned_tasks.json`, aggregate validated task outputs back into shard proposals, and only the near-miss repair pass still drops to one inline structured repair call per affected task. Recipe worker roots now also mirror `tools/recipe_worker.py`, `CURRENT_TASK.md`, and `CURRENT_TASK_FEEDBACK.md`, and the repo prewrites `scratch/<task_id>.json` drafts plus `scratch/_prepared_drafts.json` before the worker starts. The preferred local loop is now the prewritten-draft path: `CURRENT_TASK.md` / `current_task.json` -> open the named `hint` / `input` / `scratch_draft_path` -> local draft edits -> `finalize-all scratch/`, with `prepare-all`, `show`, `check`, and single-task `finalize` kept as fallback/debug verbs instead of ad hoc heredocs or manifest-dump shell loops. `stamp-status` can bulk-mark prepared drafts as `fragmentary` or `not_a_recipe`, bounded inline Python draft edits are tolerated when every visible path stays workspace-local, startup shell commands under the sterile `~/.codex-recipe/...` execution root now classify the same way in live watchdog checks and telemetry, and recipe watchdog recovery is now split by failure shape: mixed-output misses still recover per task, while early whole-shard deaths recover through one shard-packed structured retry so recovered shards land as final `completed` outcomes without the old five-call fan-out.
...
- knowledge promotion is now task-aware at the shard handoff: a shard wrapper that is invalid only for `missing_owned_chunk_results` can still promote chunk rows from its accepted task packets, while missing or semantic-invalid packets remain unreviewed and the rollups count that partial state explicitly.
- knowledge phase manifests now also persist the blast-radius policy explicitly in `runtime_metadata`: `bundle_policy`, `task_packet_total`, `worker_task_packet_counts`, and min/max task packets per worker show how much packet work any one long-lived worker owned.
...
- recipe and line-role helper CLIs now keep the old `scaffold` / `check` / `install` verbs as compatibility fallbacks, but the cheap happy path is now repo-written drafts plus `finalize` / `finalize-all` rather than hand-built JSON. Line-role still treats `prepare` / `prepare-all` as fallback/debug draft generation, while recipe now also prewrites those same drafts before the workspace worker starts, keeps `prepare-all` as a regeneration fallback, writes `scratch/_prepared_drafts.json`, and exposes `stamp-status` for bulk terminal-status stamping before `finalize-all`.
...
- knowledge worker assignments now launch one long-lived workspace-worker Codex session per worker assignment with shared `assigned_shards.json`, a skinny worker-local `assigned_tasks.json`, repo-written `current_task.json`, `CURRENT_TASK.md`, `CURRENT_TASK_FEEDBACK.md`, authoritative `in/*.json`, harvested `out/*.json`, `scratch/`, `OUTPUT_CONTRACT.md`, `examples/`, and `tools/knowledge_worker.py`; the worker starts from the current-task bundle, now gets a representative good-vs-bad snippet example instead of a first-task front-matter example, uses the repo-written helper verbs `current|complete-current|check-current|install-current|next|explain-failure` (with lower-level `overview|show|scaffold|check|install` still mirrored), and the repo advances the queue only after the current task validates while also mirroring the advanced current-task bundle back into the live execution workspace so the same worker session can continue with task N+1. The prompt, `CURRENT_TASK.md`, `CURRENT_TASK_FEEDBACK.md`, and `install-current` stdout now all frame that handoff as one continuing assignment: do not ask permission mid-queue, and do not treat a successful install as a stopping point while later tasks remain.
- those knowledge `out/*.json` files are semantic packet-result candidates, not authoritative compact bundle envelopes; repo code serializes the canonical `v` / `bid` / `r` packet payload after accepting the worker's semantic judgment
- knowledge runtime now also writes `task_manifest.jsonl` plus worker-local `assigned_tasks.json`; those task rows are the repo-owned packet inventory even for single-chunk shards, while the live worker loop is driven by the current leased packet files instead of by the whole task list
...
- the authoritative knowledge contract is now: `knowledge/in/*.json` immutable shard payloads, worker-local semantic packet-result candidates under `workers/*/out/*.json`, `knowledge/proposals/*.json` validated repo-serialized shard proposals, then deterministic promotion into `08_nonrecipe_spans.json`, `09_knowledge_outputs.json`, and reviewer-facing knowledge artifacts
- valid knowledge worker packet files remain authoritative even if the workspace session ends with no final assistant message; `final_agent_message_state` is still recorded in telemetry/live-status for debugging, but follow-up recovery only keys off file-level validation failure
...
- prompt-cost control for this stage lives before worker execution:
  ...
  - `build_knowledge_jobs(...)` bundles nearby chunks into local jobs instead of writing one prompt per chunk, while keeping deterministic judgments out of the billed reviewer payload
