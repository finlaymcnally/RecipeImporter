...
Line-role inline validation is now strict labels-only: live inline turns must return one ordered top-level `labels` array with no extra keys. Each initial and repair packet now also gets its own packet-local output schema with an exact `labels` length, so short arrays can be rejected before promotion rather than only in later repo validation. Deterministic code zips `labels[i]` back onto the shard's true `atomic_index` order, validates count and label legality, narrows repair packets to only unresolved owned rows, allows up to three bounded shard-local repair followups using the newest unresolved ledger state each time, and fails closed only after those repair attempts still leave unresolved rows. It still fails closed if the model returns any row-shaped fallback instead of the labels array.
Line-role repair followups now reuse the prepared execution workspace for stable local paths, but they do not resume the previous Codex transcript. Repair runs as a fresh structured prompt in that same workspace so `--output-schema` stays active on repair too.
