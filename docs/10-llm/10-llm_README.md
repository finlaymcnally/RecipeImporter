...
LLM usage in this repo is optional. The direct-exec transport is now mixed by attempt type rather than by stage: recipe, line-role, and knowledge main work uses assignment-first workspace-worker sessions, while narrow retry / repair follow-up attempts can still use structured one-shot calls. The happy path is now the same across those stages: start from `worker_manifest.json`, open the immutable assignment file, read only the named `hints/*.md` and `in/*.json` files you own, write stable `out/*.json` results, and stop.
...
- workspace-worker sessions now also get a repo-written `worker_manifest.json` in the worker root and mirrored sterile workspace. The worker contract is explicit: start from `worker_manifest.json`, then open the immutable assignment file named there (`assigned_tasks.json` for recipe, `assigned_shards.json` for knowledge and line-role), then use the named `hints/*.md` and `in/*.json` files directly, and stay inside the bounded workspace rather than reaching back into the repo.
- watchdog policy is now split by transport: structured retry / repair calls still treat shell commands as immediate off-contract behavior, while workspace-worker happy paths are assignment-first and file-first rather than queue-first or shell-first.
...
- the relaxed workspace-worker policy now also tolerates bounded local helper commands inside the worker root when the agent genuinely needs them, but the paved road is still direct file reads over the assignment file, `hints/*.md`, `in/*.json`, and stable `out/*.json` targets.
...
- workspace-worker main attempts still terminate primarily on clear boundary/liveness violations. Narrow local helpers may still be tolerated when they stay inside the worker root and do not invent queue schedulers, broad workspace crawls, or repo-boundary escapes.
...
- knowledge main-worker sessions no longer rely on a repo-advanced queue. Missing shard outputs now fail closed from the assignment-first session evidence in `live_status.json`, while invalid shard outputs can get one explicit structured repair attempt and then fail closed.
...
3. The line-role Codex surface reviews the whole book line set in worker-local shard files. The live main path now gives each worker session an immutable shard assignment, named `in/*.json` inputs, optional `hints/*.md`, and stable `out/<shard_id>.json` targets. Deterministic code validates owned-row coverage after the worker writes those shard outputs and fails closed when validation does not pass.
...
5. The recipe Codex surface reviews the recipe side in owned recipe shards. One worker session processes its immutable `assigned_tasks.json` inventory, reads the named `in/*.json` task files, writes one `out/<task_id>.json` per task, and stops. Repo code validates those task outputs, rejoins them into shard proposals, and near-miss invalid tasks can still get one structured repair pass.
...
7. The knowledge Codex surface finalizes the non-recipe side. The program partitions the category-neutral `nonrecipe-route` candidate queue into contiguous shards using the configured prompt target as a hard cap, writes one shard input file per shard under `in/`, assigns those shard ids up front in `assigned_shards.json`, and keeps one long-lived worker session on that immutable assignment. The worker makes the `knowledge|other` and grouping calls directly in one final shard output per assigned shard. Repo code validates exact owned block coverage plus kept-block group coverage, promotes only accepted shard outputs, and uses one explicit repair pass only when deterministic validation rejects a written shard result.
...
- recipe worker assignments now launch one long-lived workspace-worker Codex session per worker assignment with `assigned_tasks.json`, authoritative `in/*.json`, optional `hints/*.md`, and harvested `out/*.json`; recipe tasks own one `recipe_id`, workers write `out/<task_id>.json`, deterministic code validates those task outputs, and shard proposals are rebuilt from the accepted task files
- recipe runtime still writes `task_manifest.jsonl` for reporting, but the live worker surface is the immutable task inventory rather than a repo-advanced packet queue
...
- knowledge worker assignments now launch one long-lived workspace-worker Codex session per worker assignment with `assigned_shards.json`, authoritative `in/*.json`, optional `hints/*.md`, harvested `out/*.json`, worker-local `scratch/*.json`, `OUTPUT_CONTRACT.md`, and `examples/`; deterministic preflight still rejects malformed shards before the worker session starts
...
- `knowledge_phase_workspace_tools.py` still owns the validator and contract helpers for the worker surface. The normal paved road is: open `assigned_shards.json`, open the named shard inputs directly, write exactly one final JSON object to the declared `out/<shard_id>.json` path, and stop. `OUTPUT_CONTRACT.md` and `examples/` are fallback contract/debug surfaces rather than the default first read.
...
- completed knowledge workspace workers should now always carry a non-null `reason_code` in `live_status.json`; successful assignment-first workers finish as ordinary completed sessions once every declared shard output validates, missing outputs fail closed as `process_exited_without_final_packet_state`, invalid outputs can roll into `repair_packet_exhausted` after one repair pass, and true watchdog kills keep their original watchdog reason codes
...
- line-role worker assignments now launch one long-lived workspace-worker Codex session per worker assignment with `assigned_shards.json`, shared `prompt.txt`, worker-local `hints/*.md`, authoritative `in/*.json`, richer `debug/*.json`, and harvested `out/*.json`; the happy path no longer uses `CURRENT_PHASE*`, `work/*.json`, or same-session install/check loops
- the cheap line-role happy path is now: open `assigned_shards.json`, read the named shard input and optional hint directly, write one finished `out/<shard_id>.json`, and stop
...
- line-role worker roots now also carry `worker_manifest.json` and `OUTPUT_CONTRACT.md`. The shared worker prompt frames deterministic labels as weak hints rather than starting truth or tie-breakers, tells the model to read the shard-local hint/input files first, and treats `OUTPUT_CONTRACT.md` as fallback output-shape reference rather than a startup requirement.
- line-role no longer launches a repo-owned post-session repair prompt. If the worker leaves `out/<shard_id>.json` missing or invalid, the runtime may allow one bounded watchdog retry after a retryable `watchdog_killed` outcome and otherwise fails closed without reintroducing a phase-ledger surface.
...
- Recipe worker cost regressions are usually readback regressions now, not watchdog regressions. If recipe token spend spikes again, inspect whether workers have slipped back into raw hint/input rereads, `assigned_tasks.json` inventory dumps, or wider-than-needed workspace exploration before changing retry policy or prompt wording.
...
- line-role main workspace workers now use the same rare-kill watchdog contract as the other workspace stages, and the active control surface is now the same assignment-first contract: start from `worker_manifest.json`, `assigned_shards.json`, and the named shard files. Bounded local orientation and helper shell use inside the worker root are telemetry, not kill reasons, unless they turn into a real boundary violation or a pathological no-progress loop.
...
- Knowledge direct prompts must describe the real assignment-first file contract. The live knowledge worker opens the named shard files from `assigned_shards.json`; it is no longer a leased-packet inline JSON surface.
