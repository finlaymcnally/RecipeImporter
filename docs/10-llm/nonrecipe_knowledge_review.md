...
The knowledge stage is an **optional** shard-worker CodexFarm phase that reviews Stage 7 review-eligible non-recipe spans, is the only semantic authority for review-eligible outside-recipe `knowledge` versus `other`, and extracts **durable cooking knowledge** from the spans that remain knowledge. The positive class is not "anything true about cooking." The positive class is "information worth preserving because it improves future cooking decisions, diagnosis, or technique."
...
Line-role is now strictly routing-only for outside-recipe prose. Useful lesson headings and explanatory cookbook prose should arrive here as review-eligible seed `other`, not provisional line-role `knowledge`. Semantic `knowledge` only appears after this stage reviews and promotes final authority.

Mixed-packet rule:
- when a review packet mixes memoir, praise, author/book framing, or scene-setting with a few reusable cooking sentences, the reviewer must not promote the whole packet by inertia.
- final decisions are block-local: framing blocks stay `other`, and only the narrow blocks that stand alone as reusable cooking guidance should become `knowledge`.
- strong negative utility cues plus borderline mixed signals should start skeptical; a little cause/effect language is not enough by itself to turn a memoir-heavy packet into a strong positive keep.
...
- worker-local hint files and task metadata may summarize utility cues such as cause/effect, troubleshooting, substitutions, storage/safety, memoir, framing, navigation, or true-but-low-utility filler, but those cues stay local guidance only and must not become hidden final authority.
...
- the main worker writes one semantic packet-result object to each task row's declared `metadata.result_path`, using semantic keys such as `packet_id`, `chunk_results`, `chunk_id`, `is_useful`, `block_decisions`, `snippets`, and required `reason_code`. Repo code then serializes the canonical compact `v` / `bid` / `r` packet payload before proposal validation and promotion.
- proposal validation now also synthesizes `reason_code` rollups when a packet re-enters through canonical-bundle compatibility (`v` / `bid` / `r`) rather than the semantic packet contract, so aggregated shard wrappers and `promotion_report.json` keep the same reason-count observability as worker-native semantic results.
...
- semantic packet results now also carry a constrained `reason_code` vocabulary so stage summaries can distinguish why chunks stayed `other` (`book_framing_or_marketing`, `memoir_or_scene_setting`, `navigation_or_chapter_taxonomy`, `true_but_low_utility`, etc.) from why they were kept as useful (`technique_or_mechanism`, `diagnostic_or_troubleshooting`, `reference_or_definition`, `substitution_storage_or_safety`).
- the knowledge worker prompt now also explicitly says to open `worker_manifest.json`, then the batch sidecars, then the repo-named `tasks[*].input_path` / `tasks[*].hint_path` files directly only when the batch packet is insufficient. Ordinary shards now reach that loop as small consecutive multi-chunk task packets. The happy path is `python3 tools/knowledge_worker.py current-batch|complete-batch|check-batch|install-batch|explain-failure`; queue inspection and the older single-task verbs live behind `python3 tools/knowledge_worker.py debug ...` for narrow recovery/debugging only. Avoid broad queue dumps or ad hoc shell schedulers, and keep any helper files in `scratch/` or short-lived local temp roots such as `/tmp`. Strong-cue scaffolds are now visibly prewritten as `strong_cue_review_required`, `install-batch` accepts the longest valid prefix in order, direct writes to `out/` without a passing checker are incomplete work, and both snippet-copy failures and `semantic_all_false_empty_shard` failures now name the narrow chunk-local reason in worker language instead of forcing rediscovery through helper-source inspection.
- the knowledge main-worker watchdog now also fails closed only on ad hoc queue-control rewrites. Bounded local automation over only the active batch drafts is tolerated, helper-source reads of `tools/knowledge_worker.py` are discouraged/telemetry-only rather than an immediate fatal violation, and `assigned_tasks.json` dumps, single-task helper detours, plus broad shell scheduler loops are now discouraged/telemetry-only fallback moves rather than hard watchdog kills. The hard runtime boundary is now “do not rewrite repo-owned batch control state,” not “never be clumsy in shell.”
