...
- the semantic packet worker contract is intentionally narrow: final `category` values are only `knowledge` or `other`, `reviewer_category` carries any richer internal reason, and `snippets[*].body` should be a short grounded extraction rather than a whole-chunk echo.
- the knowledge worker prompt now also explicitly says to open the repo-named `metadata.input_path` and `metadata.hint_path` files directly, avoid broad queue dumps or ad hoc shell schedulers, and keep any helper files in `scratch/` or short-lived local temp roots such as `/tmp`.
