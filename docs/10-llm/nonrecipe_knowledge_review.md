...
- the knowledge worker prompt now also explicitly says to open `worker_manifest.json`, then the current-task sidecars, then the repo-named `metadata.input_path` and `metadata.hint_path` files directly; use `python3 tools/knowledge_worker.py` for the paved-road `complete-current`, `check-current`, and `install-current` loop (with lower-level `overview|show|scaffold|check|install` still available); avoid broad queue dumps or ad hoc shell schedulers; and keep any helper files in `scratch/` or short-lived local temp roots such as `/tmp`. `check` is the paved road before `install`, direct writes to `out/` without a passing check are incomplete work, and snippet-copy failures now name the offending chunk/snippet plus quote preview in worker language.
