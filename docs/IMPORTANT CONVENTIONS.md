...
- Stage conversion reports are written at the run root as `<workbook_slug>.excel_import_report.json` (not under a `reports/` subfolder).
...
- Run folder timestamps use `YYYY-MM-DD_HH.MM.SS` in current code (for example `2026-02-10_23.21.33`) across stage outputs, Label Studio import/export run folders, and benchmark eval folders.
