...
- interactive benchmark setup applies Codex choices by patching the existing `RunSettings` surfaces, not benchmark-only shadow settings:
  - recipe pipeline: `off | codex-farm-3pass-v1 | codex-farm-2stage-repair-v1`
  - block labelling: `deterministic-v1 | codex-line-role-v1`
  - knowledge harvest: `off | codex-farm-knowledge-v1`
...
- `analysis.recipe_pipeline_context` and `analysis.stage_separated_comparison` should now be read as topology-model outputs, not as script-local guesses from current raw folder names. The shared upload-bundle model is the seam to update when recipe-stage topology changes.
