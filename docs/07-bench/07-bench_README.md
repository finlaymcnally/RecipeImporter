...
- benchmark knowledge runs now use the same single live product behavior as stage: first-pass classification is binary `keep_for_review` versus `other`, and the second pass handles both grouping and shared group grounding/tagging. Benchmark readers should expect group-oriented knowledge counts rather than row-level proposal-candidate counts.
