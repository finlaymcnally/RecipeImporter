...
- the Oracle benchmark wrapper now speaks in semantic lanes first: default operator review is `pro`, helper/test review is `instant`, and `thinking` is an explicit opt-in lane instead of the default policy. `--model ...` accepts those selectors or a literal Oracle model id as a low-level escape hatch. The final Oracle command still receives one concrete model string, resolved from `ORACLE_PRO_MODEL`, `ORACLE_INSTANT_MODEL`, `ORACLE_THINKING_MODEL`, and the older compatibility env names (`ORACLE_GENUINE_MODEL`, `ORACLE_TEST_MODEL`, `ORACLE_REVIEW_MODEL`, `ORACLE_DEEP_REVIEW_MODEL`, `ORACLE_FAST_MODEL`, `ORACLE_FAST_SMOKE_MODEL`).
