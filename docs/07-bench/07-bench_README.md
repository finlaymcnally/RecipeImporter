...
- browser uploads use the canonical Oracle browser profile at `~/.local/share/oracle/browser-profile`; the old `~/.oracle/browser-profile` path is now only a symlink to that same directory
