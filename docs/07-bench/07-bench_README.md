...
- Single-run codex-first `upload_bundle_v1` lane packets are codex-run-first packets: they should stage codex artifacts plus derived summaries only, and should not request missing `vanilla/...` raw files as if every packet were a paired codex-vs-vanilla review.
...
- local starter-pack recipe-triage and call-inventory count fields in the cutdown/reporting layer are row-native now (`input_row_count`, `correction_input_row_count`, `build_intermediate_selected_row_count`, and row-loss variants); do not reintroduce block-count aliases there unless you are intentionally changing the upstream prompt-wire-format contract too
- recipe prompt-artifact selection/context keys on the benchmark recipe surface are row-native now too (`rows_candidate`, `rows_before`, `rows_after`, `rows`, `excluded_row_ids`); keep knowledge transport block-context payloads separate rather than reusing the old recipe block-selection names
...
- upload-bundle active recipe span breakout now projects prompt-derived recipe spans through `line-role-pipeline/projected_spans.jsonl` when that artifact is available; prompt `start_row_index` / `end_row_index` values are source-row coordinates, not canonical scored line coordinates
- upload-bundle recipe-stage blame uses that same projected span mapping so recipe-local changed lines do not silently fall back into the default `line_role` bucket just because prompt evidence was recorded in source row indices
...
- `semantic_row_predictions.json` (`schema_version=semantic_row_predictions.v1`)
  - the current staging owner map for that artifact is: `recipe_row_evidence.py` for recipe-local evidence, `knowledge_row_evidence.py` for final non-recipe evidence, `row_label_resolution.py` for priority rules, and `semantic_row_predictions.py` as the assembly root for `semantic_row_predictions.json`
