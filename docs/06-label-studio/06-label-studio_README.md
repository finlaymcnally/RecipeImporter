...
- Active Label Studio export -> row-gold derivation also expects row-native `source_map.rows[*]` / `touched_rows[*]` with `row_index` plus `source_block_index`; do not rely on `block_index` when the task payload is already row-native.
...
- Rebuilt row-gold seed tasks should pass row-native task rows (`row_index`, `source_block_index`, `source_block_id`) into `freeform_tasks.py`; do not rely on `block_index` when the input is already row-like.
