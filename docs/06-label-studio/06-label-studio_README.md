...
- Migration footgun: row-native freeform exports may carry `touched_rows[].block_index` as the row index while the real source block lives in `touched_rows[].source_block_index`. `migrate_to_source_rows.py` must prefer `source_block_index` when projecting freeform spans back onto `source_rows.jsonl`, or recipe labels can silently land on the wrong rows and flatten `row_gold_labels.jsonl`.
- Newer row-native freeform exports already name exact `touched_blocks[].row_id` rows and row-local `segment_start` / `segment_end` spans. Migration must trust that exact row mapping first; re-running block-wide overlap math against those row-local ranges will smear later rows in the same source block backward onto earlier rows and create fake conflicts/ambiguous rows.
