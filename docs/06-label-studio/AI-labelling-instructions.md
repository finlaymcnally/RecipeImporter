...
- Extra notes specific to the CURRENT recipe (not a distinct alternate version):
  - tips, storage, make-ahead, serving suggestions, substitutions that don’t define a new variant,
    warnings (“don’t overmix”), sourcing for an ingredient used above, etc.
...
- If it’s a small tip and not a distinct version, use RECIPE_NOTES instead.
...
OTHER
- Anything that doesn’t fit the above labels, including:
...
Blocks:
{ONE_JSON_OBJECT_PER_BLOCK_HERE}