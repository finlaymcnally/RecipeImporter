...
- Benchmark smoke should also keep one isolated-cache offline vanilla single-book rerun that exercises the real deterministic prep build on the first pass and a cache hit on the second pass; pin `COOKIMPORT_BOOK_CACHE_ROOT` to a temp-local path so repo-global cache state cannot make that assertion flaky.
...
- `tests/cli/test_run_settings_adapters.py` should keep one cheap contract test that binds adapter-built kwargs against the real registered `stage(...)` and `labelstudio_benchmark(...)` signatures, so new `RunSettings` fields cannot drift into interactive/runtime `unexpected keyword argument` failures.
